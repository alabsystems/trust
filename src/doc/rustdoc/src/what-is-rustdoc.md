...
The standard Trust distribution ships with a tool called `trustdoc`. Its job is
to generate documentation for Rust projects. On a fundamental level, Trustdoc
...
Let's run `trustdoc` on our code. To do so, we can call it with the path to
...
See [Using trustdoc with Cargo](#using-trustdoc-with-cargo).

## Configuring trustdoc

There are two problems with this: first, why does it
think that our crate is named "lib"? Second, why does it not have any
contents?

The first problem is due to `trustdoc` trying to be helpful; like `trustc`,
...
```bash
$ trustdoc src/lib.rs --crate-name docs
```

Now, `doc/docs/index.html` will be generated, and the page says "Crate docs."

For the second issue, it is because our function `foo` is not public; `trustdoc`
...
## Using trustdoc with Cargo

Cargo also has integration with `trustdoc` to make it easier to generate
docs. Instead of the `trustdoc` command, we could have done this:
...
Internally, `cargo doc` calls out to `trustdoc` like this:

```bash
$ trustdoc --crate-name docs src/lib.rs -o <path>/docs/target/doc -L
...
`src/lib.rs`. But what about those other arguments?
 ...
 - `-L` flag helps trustdoc find the dependencies your code relies on.
...
`trustdoc` can also generate HTML from standalone Markdown files. Let's
give it a try: create a `README.md` file with these contents:

````text
# Docs

This is a project to test out `trustdoc`.

[Here is a link!](https://github.com/alabsystems/Trust)
...
`trustdoc` is the Trust documentation entrypoint.

This covers the simplest use-cases of `trustdoc`. The rest of this book will
explain all of the options that `trustdoc` has, and how to use them.
