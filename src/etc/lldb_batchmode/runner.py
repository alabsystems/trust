...
def start_watchdog():
    ...
    def watchdog():
        ...
        # Trust: `interrupt_main()` only raises KeyboardInterrupt once the main
        # thread executes Python bytecode again. If the main thread is blocked
        # inside a native LLDB call — e.g. `run` waiting on a debuggee whose
        # macOS launch/attach wedged, leaving it suspended in T state — the
        # interrupt is never delivered and the whole LLDB process hangs forever
        # (observed: run exec-c30dc44143, every concurrent lldb debuginfo test
        # wedged). Give the interrupt a grace period, then hard-exit: a bounded
        # failure beats an unbounded hang. (This thread is a daemon, so if the
        # session finishes normally in the meantime, LLDB just exits and this
        # code never runs.)
        grace_deadline = clock() + 10
        while clock() < grace_deadline:
            time.sleep(1)
        print(
            "TIMEOUT: KeyboardInterrupt was not delivered (main thread is stuck "
            "in a native LLDB call). Hard-exiting LLDB."
        )
        sys.stdout.flush()
        sys.stderr.flush()
        os._exit(3)
