...
impl LinkCollector<'_, '_> {
    ...
    fn report_rawptr_assoc_feature_gate(
        ...
        .with_note("trustdoc does not allow disambiguating between `*const` and `*mut`, and pointers are unstable until it does")
        .emit();
    }
...
impl Disambiguator {
    ...
    fn from_str(link: &str) -> Result<Option<(Self, &str, &str)>, (String, Range<usize>)> {
        ...
        let suffixes = [
            // If you update this list, please also update the relevant Trust documentation section!
...
fn disambiguator_error(
    ...
    report_diagnostic(cx.tcx, BROKEN_INTRA_DOC_LINKS, msg, &diag_info, |diag, _sp, _link_range| {
        diag.note(
            "see the Trust rustdoc book's namespaces and disambiguators section for more info about disambiguators",
        );
    });
}
        ...
        |diag, sp, _link_range| {
            let note = match err {
                ...
                MalformedGenerics::HasFullyQualifiedSyntax => {
                    diag.note("see issue #74563 for more information");
