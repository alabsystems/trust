...
fn wrapped_rustc_command(rustc_wrappers: &[PathBuf], rustc_binary: &Path) -> Command {
    ...
    let exe = args.next().expect("unable to create trustc command");
...
fn run_test(
    ...
    let rustc_binary = match rustdoc_options
        .test_builder
        .as_deref()
        .or_else(|| rustc_interface::util::rustc_path(sysroot))
    {
        Some(rustc_binary) => rustc_binary,
        None => {
            eprintln!("error: unable to locate canonical Trust compiler `trustc` for doctests");
...
impl IndividualTestOptions {
    fn new(options: &RustdocOptions, test_id: &Option<String>, test_path: PathBuf) -> Self {
        let outdir = if let Some(ref path) = options.persist_doctests {
            ...
            DirState::Temp(get_doctest_dir(options).expect("trustdoc needs a tempdir"))
        };
