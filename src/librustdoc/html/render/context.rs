...
impl<'tcx> Context<'tcx> {
    ...
    fn render_item(&mut self, it: &clean::Item, is_module: bool) -> String {
        ...
        if !render_redirect_pages {
            ...
            title.push_str(" - Trust");
            let tyname = it.type_();
            let desc = plain_text_summary(&it.doc_value(), &it.link_names(self.cache()));
            let desc = if !desc.is_empty() {
                desc
            } else if it.is_crate() {
                format!("API documentation for the Trust `{}` crate.", self.shared.layout.krate)
            } else {
                format!(
                    "API documentation for the Trust `{name}` {tyname} in crate `{krate}`.",
...
impl<'tcx> FormatRenderer<'tcx> for Context<'tcx> {
    ...
    fn after_krate(mut self) -> Result<(), Error> {
        ...
        if shared.should_merge.write_rendered_cci {
            ...
            page.title = "Settings";
            page.description = "Settings of Trustdoc";
            page.root_path = "./";
            page.rust_logo = true;
            ...
            page.title = "Help";
            page.description = "Documentation for Trustdoc";
            page.root_path = "./";
            page.rust_logo = true;
                ...
                format_args!(
                    "<div class=\"main-heading\">\
                        ...
                        <section>\
                            ...
                            <p>For more information, use the Trust documentation shipped with this toolchain.</p>\
                        </section>\
                        </noscript>",
                ),
                &shared.style_files,
        ...
        if shared.layout.scrape_examples_extension && shared.should_merge.write_rendered_cci {
            ...
            page.description = "How the scraped examples feature works in Trustdoc";
            let v = layout::render(
                &shared.layout,
                &page,
                "",
