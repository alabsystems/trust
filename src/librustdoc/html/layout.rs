...
struct PageLayout<'a> {
    ...
    // where these things instead give Trustdoc name and version.
    //
    // These are separate from the variables used for the search
    // engine, because "Trustdoc" isn't necessarily a crate in
...
pub(crate) fn render<T: Display, S: Display>(
    ...
    let (display_krate, display_krate_version, display_krate_with_trailing_slash) =
        if page.root_path == "./" {
            // top level pages use Trust branding
            ("Trustdoc", rustdoc_version, String::new())
        } else {
