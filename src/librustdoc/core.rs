...
pub(crate) fn run_global_ctxt(
    ...
    if krate.module.doc_value().is_empty() {
        let help = String::from(
            ...
            the Trust rustdoc book's how to write documentation chapter",
        );
        tcx.emit_node_lint(
...
impl<'tcx> Visitor<'tcx> for EmitIgnoredResolutionErrors<'tcx> {
    ...
    fn visit_path(&mut self, path: &Path<'tcx>, _id: HirId) {
        ...
        if path.res == Res::Err {
            ...
            .with_note("this error was originally ignored because you are running `trustdoc`")
            .with_note("try running again with `trustc` or `targo trust check` and you may get a more detailed error")
            .emit();
        }
