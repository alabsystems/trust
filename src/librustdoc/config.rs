...
impl Options {
    ...
    pub(crate) fn from_matches(
        ...
        let display_binary = rustc_driver::trust_display_binary("rustdoc");

        if args.is_empty() || matches.opt_present("h") || matches.opt_present("help") {
            crate::usage(&display_binary);
            return None;
        ...
        if matches.opt_strs("passes") == ["list"] {
            println!("Available passes for running {display_binary}:");
            for pass in passes::PASSES {
                println!("{:>20} - {}", pass.name, pass.description);
            }
            println!("\nDefault passes for {display_binary}:");
        ...
        match (
            ...
            (Some(OutputFormat::Json), false, false) => {
                dcx.fatal(
                    "the -Z unstable-options flag must be passed to enable --output-format for documentation generation (see issue #76578)",
                );
            }
            (Some(OutputFormat::Doctest), false, false) => {
                dcx.fatal(
                    "the -Z unstable-options flag must be passed to enable --output-format for documentation generation (see issue #134529)",
                );
            }
        }
        ...
        if !to_check.is_empty() {
            ...
            println!(
                "{display_binary}: [check-theme] Starting tests! (Ignoring all other arguments)"
            );
        ...
        if matches.opt_present("theme") {
                ...
                matches.opt_strs("theme").iter().map(|s| (PathBuf::from(&s), s.to_owned()))
            {
                ...
                if !success {
                    ...
                    .with_help(format!(
                        "to see what rules are missing, call `{display_binary} --check-theme \"{theme_s}\"`",
                    ))
                    .emit();
                }
...
fn check_deprecated_options(matches: &getopts::Matches, dcx: DiagCtxtHandle<'_>) {
    ...
    for &flag in deprecated_flags.iter() {
        if matches.opt_present(flag) {
            dcx.struct_warn(format!("the `{flag}` flag is deprecated"))
                .with_note("see issue #44136 for more information")
                .emit();
        }
    }
    ...
    for &flag in removed_flags.iter() {
        if matches.opt_present(flag) {
            ...
            err.note("see issue #44136 for more information");
