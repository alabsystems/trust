...
pub enum CodegenBackendKind {
    #[default]
    Llvm,
    // Trust: trust-cg verified codegen backend (#829).
    TrustCg,
    Custom(String),
}
...
impl CodegenBackendKind {
    ...
    pub fn name(&self) -> &str {
        match self {
            ...
            // Trust: trust-cg verified codegen backend (#829).
            CodegenBackendKind::TrustCg => "trust-cg",
            CodegenBackendKind::Custom(name) => name,
        }
    }

    /// Name of the codegen backend's crate, e.g. `rustc_codegen_llvm`.
    pub fn crate_name(&self) -> String {
        format!("rustc_codegen_{}", self.name().replace('-', "_"))
    }
    ...
    // Trust: trust-cg verified codegen backend (#829).
    pub fn is_trust_cg(&self) -> bool {
        matches!(self, Self::TrustCg)
    }
}
...
pub struct Build {
    ...
    // Version information
    //
    // Trust: TWO version lines, deliberately. `version` is Trust's own
    // (major.minor.dev — release/trust-version.toml) and names the product:
    // dist archives, release evidence, `targo trust version`, and the `trust:`
    // line in `trustc -Vv`. `rust_compat_version` is the version the compiler
    // reports through rustc's `-V` protocol, which the crates.io ecosystem
    // (autocfg, rustc_version, version_check) parses in build scripts and which
    // backs `cfg(version)` and stability attributes. Conflating them is what
    // made a Trust-native version un-shippable: Trust's identity is 0.1.0, but
    // a compiler claiming `rustc 0.1.0` stops being drop-in.
    version: String,
    rust_compat_version: String,
...
impl Build {
    ...
    pub fn new(mut config: Config) -> Build {
        ...
        let rust_compat_version =
            std::fs::read_to_string(src.join("src").join("rust-compat-version"))
                .expect("failed to read src/rust-compat-version");
        let rust_compat_version = rust_compat_version.trim();
        ...
        let mut build = Build {
            ...
            rust_compat_version: rust_compat_version.to_string(),
            src,
            out,
            bootstrap_out,
    ...
    pub fn require_submodule(&self, submodule: &str, err_hint: Option<&str>) {
        ...
        // Trust carries former upstream submodules as ordinary source trees.
        // With no `.gitmodules` file there is nothing for `git submodule` to
        // update; the existence check below is the authoritative local-source
        // gate.
    ...
    fn update_existing_submodules(&self) {
        ...
        if !self.config.submodules() || !self.src.join(".gitmodules").exists() {
            return;
        }
    ...
    fn rustc_features(&self, kind: Kind, target: TargetSelection, crates: &[String]) -> String {
        ...
        // Trust: Activate trust-cg feature when trust-cg backend is enabled (#956).
        if self.config.enabled_codegen_backends(target).contains(&CodegenBackendKind::TrustCg)
            && check("trust-cg")
        {
            features.push("trust-cg");
        }
        if self.config.llvm_enzyme {
    ...
    fn release(&self, num: &str) -> String {
        match &self.config.channel[..] {
            ...
            // Keep the compiler release string parseable by the ecosystem's
            // rustc_version crate. Trust package/archive identity remains
            // trust-specific via rust_package_vers and distribution metadata.
            "trust" => format!("{num}-dev"),
            _ => format!("{num}-dev"),
        }
    }
    ...
    /// The rustc-protocol release string (`CFG_RELEASE`).
    ///
    /// Trust: sourced from the Rust COMPAT line, not Trust's own version — see
    /// the `rust_compat_version` field. This is what build scripts parse.
    fn rust_release(&self) -> String {
        self.release(&self.rust_compat_version)
    }

    /// Trust's own product version (`CFG_TRUST_VERSION`), `major.minor.dev`.
    fn trust_version(&self) -> &str {
        &self.version
    }
    ...
    fn rust_package_vers(&self) -> String {
        match &self.config.channel[..] {
            ...
            "trust" => format!("{}-trust", self.version),
    ...
    fn rust_version(&self) -> String {
        let mut version = self.rust_info().version(self, &self.rust_compat_version);
