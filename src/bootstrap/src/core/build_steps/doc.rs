...
//! for the Trust toolchain: the tool books (`trustc`, `trustdoc`, `targo`,
//! `tippy`), the unstable book, the standard-library API docs and the
//! standalone pages.
//!
...
// Trust: every book listed here documents a tool this toolchain actually
// ships. Prose about the Rust language and its ecosystem is upstream's
// product, not Trust's, and is no longer carried in-tree — which is also why
// none of these paths is a submodule any more.
book!(
    CargoBook, "src/tools/targo/src/doc", "cargo", &[];
    ClippyBook, "src/tools/tippy/book", "tippy", &[];
...
impl<P: Step> Step for RustbookSrc<P> {
    ...
    fn metadata(&self) -> Option<StepMetadata> {
        Some(StepMetadata::doc(&format!("{} (book)", self.name), self.target))
    }
}
