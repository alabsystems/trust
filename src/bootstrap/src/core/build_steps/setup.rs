...
impl Profile {
    ...
    pub fn purpose(&self) -> String {
        ...
        match self {
            ...
            Tools => "Contribute to tools which depend on the compiler, but do not modify it directly (e.g. trustdoc, tippy, trust-miri)",
            Dist => "Install Trust from source",
...
impl FromStr for Profile {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // Trust: the `--profile=<name>` argument is user-facing, so the
        // tool-themed shortcuts here use Trust-canonical names (the same
        // spellings the `tools = [...]` list accepts after commit
        // d3d70a6ab1). The inherited Rust spellings (`rustdoc`, `clippy`,
        // `miri`, `rustfmt`) are deliberately not accepted, matching
        // INSTALL.md and `tool_matches_config_entry`.
        match s {
            "lib" | "library" => Ok(Profile::Library),
            "compiler" => Ok(Profile::Compiler),
            "maintainer" | "dist" | "user" => Ok(Profile::Dist),
            "tools" | "tool" | "trustdoc" | "tippy" | "trust-miri" | "trustfmt" => {
                Ok(Profile::Tools)
            }
...
pub fn setup(config: &Config, profile: Profile) {
    let suggestions: &[&str] = match profile {
        ...
        Profile::Tools => &[
            ...
            "build",
            "test tests/rustdoc*",
            "test src/tools/tippy",
            "test src/tools/miri",
            "test src/tools/trustfmt",
        ],
...
impl Step for Link {
    ...
    fn run(self, builder: &Builder<'_>) -> Self::Output {
        ...
        if !stage_dir_exists(&stage_path) {
            eprintln!(
                "WARNING: `{}` does not exist; build that stage before linking `{toolchain_name}`.",
                stage_path.display()
            );
            return;
        }

        if !stage_has_real_rustc(&stage_path) {
            eprintln!(
                "WARNING: `{}` does not contain a built rustc; rebuild before linking `{toolchain_name}`.",
                stage_path.display()
            );
            return;
        }
...
fn stage_has_real_rustc(stage_path: &Path) -> bool {
    let rustc = stage_path.join("bin").join(format!("rustc{EXE_SUFFIX}"));
    fs::metadata(rustc).is_ok_and(|metadata| metadata.is_file() && metadata.len() > 0)
}
...
pub fn interactive_path() -> io::Result<Profile> {
    ...
    println!("Welcome to the Trust project! What do you want to do with x.py?");
...
fn install_git_hook_maybe(builder: &Builder<'_>, config: &Config) -> io::Result<()> {
    ...
    println!(
        "\nTrust review/CI gates will fail if this checkout doesn't pass `tidy`, the internal tool for ensuring code quality.
