...
use crate::utils::helpers::{
    ...
    dylib_path, dylib_path_var, exe, linker_args, linker_flags, t, up_to_date,
};
use crate::utils::render_tests::{add_flags_and_try_run_tests, try_run_tests};
use crate::utils::shared_helpers::{TRUST_CARGO_TEST_SHIM_CONFIG, TRUST_CARGO_TEST_SHIM_VERSION};
use crate::{CLang, FileType, GitRepo, Mode, PathSet, TestTarget, envify};
...
const TARGO_TEST_PACKAGE_NAMES: [&str; 11] = [
    "cargo",
    "build-rs-test-lib",
    "cargo-credential",
    "cargo-platform",
...
fn targo_test_packages() -> [String; 11] {
    TARGO_TEST_PACKAGE_NAMES.map(str::to_owned)
}

impl Cargo {
    const CRATE_PATH: &str = "src/tools/targo";
}
...
impl Step for Cargo {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        let tested_rustdoc = builder.rustdoc_for_compiler(tested_compiler);
        ...
        // The workspace root is also the `cargo` package, so Cargo's default
        // package selection omits member-crate tests. Keep every workspace
        // package that owns a unit suite under the normal Targo selector.
        let packages = targo_test_packages();
        let mut cargo = prepare_cargo_test(cargo, &[], &packages, self.host, builder);
        ...
        // Trust's compiler is batteries-on, while Cargo's inherited fixture
        // programs test Cargo rather than constituting a verified-Rust corpus.
        // Give nested fixtures dedicated rustc/rustdoc shim copies with an
        // adjacent, collision-free context sidecar. The shim temporarily hides
        // fixture-authored bootstrap-private env while choosing its compiler,
        // then restores it before spawning so ordinary Cargo `[env]` semantics
        // remain byte-for-byte observable to the compiled program.
        let rustc_shim = PathBuf::from(
            cargo
                .get_envs()
                .find(|(key, _)| *key == OsStr::new("RUSTC"))
                .and_then(|(_, value)| value)
                .expect("bootstrap Cargo test command must name its rustc shim")
                .to_os_string(),
        );
        let rustdoc_shim = rustc_shim.with_file_name(exe("rustdoc", self.host));
        let test_shim_dir = builder
            .out
            .join(tested_compiler.host)
            .join(format!("cargo-test-shims-stage{}", tested_compiler.stage));
        if !builder.config.dry_run() {
            t!(fs::create_dir_all(&test_shim_dir));
        }
        builder.copy_link(
            &rustc_shim,
            &test_shim_dir.join(exe("rustc", self.host)),
            FileType::Executable,
        );
        builder.copy_link(
            &rustdoc_shim,
            &test_shim_dir.join(exe("rustdoc", self.host)),
            FileType::Executable,
        );

        let tested_rustc = builder.rustc(tested_compiler);
        let tested_libdir = builder.rustc_libdir(tested_compiler);
        let mut shim_context = BTreeMap::new();
        let mut insert_path = |key: &str, value: &Path| {
            shim_context.insert(
                key.to_owned(),
                value
                    .to_str()
                    .unwrap_or_else(|| {
                        panic!("{key} path must be valid Unicode: {}", value.display())
                    })
                    .to_owned(),
            );
        };
        insert_path("RUSTC_REAL", &tested_rustc);
        insert_path("RUSTC_SNAPSHOT", &tested_rustc);
        insert_path("RUSTC_LIBDIR", &tested_libdir);
        insert_path("RUSTC_SNAPSHOT_LIBDIR", &tested_libdir);
        insert_path("RUSTDOC_REAL", &tested_rustdoc);
        insert_path("RUSTDOC_LIBDIR", &tested_libdir);
        insert_path("RUSTC_SYSROOT", &builder.sysroot(tested_compiler));
        shim_context.insert("RUSTC_STAGE".into(), tested_compiler.stage.to_string());
        shim_context
            .insert("CFG_COMPILER_BUILD_TRIPLE".into(), tested_compiler.host.triple.to_string());
        shim_context.insert(
            "RUSTC_LINK_STD_INTO_RUSTC_DRIVER".into(),
            if builder.link_std_into_rustc_driver(self.host) { "1" } else { "0" }.into(),
        );
        shim_context.insert("TRUST_BOOTSTRAP_SHIM_NO_VERIFY".into(), "1".into());
        shim_context
            .insert("TRUST_CARGO_TEST_SHIM_VERSION".into(), TRUST_CARGO_TEST_SHIM_VERSION.into());
        if !builder.config.dry_run() {
            t!(fs::write(
                test_shim_dir.join(TRUST_CARGO_TEST_SHIM_CONFIG),
                t!(serde_json::to_vec(&shim_context)),
            ));
        }

        cargo.env("TEST_TRUST_SHIM_DIR", &test_shim_dir).env("TRUST_TARGO_TEST_NO_VERIFY", "1");
        ...
        let mut existing_dylib_paths = cargo
            ...
            .unwrap_or_default();
        // `add_dylib_path` prepends each entry in iteration order, so the last
        // supplied path receives the highest precedence. Keep the tested
        // compiler last here: resolving its hard-linked rustc driver through
        // the build compiler's sysroot makes rustc infer that older sysroot
        // and rejects its standard library with E0514.
        existing_dylib_paths.push(builder.rustc_libdir(tested_compiler));
...
/// Runs `cargo test` for the shipped `targo-trust` verifier frontend.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
impl TCargoTrust {
    const CRATE_PATH: &str = "targo-trust";
}
...
impl Step for TCargoTrust {
    ...
    fn run(self, builder: &Builder<'_>) {
        // Test the same feature surface that bootstrap ships. Building the
        // production tool first also catches divergence between the ToolBuild
        // recipe and this test recipe before any test binary is executed.
        builder.ensure(tool::TCargoTrust::from_build_compiler(self.build_compiler, self.host));

        let record_failed_tests = builder.ensure(SetupFailedTestsFile);
        let features = tool::targo_trust_in_process_features();
        ...
        cargo.allow_features(tool::CARGO_TRUST_ALLOW_FEATURES);

        run_cargo_test(cargo, &[], &[], "targo-trust", self.host, builder, record_failed_tests);
    }

    fn metadata(&self) -> Option<StepMetadata> {
        Some(StepMetadata::test("targo-trust", self.host).built_by(self.build_compiler))
    }
}
...
const TRUSTFMT_TEST_PACKAGE_NAMES: [&str; 2] = ["rustfmt-nightly", "rustfmt-config_proc_macro"];

fn trustfmt_test_packages() -> [String; 2] {
    TRUSTFMT_TEST_PACKAGE_NAMES.map(str::to_owned)
}
...
impl Step for Rustfmt {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        // A path dependency is built but its own tests are not selected by a
        // root-package `cargo test`. Explicitly include the configuration
        // proc-macro crate whose diagnostics and serialization shape are part
        // of Trustfmt's supported configuration surface.
        let packages = trustfmt_test_packages();
        run_cargo_test(cargo, &[], &packages, "rustfmt", target, builder, record_failed_tests);
    }
...
#[cfg(test)]
#[path = "test/tests/tool_test_package.rs"]
mod tool_test_package_tests;
...
const TIPPY_TEST_PACKAGE_NAMES: [&str; 6] =
    ["tippy", "clippy_config", "clippy_utils", "clippy_lints", "rustc_tools_util", "clippy_dev"];

fn tippy_test_packages() -> [String; 6] {
    TIPPY_TEST_PACKAGE_NAMES.map(str::to_owned)
}
...
impl Step for Clippy {
    ...
    fn should_run(run: ShouldRun<'_>) -> ShouldRun<'_> {
        run.suite_path("src/tools/tippy/tests").path("src/tools/tippy")
    }
    ...
    /// Runs Tippy's inherited Clippy-compatible test suite.
    fn run(self, builder: &Builder<'_>) {
        let target = self.compilers.target();
        ...
        // Build the production driver recipe first, then exercise the same
        // target-dependent feature surface below. This makes `x test
        // src/tools/tippy` cover the binary that a configured distribution
        // actually ships (notably the jemalloc-enabled macOS jobs).
        builder.ensure(tool::Clippy::from_compilers(self.compilers));
        let features = tool::tippy_driver_features(builder, target);
        ...
        // Tippy's UI dependency builder launches a nested Cargo process with
        // bootstrap's rustc shim. Pass the shim context under TEST_* names so
        // the test binary can restore it deliberately instead of relying on
        // ambient variables that Cargo does not guarantee to preserve.
        cargo.env("TEST_RUSTC_STAGE", target_compiler.stage.to_string());
        cargo.env("TEST_COMPILER_BUILD_TRIPLE", target_compiler.host.triple);
        cargo.env(
            "TEST_RUSTC_LINK_STD_INTO_RUSTC_DRIVER",
            if builder.link_std_into_rustc_driver(target) { "1" } else { "0" },
        );
        // Trust: direct UI compiles bypass the bootstrap rustc shim and invoke
        // the staged lint driver, which enters rustc through the no-evidence
        // boundary. That policy turns verification off for the lint run on its
        // own and fatally rejects every `-Ztrust-*` option, so the harness must
        // pass no Trust control here and needs no marker saying it may.
        ...
        'partially_test: {
            ...
            for path in paths {
                match helpers::is_valid_test_suite_arg(path, "src/tools/tippy/tests", builder) {
                    TestFilterCategory::Arg(path) => {
                        test_names.push(path);
                    }
                    TestFilterCategory::Fullsuite => {
                        // When src/tools/tippy is called directly, all tests should be run.
        ...
        cargo.add_rustc_lib_path(builder);
        // Cargo otherwise tests only the manifest's root `tippy` package. Run
        // every package that owns a unit suite, including developer tooling.
        // `declare_clippy_lint` has no tests and is built transitively here;
        // selecting it directly creates an invalid rustc-private libtest that
        // asks for compiler crates in rlib form. `clippy_lints_internal` also
        // has no unit tests and is covered by Tippy's `--features internal`
        // dogfood path; its internal UI suite cannot run in rustc's stage1
        // bootstrap because those tests reuse normal-build dependency files.
        let packages = tippy_test_packages();
        let cargo = prepare_cargo_test(cargo, &[], &packages, target, builder);

        let _guard = builder.msg_test("tippy", target, target_compiler.stage);
    ...
    fn metadata(&self) -> Option<StepMetadata> {
        Some(
            StepMetadata::test("tippy", self.compilers.target())
                .built_by(self.compilers.build_compiler()),
        )
    }
}
...
#[cfg(test)]
#[path = "test/tests/tippy_test_package.rs"]
mod tippy_test_package_tests;
...
HELP: to skip test's attempt to check tidiness, pass `--skip src/tools/tidy` to `x.py test`",
        ...
        // A dry run records the scheduled Tidy step but must not inspect generated repository
        // artifacts. Command execution above is already suppressed by the dry-run command
        // wrapper; keep the direct completion/help comparisons equally hermetic. Otherwise
        // builder snapshot tests fail based on unrelated checkout state instead of the graph they
        // are testing.
        if builder.config.dry_run() {
            return;
        }
...
const RUSTC_PRIVATE_SANITY_CRATES: &[&str] =
    &["librustc_span-", "librustc_data_structures-", "libhashbrown-"];

fn rustc_private_sysroot_dir_contains(dir: &Path, crate_prefix: &str) -> bool {
    t!(fs::read_dir(dir)).any(|entry| {
        let entry = t!(entry);
        entry.file_name().to_str().is_some_and(|file_name| file_name.starts_with(crate_prefix))
    })
}

fn verify_rustc_private_sysroot_dirs(
    suite: &str,
    rustc_build_compiler: Compiler,
    rustc_private_target: TargetSelection,
    label: &str,
    host_dir: &Path,
    target_dir: &Path,
) {
    for (kind, dir) in [("host", host_dir), ("target", target_dir)] {
        if !dir.is_dir() {
            panic!(
                "prepared rustc_private {kind} directory for {suite} is missing: {}; \
                 compiler stage{} ({}) targeting {}; refusing to pass a stale -L search path",
                dir.display(),
                rustc_build_compiler.stage,
                rustc_build_compiler.host,
                rustc_private_target,
            );
        }
    }

    let missing_crates = RUSTC_PRIVATE_SANITY_CRATES
        .iter()
        .filter(|crate_prefix| {
            !rustc_private_sysroot_dir_contains(host_dir, crate_prefix)
                && !rustc_private_sysroot_dir_contains(target_dir, crate_prefix)
        })
        .copied()
        .collect::<Vec<_>>();
    ...
    if !missing_crates.is_empty() {
        panic!(
            "prepared rustc_private sysroot for {suite} is missing required artifacts [{}]; \
             host dir: {}; target dir: {}; compiler stage{} ({}) targeting {}; \
             refusing to pass a stale -L search path",
            missing_crates.join(", "),
            host_dir.display(),
            target_dir.display(),
            rustc_build_compiler.stage,
    ...
    println!(
        "NOTE: verified rustc_private sysroot for {suite} ({label}): host {}; target {}",
        host_dir.display(),
        target_dir.display(),
    );
}
...
fn compiletest_rustc_path(
    mut inherited_path: PathBuf,
    local_trust_compiler: bool,
    vanilla: bool,
    rustc_file_name: &str,
    trustc_file_name: &str,
) -> PathBuf {
    if local_trust_compiler {
        // Stage1 retains a `rustc` compatibility hard-link, while normal Trust
        // compiletest expectations are deliberately adapter-branded. Select the
        // appropriate sibling explicitly in both directions; Builder::rustc's
        // default differs between stage1 and stage2.
        inherited_path.set_file_name(if vanilla { rustc_file_name } else { trustc_file_name });
    }
    inherited_path
}

#[derive(Debug, PartialEq, Eq)]
enum CompiletestRustdocPath {
    Direct(PathBuf),
    PrivateAlias { source: PathBuf, destination: PathBuf },
}

fn compiletest_rustdoc_path(
    mut inherited_path: PathBuf,
    local_trust_compiler: bool,
    vanilla: bool,
    stage: u32,
    ...
    private_rustdoc_alias: PathBuf,
) -> CompiletestRustdocPath {
    if !local_trust_compiler {
        return CompiletestRustdocPath::Direct(inherited_path);
    }

    if !vanilla {
        inherited_path.set_file_name(trustdoc_file_name);
        return CompiletestRustdocPath::Direct(inherited_path);
    }

    if stage < 2 {
        inherited_path.set_file_name(rustdoc_file_name);
        CompiletestRustdocPath::Direct(inherited_path)
    } else {
        // Installed stage2+ sysroots intentionally have no `rustdoc` alias.
        // Give vanilla compiletest a private argv identity without weakening
        // the user-facing Trust-branded-names-only policy.
        CompiletestRustdocPath::PrivateAlias {
            source: inherited_path,
            destination: private_rustdoc_alias,
        }
    }
}

#[cfg(test)]
#[path = "test/tests/compiletest_path.rs"]
mod trust_compiletest_path_tests;
...
NOTE: if you're sure you want to do this, please open an issue as to why. In the meantime, you can override this with `--set build.compiletest-allow-stage0=true`."
        ...
        let rustc_private_build_compiler = if suite.ends_with("fulldeps") {
            let rustc_build_compiler = if test_compiler.stage == 0 {
                test_compiler
            } else {
                RustcPrivateCompilers::from_target_compiler(builder, test_compiler).build_compiler()
            };
            let built_rustc = builder.ensure(compile::Rustc::new(rustc_build_compiler, target));
            Some(built_rustc.build_compiler)
        } else {
            None
        };
        ...
        let rustc_supports_trust_verification =
            test_compiler.stage > 0 && !builder.download_rustc();
        let rustc_path = compiletest_rustc_path(
            builder.rustc(test_compiler),
            rustc_supports_trust_verification,
            builder.config.cmd.trust_vanilla(),
            &helpers::exe("rustc", test_compiler.host),
            &helpers::exe("trustc", test_compiler.host),
        );
        cmd.arg("--rustc-path").arg(rustc_path);
        // Locally-built stage1+ compilers support Trust's verification switch.
        // Tell compiletest explicitly so ordinary Rust UI crates and auxiliaries
        // are isolated from batteries-on verification. Never infer this merely
        // from a stage number: download-rustc and forced stage0 testing can use
        // an upstream compiler that must not see Trust-only options.
        if rustc_supports_trust_verification {
            cmd.arg("--rustc-supports-trust-verification");
        }
        ...
        {
            let built_rustdoc = builder.rustdoc_for_compiler(test_compiler);
            let private_rustdoc_alias = testdir(builder, test_compiler.host)
                .join("compiletest-private-tools")
                .join(format!("stage{}", test_compiler.stage))
                .join(suite)
                .join(mode.as_str())
                .join(helpers::exe("rustdoc", test_compiler.host));
            let rustdoc_path = match compiletest_rustdoc_path(
                built_rustdoc,
                rustc_supports_trust_verification,
                builder.config.cmd.trust_vanilla(),
                test_compiler.stage,
                &helpers::exe("rustdoc", test_compiler.host),
                &helpers::exe("trustdoc", test_compiler.host),
                private_rustdoc_alias,
            ) {
                CompiletestRustdocPath::Direct(path) => path,
                CompiletestRustdocPath::PrivateAlias { source, destination } => {
                    if !builder.config.dry_run() {
                        t!(fs::create_dir_all(destination.parent().unwrap()));
                    }
                    builder.copy_link(&source, &destination, FileType::Executable);
                    destination
                }
            };
            cmd.arg("--rustdoc-path").arg(rustdoc_path);
        }
...
HELP: You can add it into `bootstrap.toml` in `rust.codegen-backends = [{name:?}]`",
        ...
        if builder.config.cmd.trust_vanilla() {
            cmd.arg("--trust-vanilla");
        }
        ...
        if suite.ends_with("fulldeps") {
            // Keep this in sync with the `compile::Rustc::new` ensure above: fulldeps tests link
            // to the rustc_private artifacts of the compiler under test, which were produced by
            // that compiler's build compiler. In the stage 1 special case `test_compiler` has
            // already been rewritten to stage 0, which deliberately selects the stage 1 artifacts.
            let rustc_build_compiler =
                rustc_private_build_compiler.expect("fulldeps rustc build compiler missing");
            let mut host_rustc_deps_dirs = Vec::new();
            let mut target_rustc_deps_dirs = Vec::new();

            let prepare_rustc_private_deps = |rustc_deps_dirs: &mut Vec<PathBuf>,
                                              rustc_private_target,
                                              label| {
                if let Some((host_dir, target_dir)) = compile::prepare_rustc_private_sysroot(
                    builder,
                    ...
                    if !builder.config.dry_run() {
                        verify_rustc_private_sysroot_dirs(
                            suite,
                            rustc_build_compiler,
                            rustc_private_target,
                            label,
                    ...
                    rustc_deps_dirs.extend([host_dir, target_dir]);
                } else if !builder.config.dry_run() {
                    panic!(
                        "failed to prepare rustc_private sysroot for {suite}: \
                             missing librustc stamp for compiler stage{} ({}) targeting {}; \
                             refusing to run fulldeps tests without an -L search path",
                        rustc_build_compiler.stage, rustc_build_compiler.host, rustc_private_target,
                    );
                }
            };

            prepare_rustc_private_deps(
                &mut host_rustc_deps_dirs,
                test_compiler.host,
                "ui-fulldeps-host",
            );
            prepare_rustc_private_deps(&mut target_rustc_deps_dirs, target, "ui-fulldeps-target");

            host_rustc_deps_dirs.dedup();
            target_rustc_deps_dirs.dedup();
            // Trust: re-provide the pruned rustc_private crates with `-Lall`, not
            // `-Ldependency`. Upstream ui-fulldeps finds `rustc_span`, `rustc_macros`,
            // `rustc_driver`, ... inside the compiler's own sysroot `rustlib/<host>/lib`,
            // which rustc registers as a `PathKind::All` search path
            // (`SearchPath::from_sysroot_and_triple`) — searched for *every* crate-load
            // kind. Trust prunes rustc_private from the test sysroot (upstream #108767,
            // dist-shaped sysroot parity) and re-hands these coherent dirs to the fulldeps
            // invocation here. A flat `-Ldependency` registers them as `PathKind::Dependency`
            // only, which `PathKind::matches` does NOT match against the `Crate`-kind loads
            // used for direct `extern crate rustc_span;` / proc-macro (`rustc_macros`)
            // resolution — so the crates go unfound (E0463) even though the files are on
            // disk. `-Lall` mirrors the real sysroot's `PathKind::All` exactly, so both the
            // direct extern crates and their transitive (`Dependency`-kind) deps resolve.
            for rustc_deps_dir in host_rustc_deps_dirs {
                hostflags.push(format!("-Lall={}", rustc_deps_dir.display()));
            }
            for rustc_deps_dir in target_rustc_deps_dirs {
                targetflags.push(format!("-Lall={}", rustc_deps_dir.display()));
            }

            // A rustc_private consumer inherits the native libraries named by
            // rustc_driver (LLVM support libraries such as Polly and zstd), but
            // Cargo/rustc metadata does not carry the corresponding native
            // search directories into this separate compiletest invocation.
            // This is the compiletest analogue of the ToolRustcPrivate
            // workaround in `core::builder::cargo`: recover LLVM's own libdir
            // and any absolute system-library parents reported by llvm-config.
            // Homebrew reports some dependencies as bare `-l` names, so also
            // include its common prefix libdir when llvm-config lives below an
            // `opt` directory. Missing/non-directory candidates are ignored.
            if let Some(llvm_config) = builder.llvm_config(target) {
                ...
                if llvm_libdir.is_dir() {
                    native_dirs.push(llvm_libdir);
                }

                let system_libs = command(&llvm_config)
                    .cached()
                ...
                for token in system_libs.split_whitespace() {
                        ...
                        && parent.is_dir()
                    {
                        ...
                        if !native_dirs.contains(&parent) {
                            native_dirs.push(parent);
                        }
                    }
                }

                if let Some(opt_dir) =
                    llvm_config.ancestors().find(|path| path.file_name() == Some(OsStr::new("opt")))
                    && let Some(prefix) = opt_dir.parent()
                {
                    let common_libdir = prefix.join("lib");
                    if common_libdir.is_dir() && !native_dirs.contains(&common_libdir) {
                        native_dirs.push(common_libdir);
                    }
                }

                for native_dir in native_dirs {
                    let flag = format!("-Lnative={}", native_dir.display());
                    hostflags.push(flag.clone());
                    targetflags.push(flag);
                }
            }
        }
...
impl Step for BookTest {
    ...
    fn run(self, builder: &Builder<'_>) {
        // External docs are different from local because some books need
        // pre-processing by mdbook before being tested.
        //
...
impl BookTest {
    ...
    fn run_ext_doc(self, builder: &Builder<'_>) {
        ...
        let _time = helpers::timeit(builder);
        rustbook_cmd.delay_failure().run(builder);
    }
...
fn prepare_cargo_test(
    ...
    let compiler = cargo.compiler();
    if builder.config.cmd.trust_vanilla() {
        cargo.rustflag("-Ztrust-verify=off");
        cargo.rustdocflag("-Ztrust-verify=off");
    }
...
impl Step for CrateRustdoc {
    ...
    fn should_run(run: ShouldRun<'_>) -> ShouldRun<'_> {
        run.selectors(&["src/librustdoc", "src/tools/trustdoc"])
    }
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        // Trust: this `cargo test` recompiles librustdoc (lib + lib test), and
        // rustdoc is a rustc_private tool. Trust keeps staged sysroots pruned of
        // rustc_private artifacts (upstream #108767 parity), so — exactly like
        // `ToolBuild::run` does for the tool *build* — hand the test build the
        // coherent `rustc-private-tool` sysroot dirs via
        // `RUSTC_ADDITIONAL_SYSROOT_PATHS` (the bootstrap rustc shim turns them
        // into `-L` / `PathKind::All` search paths). Without this, the rebuild
        // resolves `extern crate rustc_*` against whatever partially-pruned
        // leftovers sit in the build compiler's sysroot and dies with an
        // E0463/E0460 cascade ("can't find crate for `rustc_abi`", ...).
        tool::add_rustc_private_metadata_search_path(
            builder,
            &mut cargo,
            tool::output_compiler_for_tool(compiler, target),
            target,
        );
...
impl Step for Distcheck {
    ...
    /// - Check that we can run `cargo metadata` on the workspace in the `trustc-dev` component
    ///
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        distcheck_plain_source_tarball(builder, &root_dir.join("distcheck-trust-src"));
        distcheck_rust_src(builder, &root_dir.join("distcheck-trust-src-component"));
        distcheck_rustc_dev(builder, &root_dir.join("distcheck-trustc-dev"));
    }
}
...
/// Check that trustc-dev's compiler crate source code can be loaded with `cargo metadata`
fn distcheck_rustc_dev(builder: &Builder<'_>, dir: &Path) {
    builder.info("Distcheck trustc-dev");
    ...
    command(&builder.initial_cargo)
        .arg("metadata")
        .arg("--manifest-path")
        .arg("trustc-dev/lib/rustlib/rustc-src/rust/compiler/rustc/Cargo.toml")
        .env("RUSTC_BOOTSTRAP", "1")
