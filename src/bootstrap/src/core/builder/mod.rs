...
use crate::core::build_steps::{
    check, clean, clippy, compile, dist, doc, install, llvm, run, setup, test, tool, vendor,
};
...
pub enum PathSet {
    ...
    /// For example, `src/librustdoc` and `src/tools/trustdoc` should be in the same set,
...
",
    ...
    pub fn rustc(&self, compiler: Compiler) -> PathBuf {
        ...
        } else if compiler.stage >= 2 {
            self.sysroot(compiler).join("bin").join(exe("trustc", compiler.host))
        } else {
    ...
    pub fn targo_trust_cmd(&self, run_compiler: Compiler) -> BootstrapCommand {
        assert!(run_compiler.stage > 0, "targo-trust can not be invoked at stage 0");

        let targo_trust = self.ensure(tool::TCargoTrust::from_target_compiler(self, run_compiler));
        let mut cmd = command(targo_trust.tool_path);
    ...
    pub fn cargo_clippy_cmd(&self, build_compiler: Compiler) -> BootstrapCommand {
        if build_compiler.stage == 0 {
            let cargo_clippy = self
                ...
                .clone()
                .unwrap_or_else(|| self.build.config.download_tippy());
    ...
    pub fn submodule_paths(&self) -> &[String] {
        self.submodule_paths_cache.get_or_init(|| {
            let mut paths = build_helper::util::parse_gitmodules(&self.src);

            // Trust carries former upstream submodules as checked-in source
            // trees, without a root `.gitmodules` file. Bootstrap still needs
            // to recognize those roots when deciding whether a path belongs to
            // an external source tree.
            if paths.is_empty() {
                for path in ["src/tools/targo", "src/llvm-project"] {
                    if self.src.join(path).exists() {
