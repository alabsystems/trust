...
This README is aimed at helping to explain how Trust is bootstrapped,
...
1. The entry point script (`x` for unix like systems, `x.ps1` for windows systems,
   `x.py` cross-platform) is run. This script materializes the configured stage0
   compiler/Cargo artifacts. For the Trust-owned path, `src/stage0` points at the
   repo-local `bootstrap/trust-stage0` dist root; inherited remote download support
   is compatibility machinery, not the default ownership proof. The script then
   compiles the build system itself (this folder) and invokes the actual
   `bootstrap` binary build system.
2. In Trust, the bootstrap binary reads all configuration, performs a number of sanity
...
3. The stage 0 compiler and standard library, materialized earlier, are used to build the
...
that (b) leverage the inherited Rust compiler stack as much as possible.
...
build/
  ...
  # Location where the stage0 compiler artifacts are all cached. This directory
  ...
  x86_64-unknown-linux-gnu/
    ...
    # Location where the stage0 Cargo and inherited Rust compiler are unpacked. This
...
before the Trust compiler is built. To work around this, there are two components
of bootstrap: the main one written in Rust, and `bootstrap.py`. `bootstrap.py`
is what gets run by the entry point script. It materializes the configured stage 0
compiler, std, and Cargo artifacts, which are then used to build the bootstrap
binary. In a review-ready Trust checkout, those artifacts come from the local
Trust stage0 seed described by `src/stage0`, not from an upstream Rust dist host.
...
If you have any questions about inherited bootstrap behavior, the upstream
[`#t-infra/bootstrap` channel][rust-bootstrap-zulip] remains useful historical context. When you
encounter Trust bugs, please file issues on the [Trust issue tracker][trust-issue-tracker].

[rust-bootstrap-zulip]: https://rust-lang.zulipchat.com/#narrow/stream/t-infra.2Fbootstrap
[trust-issue-tracker]: https://github.com/alabsystems/Trust/issues
