...
Build and link the local Trust toolchain; Miri is included in that toolchain:
...
All the following commands assume `cargo +trust ...` / `rustup run trust ...`.
Plain no-flags commands are default-toolchain evidence only after
...
When running Miri on CI for this fork, build and link the Trust toolchain:

```sh
CARGO_NET_OFFLINE=true ./x.py build --set llvm.ninja=false --stage 2 compiler/rustc library/std
rustup toolchain link trust ./build/host/stage2

cargo +trust miri test
```
    ...
    steps:
      ...
      - name: Install Miri
        run: |
          CARGO_NET_OFFLINE=true ./x.py build --set llvm.ninja=false --stage 2 compiler/rustc library/std
          rustup toolchain link trust ./build/host/stage2
          cargo +trust miri setup
      - name: Test with Miri
        run: cargo +trust miri test
```

The explicit `cargo +trust miri setup` helps to keep the output of the actual test step
clean.
...
# one-time setup
cargo +trust miri setup
SYSROOT=$(cargo +trust miri setup --print-sysroot)
# per file
~/.rustup/toolchains/trust/bin/miri --sysroot "$SYSROOT" file.rs
```
