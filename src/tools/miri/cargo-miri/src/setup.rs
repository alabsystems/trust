...
/// Performs the setup required to make `targo miri` work: Getting a custom-built libstd. Then sets
...
pub fn setup(
    ...
    if !only_setup && let Some(sysroot) = std::env::var_os("MIRI_SYSROOT") {
        // Skip setup step if MIRI_SYSROOT is explicitly set, *unless* we are `targo miri setup`.
        return sysroot.into();
    }
    ...
    let rust_src = match rust_src_env_var {
        ...
        None => {
            let trust_src = rustc_build_sysroot::rustc_sysroot_src(miri_for_host())
                .expect("could not determine sysroot source directory");
            if !trust_src.exists() {
                show_error!(
                    "standalone Trust sysroot is missing library sources at `{}`",
                    trust_src.display()
                );
            }
            trust_src
    ...
    let cargo_cmd = {
        ...
        if env::var_os("RUSTC_STAGE").is_some() {
            assert!(
                ...
                "targo-miri setup is running inside Trust bootstrap but RUSTC or RUST_WRAPPER is not set"
            );
