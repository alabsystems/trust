...
pub fn find_miri() -> PathBuf {
    ...
    path.set_file_name(format!("trust-miri{}", env::consts::EXE_SUFFIX));
    path
}
...
pub fn cargo() -> Command {
    let mut command = if let Some(cargo) = sibling_tool("targo") {
        Command::new(cargo)
    } else {
        Command::new(OsString::from("targo"))
    };
...
fn sibling_tool(tool: &str) -> Option<PathBuf> {
    ...
    path.set_file_name(format!("{tool}{}", env::consts::EXE_SUFFIX));
