...
Usage:
    targo miri [subcommand] [<cargo options>...] [--] [<program/test suite options>...]
...
Examples:
    targo miri run
    targo miri test -- test-suite-filter

    targo miri setup --print-sysroot
...
pub(crate) fn show_version() {
    print!("targo-miri {}", env!("CARGO_PKG_VERSION"));
...
pub fn phase_cargo_miri(mut args: impl Iterator<Item = String>) {
    ...
    let Some(subcommand) = args.next() else {
        show_error!("`targo miri` needs to be called with a subcommand (`run`, `test`, `clean`)");
    };
    let subcommand = match &*subcommand {
        ...
        _ => {
            ...
            show_error!(
                "`targo miri` supports the following subcommands: `run`, `test`, `nextest`, `clean`, and `setup`."
            )
        }
    };
    if has_arg_flag("--help") || has_arg_flag("-h") {
        match subcommand {
            MiriCommand::Forward(verb) => {
                println!("`targo miri {verb}` supports the same flags as `targo {verb}`:\n");
                let mut cmd = cargo();
                cmd.arg(verb);
    ...
    if cargo_cmd == "nextest" {
        cmd.arg(
            args.next().unwrap_or_else(|| {
                show_error!("`targo miri nextest` expects a verb (e.g. `run`)")
            }),
        );
    }
        ...
        ArgSplitFlagValue::from_string_iter(&mut args, "--target-dir").filter_map(Result::err)
    {
        if arg == "--many-seeds" || arg.starts_with("--many-seeds=") {
            show_error!(
                "ERROR: the `--many-seeds` flag has been removed from targo-miri; use MIRIFLAGS=-Zmiri-many-seeds instead"
            );
        } else {
            cmd.arg(arg);
        }
...
pub fn phase_runner(mut binary_args: impl Iterator<Item = String>, phase: RunnerPhase) {
    ...
    let file = File::open(&binary)
        .unwrap_or_else(|_| show_error!(
            "file {:?} not found or `targo-miri` invoked incorrectly; please invoke this binary through `targo miri`", binary
        ));
    let file = BufReader::new(file);
    let binary_args = binary_args.collect::<Vec<_>>();

    let info: CrateRunInfo = serde_json::from_reader(file).unwrap_or_else(|_| {
        show_error!("file {:?} contains outdated or invalid JSON; try `targo clean`", binary)
    });
