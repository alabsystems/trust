...
fn main() {
    ...
    let Some(first) = args.next() else {
        show_error!("`targo-miri` needs a subcommand (`run`, `test`, `clean`, or `setup`)")
    };
    ...
    if env::var_os("MIRI_CALLED_FROM_RUSTDOC").is_some() {
        ...
        match first.as_str() {
            ...
            _ => {
                show_error!(
                    "`targo-miri` failed to recognize which phase of the build process this is, please report a bug.\n\
    ...
    match first.as_str() {
        ...
        "setup" | "test" | "t" | "run" | "r" | "nextest" | "clean" => {
            phase_cargo_miri(iter::once(first).chain(args))
        }
        "runner" => phase_runner(args, RunnerPhase::Cargo),
        arg if arg == env::var("RUSTC").unwrap_or_else(|_| {
            show_error!(
                "`targo-miri` could not identify the Trust compiler phase; invoke it as `targo miri ...` or `targo-miri ...`"
            )
        }) => {
