...
use rayon::prelude::*;
use tippy_arg_protocol::{CLIPPY_ARGS_ENV, TIPPY_ENCODED_ARGS_ENV, encode_args};
...
impl Crate {
    ...
    fn run_clippy_lints(
        ...
        let tippy_encoded_args = encode_args(false, &clippy_args);
        ...
        cmd.arg(if config.fix { "fix" } else { "check" })
            ...
            .env(CLIPPY_ARGS_ENV, clippy_args.join("__CLIPPY_HACKERY__"))
