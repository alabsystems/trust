...
/// Returns whether `expr` is the source loop for at least one authored Trust
/// loop clause in its enclosing body.
///
/// Loop predicates live on [`rustc_hir::Body`] rather than the desugared loop
/// expression. Consumers that compare, hash, or replace HIR expressions must
/// consult this side table or they can erase proof-significant differences.
pub fn expr_has_loop_contract(cx: &LateContext<'_>, expr: &Expr<'_>) -> bool {
    if !matches!(expr.kind, ExprKind::Loop(_, _, rustc_hir::LoopSource::While, _)) {
        return false;
    }
    ...
    cx.tcx
        ...
        .contract
        .is_some_and(|contract| contract.loop_clauses.iter().any(|clause| clause.loop_id == expr.hir_id))
}
...
impl HirEqInterExpr<'_, '_, '_> {
    ...
    pub fn eq_expr(&mut self, left: &Expr<'_>, right: &Expr<'_>) -> bool {
        let left_has_loop_contract = expr_has_loop_contract(self.inner.cx, left);
        let right_has_loop_contract = expr_has_loop_contract(self.inner.cx, right);
        if left_has_loop_contract || right_has_loop_contract {
            // Span-only verifier predicates cannot be compared through HIR.
            // The same HIR expression is safe to compare with itself; distinct
            // authored loops are conservatively different even when their
            // executable desugarings happen to match.
            return left_has_loop_contract && right_has_loop_contract && left.hir_id == right.hir_id;
        }
...
impl<'a, 'tcx> SpanlessHash<'a, 'tcx> {
    ...
    pub fn hash_expr(&mut self, e: &Expr<'_>) {
        if expr_has_loop_contract(self.cx, e) {
            // Keep this consistent with `SpanlessEq`: distinct authored loops
            // are unequal even when their executable HIR is identical.
            mem::discriminant(&e.kind).hash(&mut self.s);
            e.hir_id.hash(&mut self.s);
            return;
        }
