...
const TIPPY_ROOT_PACKAGE: &str = "\"tippy\"";
    ...
    pub fn search_for_manifest() -> Self {
        ...
        loop {
            ...
            if let Some(mut file) = File::open_if_exists(&path, OpenOptions::new().read(true)) {
                ...
                if package.name == TIPPY_ROOT_PACKAGE {
                    ...
                    panic!("error reading Tippy version from `{}`", file.path.display());
                }
            }
            ...
            assert!(
                path.pop(),
                "error finding project root, please run from inside the Tippy directory"
            );
        }
    }
}
...
#[test]
fn renamed_tippy_root_manifest_is_discoverable() {
    let package = parse_cargo_package(include_str!("../../Cargo.toml"));
    assert_eq!(package.name, TIPPY_ROOT_PACKAGE);
    assert!(!package.version_range.is_empty());
}
