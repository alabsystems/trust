...
use clippy_utils::{
    as_some_pattern, expr_has_loop_contract, get_enclosing_loop_or_multi_call_closure, higher, is_refutable,
};
...
pub(super) fn check<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
    // Replacing a `while let` header with `for` would leave first-class
    // `invariant`/`decreases` clauses after a grammar form that cannot carry
    // them. Do not offer a source-breaking or proof-erasing rewrite.
    if expr_has_loop_contract(cx, expr) {
        return;
    }
