...
pub(super) fn check(
    ...
    span_lint_and_then(cx, CAST_POSSIBLE_WRAP, expr.span, message, |diag| {
        if let EmitState::LintOnPtrSize(16) = should_lint {
            diag.note("`usize` and `isize` may be as small as 16 bits on some platforms")
                .note("for more information, see the Trust Reference's machine-dependent integer types section");
        }
