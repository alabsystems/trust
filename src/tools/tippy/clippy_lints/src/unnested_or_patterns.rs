...
use clippy_utils::ast_utils::{eq_field_pat, eq_id, eq_maybe_qself, eq_pat, eq_path, pat_contains_authored_contract};
...
fn lint_unnested_or_patterns(cx: &EarlyContext<'_>, pat: &Pat) {
    // The suggestion is rendered from a cloned AST with `pprust`. Trust
    // verifier predicates are span-only and cannot be reproduced there, so a
    // rewrite spanning one would silently erase an authored obligation.
    if pat_contains_authored_contract(pat) {
        return;
    }
