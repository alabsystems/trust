...
use test_utils::{IS_RUSTC_TEST_SUITE, bootstrap_dependency_builder_env, reject_trust_frontend_controls};
    ...
    fn base_config(&self, test_dir: &str, mandatory_annotations: bool) -> Config {
        ...
        let mut config = Config {
            ...
            bless_command: Some(if IS_RUSTC_TEST_SUITE {
                "./x test src/tools/tippy --bless".into()
            } else {
                "cargo uibless".into()
            }),
            ...
            DependencyBuilder {
                program: {
                    ...
                    if let Some(rustc) = option_env!("TEST_RUSTC") {
                        ...
                        let stage = option_env!("TEST_RUSTC_STAGE").unwrap();
                        let build_triple = option_env!("TEST_COMPILER_BUILD_TRIPLE").unwrap();
                        let link_std = option_env!("TEST_RUSTC_LINK_STD_INTO_RUSTC_DRIVER").unwrap();
                        if let Some(rustc_wrapper) = option_env!("RUSTC") {
                            p.envs.push(("RUSTC".into(), Some(rustc_wrapper.into())));
                        }
                        p.envs.push(("RUSTC_REAL".into(), Some(rustc.into())));
                        p.envs.push(("RUSTC_LIBDIR".into(), Some(libdir.into())));
                        ...
                        p.envs
                            .extend(bootstrap_dependency_builder_env(stage, build_triple, link_std));
        ...
        if let Some(paths) = option_env!("RUSTC_ADDITIONAL_SYSROOT_PATHS") {
            config.program.args.extend(
                paths
                    .split(',')
                    .filter(|path| !path.is_empty())
                    .map(|path| format!("-L{path}").into()),
            );
        }

        config.program.program = PathBuf::from(TIPPY_DRIVER_BIN);

        reject_trust_frontend_controls(&config.program.args);
