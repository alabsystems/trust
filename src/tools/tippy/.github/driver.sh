...
# Make sure Tippy's public compiler pass-through switch behaves like rustc.
SYSROOT=$(rustc --print sysroot)
diff -u <(./target/debug/clippy-driver --trustc --version --verbose) <(rustc --version --verbose)

echo "fn main() {}" >target/driver_test.rs
# we can't run 2 rustcs on the same file at the same time
CLIPPY=$(./target/debug/clippy-driver ./target/driver_test.rs --trustc)
