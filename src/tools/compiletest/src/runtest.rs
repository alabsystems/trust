...
pub(crate) fn compute_stamp_hash(config: &Config) -> String {
    ...
    config.trust_vanilla().hash(&mut hash);
...
impl<'test> TestCx<'test> {
    ...
    fn print_source(&self, read_from: ReadFrom, pretty_type: &str) -> ProcRes {
        ...
        let mut rustc = self.config.rustc_command();
    ...
    fn maybe_add_external_args(&self, cmd: &mut Command, args: &Vec<String>) {
        ...
        for arg in args {
            if Config::is_internal_rustcflag(arg) {
                continue;
            }
    ...
    fn document_inner(
        ...
        let mut rustdoc = self.config.rustdoc_command();
        ...
        if !self.props.compile_flags.iter().any(|flag| flag.starts_with("--sysroot"))
            && !self.props.doc_flags.iter().any(|flag| flag.starts_with("--sysroot"))
        {
            rustdoc.arg("--sysroot").arg(&self.config.sysroot_base);
        }
    ...
    fn exec_compiled_test_general(
        ...
        let prepare_env = |cmd: &mut Command| {
            ...
            // Trust: --trust-vanilla propagates no-verify into the RUN environment
            // so NESTED compilations a test binary spawns via the rustc_driver /
            // rustc_public API also behave like vanilla rustc. Those build fixed
            // argv (pristine upstream tests), so only an inherited env can reach
            // them; the driver translates exact `=1` into the tracked
            // `-Ztrust-verify=off` option. Apply it last so per-test exec-env or
            // unset directives cannot silently re-enable verification in a
            // vanilla process subtree.
            if self.config.trust_vanilla() {
    ...
    fn build_minicore(&self) -> Utf8PathBuf {
        ...
        // `make_compile_args` uses the primary test's properties, including a
        // Trust verifier test's explicit `-Ztrust-verify=on`. Minicore is an
        // auxiliary implementation detail and must not inherit that opt-in.
        // Reapply isolation here; dedicated minicore flags remain last so an
        // exceptional fixture can opt in intentionally.
        self.config.configure_compiletest_rustc(&mut rustc);
    ...
    fn make_compile_args(
        ...
        let mut compiler = match compiler_kind {
            CompilerKind::Rustc => self.config.rustc_command(),
            CompilerKind::Rustdoc => self.config.rustdoc_command(),
        };
    ...
    fn normalize_output(&self, output: &str, custom_rules: &[(String, String)]) -> String {
        ...
        // Trust: comparison-space rules ONLY for classes the release porter's
        // adapter rules (tests/upstream-rust/patches.toml) do NOT encode into
        // in-tree expectations — expectations for these stay pristine upstream.
        // Branding/prose (explain hints, ICE notes, doc-URL rewrites) is handled
        // in-tree by the porter; compiletest must NOT rewrite those.
        // (a) std panic locations: our std embeds library-relative paths
        //     (`core/src/...`); upstream's dist shape is `library/core/src/...`.
        normalized = static_regex!(r"panicked at (core|alloc|std)/src/")
            .replace_all(&normalized, "panicked at library/$1/src/")
            .into_owned();
        // (b) Trust omits the tracking-issue URL in feature-gate notes.
        normalized = static_regex!(r"see issue #(\d+) for more information")
            .replace_all(
                &normalized,
                "see issue #$1 <https://github.com/rust-lang/rust/issues/$1> for more information",
            )
            .into_owned();
        // (c) tests compiled under `-Z ui-testing=no` print the raw
        //     invocation-derived `--explain` binary name — `trustc` when the
        //     Trust lane drives the branded binary — while their expectations
        //     stay pristine upstream (`rustc`): the porter exempts them from
        //     the `explain_tool_name` rebrand precisely because the name is
        //     invocation-derived (see `is_ui_testing_optout_expectation` in
        //     crates/trust-upstream-compat/src/porting.rs). Map the branded
        //     spelling back for comparison, for exactly that corpus.
        if self.props.compile_flags.windows(2).any(|w| w[0] == "-Z" && w[1] == "ui-testing=no")
            || self.props.compile_flags.iter().any(|f| f == "-Zui-testing=no")
        {
            normalized = normalized.replace("try `trustc --explain", "try `rustc --explain");
        }
