...
impl<'test> TestCx<'test> {
    ...
    fn run_doctests_for_coverage(
        ...
        // Route through Config so capability-scoped Trust isolation applies to
        // coverage doctests just like every other compiletest-managed rustdoc.
        let mut rustdoc_cmd = self.config.rustdoc_command();
