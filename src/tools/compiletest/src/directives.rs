...
pub(crate) struct TestProps {
    ...
    // Trust verifier tests must explicitly override compiletest's default isolation.
    pub(crate) needs_trust_verify: bool,
...
mod directives {
    ...
    pub(crate) const NEEDS_TRUST_VERIFY: &str = "needs-trust-verify";
}
...
impl TestProps {
    ...
    fn load_from(&mut self, testfile: &Utf8Path, test_revision: Option<&str>, config: &Config) {
        ...
        if let Err(message) =
            validate_trust_verification_opt_in(self.needs_trust_verify, &self.compile_flags)
        {
            panic!("{message}: {testfile}");
        }
...
fn has_explicit_trust_verification_config(flags: &[String]) -> bool {
    // The rule is EXPLICITNESS, not opt-in: a `needs-trust-verify` test must
    // state its verification mode so compiletest's default verifier isolation
    // cannot silently disable it. `=on` opts verification ON; `=off` is the
    // deliberate machinery-present-verification-off state used by
    // island/check-phase gate tests (e.g. clean_island_no_verify_trusted.rs) —
    // also explicit. Any other value is a typo the compiler would reject, so
    // it is not an explicit statement of mode either.
    let is_explicit = |flag: &str| matches!(flag, "trust-verify=on" | "trust-verify=off");
    flags.iter().any(|flag| flag.strip_prefix("-Z").is_some_and(is_explicit))
        || flags
            .windows(2)
            .any(|pair| pair[0] == "-Z" && is_explicit(pair[1].as_str()))
}

fn validate_trust_verification_opt_in(
    needs_trust_verify: bool,
    flags: &[String],
) -> Result<(), &'static str> {
    if needs_trust_verify && !has_explicit_trust_verification_config(flags) {
        Err("`//@ needs-trust-verify` requires an explicit `//@ compile-flags: \
             -Ztrust-verify=on` (verification on) or `-Ztrust-verify=off` \
             (machinery present, verification explicitly off) so compiletest's \
             default verifier isolation cannot silently disable the test")
    } else {
        Ok(())
    }
}
...
pub(crate) fn do_early_directives_check(
    ...
    // Fail during directive parsing as well as per-revision property loading.
    // The early check gives authors a deterministic wiring error before a test
    // can run with compiletest's verifier opt-out and falsely appear covered.
    let needs_trust_verify =
        file_directives.lines.iter().any(|line| line.name == "needs-trust-verify");
    let trust_compile_flags = file_directives
        .lines
        .iter()
        .filter(|line| line.name == directives::COMPILE_FLAGS)
        .filter_map(DirectiveLine::value_after_colon)
        .flat_map(|flags| split_flags(flags.trim()))
        .collect::<Vec<_>>();
    if let Err(message) =
        validate_trust_verification_opt_in(needs_trust_verify, &trust_compile_flags)
    {
        return Err(format!("ERROR: {message}: {testfile}"));
    }
