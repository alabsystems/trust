...
pub(crate) fn prepare_conditions(config: &Config) -> PreparedConditions {
    ...
    // Locally built and downstream compilers can use channel names other than the official
    // stable/beta/nightly strings while still accepting nightly-only options. Match bootstrap's
    // nightly-like behavior here: anything that is not explicitly stable or beta is treated as
    // nightly for channel directives.
