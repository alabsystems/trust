...
use crate::directives::{
    ...
    extract_version_range, has_explicit_trust_verification_config, line_directive, parse_edition,
    parse_normalize_rule, validate_trust_verification_opt_in,
};
...
#[test]
fn needs_trust_verify_requires_an_explicit_opt_in() {
    assert!(!has_explicit_trust_verification_config(&[]));
    assert!(!has_explicit_trust_verification_config(&["-Ztrust-policy=advisory".to_string()]));
    assert!(has_explicit_trust_verification_config(&["-Ztrust-verify=on".to_string()]));
    assert!(has_explicit_trust_verification_config(&[
        "-Z".to_string(),
        "trust-verify=on".to_string(),
    ]));
    assert!(validate_trust_verification_opt_in(false, &[]).is_ok());
    assert!(validate_trust_verification_opt_in(true, &[]).is_err());
    assert!(
        validate_trust_verification_opt_in(true, &["-Ztrust-verify=on".to_string()]).is_ok()
    );
    // Explicit OPT-OUT (machinery present, verification deliberately off — the
    // island/check-phase gate shape) is also explicit configuration.
    assert!(has_explicit_trust_verification_config(&["-Ztrust-verify=off".to_string()]));
    assert!(has_explicit_trust_verification_config(&[
        "-Z".to_string(),
        "trust-verify=off".to_string(),
    ]));
    assert!(validate_trust_verification_opt_in(true, &["-Ztrust-verify=off".to_string()]).is_ok());
}
...
#[test]
fn nightly_like_channels() {
    for channel in ["dev", "trust"] {
        let config: Config = cfg().channel(channel).build();
...
#[test]
fn test_needs_trust_verify_wiring_check() {
    let path = Utf8Path::new("trust-verifier.rs");
    let missing = FileDirectives::from_file_contents(path, "//@ needs-trust-verify\n");
    let error = directives::do_early_directives_check(TestMode::Ui, &missing).unwrap_err();
    assert!(error.contains("requires an explicit `//@ compile-flags: -Ztrust-verify=on`"));

    let wired = FileDirectives::from_file_contents(
        path,
        "//@ needs-trust-verify\n//@ compile-flags: -Ztrust-verify=on\n",
    );
    assert!(directives::do_early_directives_check(TestMode::Ui, &wired).is_ok());
}
