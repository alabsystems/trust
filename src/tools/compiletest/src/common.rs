...
pub(crate) const TRUST_VANILLA_RUSTC_FLAG: &str = "-Ztrust-verify=off";
pub(crate) const TRUST_VANILLA_SENTINEL: &str = "--compiletest-trust-vanilla";
...
/// A codegen backend that a test can be selected by, ignored for, or run
/// against as the compiler's default.
///
/// Trust: `Cranelift` and `Gcc` name backends this toolchain does not build.
/// They stay parseable vocabulary because an unknown name in `ignore-backends`
/// is a parser panic, not a skipped test, and 744 inherited tests already carry
/// `//@ ignore-backends: gcc`; upstream writes `cranelift` the same way, so
/// dropping either name would turn the next merge into a mass edit.
#[derive(Clone, Copy, Debug, PartialEq)]
...
impl<'a> TryFrom<&'a str> for CodegenBackend {
    ...
    fn try_from(value: &'a str) -> Result<Self, Self::Error> {
        match value.to_lowercase().as_str() {
            ...
            "gcc" => Ok(Self::Gcc),
            "llvm" => Ok(Self::Llvm),
            // Trust: `-Zcodegen-backend` accepts both spellings
            // (`rustc_interface::util::builtin_codegen_backend`), so a test
            // author who writes the underscore form must not hit the panic
            // paths in `ignore-backends` / `needs-backends`.
            "trust-cg" | "trust_cg" => Ok(Self::TrustCg),
...
impl CodegenBackend {
    pub(crate) fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Gcc => "gcc",
            Self::Llvm => "llvm",
            // Trust: the canonical spelling bootstrap passes on
            // `--default-codegen-backend` (`CodegenBackendKind::name`).
...
pub(crate) struct Config {
    ...
    /// Whether the compiler under test understands Trust verification options.
    ///
    /// Bootstrap sets this only for locally-built staged compilers. Keeping this
    /// capability explicit prevents compiletest from leaking Trust-only flags to
    /// an upstream or externally supplied stage0 compiler.
    pub(crate) rustc_supports_trust_verification: bool,
    ...
    /// Set to false to suppress output for passing tests when using --no-capture.
...
impl Config {
    ...
    pub(crate) fn trust_vanilla(&self) -> bool {
        self.target_rustcflags.iter().any(|flag| flag == TRUST_VANILLA_SENTINEL)
    }

    pub(crate) fn rustc_command(&self) -> Command {
        let mut command = Command::new(&self.rustc_path);
        self.configure_compiletest_rustc(&mut command);
        command
    }

    pub(crate) fn rustdoc_command(&self) -> Command {
        let rustdoc_path = self.rustdoc_path.as_ref().expect("no rustdoc built yet");
        let mut command = Command::new(rustdoc_path);
        self.configure_compiletest_rustc(&mut command);
        command.env("RUSTC", &self.rustc_path);
        command
    }

    pub(crate) fn query_rustc_command(&self) -> Command {
        let query_rustc_path = self.query_rustc_path.as_deref().unwrap_or(&self.rustc_path);
        let mut command = Command::new(query_rustc_path);
        self.configure_compiletest_rustc(&mut command);
        command
    }

    pub(crate) fn add_rustcflags(&self, command: &mut Command, flags: &[String]) {
        for flag in flags {
            if !Self::is_internal_rustcflag(flag) {
                command.arg(flag);
            }
        }
    }

    pub(crate) fn is_internal_rustcflag(flag: &str) -> bool {
        flag == TRUST_VANILLA_SENTINEL
    }

    pub(crate) fn configure_compiletest_rustc(&self, command: &mut Command) {
        configure_trust_verification_isolation(
            command,
            self.rustc_supports_trust_verification || self.trust_vanilla(),
        );
    }
...
fn configure_trust_verification_isolation(command: &mut Command, supported: bool) {
    if supported {
        // Compiletest's inherited Rust corpus tests compiler behavior, not the
        // batteries-on Trust verifier. Per-test compile flags are appended later,
        // so Trust-specific tests can deliberately opt back in with
        // `-Ztrust-verify=on` while auxiliary crates remain isolated by default.
        command.arg(TRUST_VANILLA_RUSTC_FLAG);
    }
}

#[cfg(test)]
#[path = "common/tests.rs"]
mod trust_verification_isolation_tests;
...
fn rustc_query_command(
    config: &Config,
    args: &[&str],
    envs: HashMap<String, String>,
) -> Command {
    let mut command = config.query_rustc_command();
    add_dylib_path(&mut command, iter::once(&config.host_compile_lib_path));
    config.add_rustcflags(&mut command, &config.target_rustcflags);
    command.args(args);
...
/// Trust: ask the compiler under test whether it can answer a query at all.
///
/// `query_rustc_output` treats a failing exit status as a compiletest bug,
/// which is right for queries every compiler must answer. Optional capabilities
/// that are linked in at build time have no such guarantee: a refusal *is* the
/// answer, and the only honest way to learn it is to ask the exact binary being
/// tested rather than to infer it from the configuration that built it.
pub(crate) fn rustc_query_succeeds(config: &Config, args: &[&str]) -> bool {
    match rustc_query_command(config, args, Default::default()).output() {
        Ok(output) => output.status.success(),
        Err(e) => {
            fatal!("failed to run rustc query {args:?}: {e}");
        }
    }
}
...
pub(crate) fn query_rustc_output(
    ...
    let mut command = rustc_query_command(config, args, envs);
