...
use crate::common::{
    CodegenBackend, CompareMode, Config, Debugger, ForcePassMode, TRUST_VANILLA_SENTINEL, TestMode,
    TestPaths, UI_EXTENSIONS, expected_output_path, output_base_dir, output_relative_path,
};
...
fn parse_config(args: Vec<String>) -> Config {
    ...
    opts.reqopt("", "compile-lib-path", "path to host shared libraries", "PATH")
        ...
        .optflag(
            "",
            "trust-vanilla",
            "pass -Ztrust-verify=off to compiletest-managed rustc invocations",
        )
        .optflag(
            "",
            "rustc-supports-trust-verification",
            "isolate ordinary tests from verification when using a Trust compiler",
        )
        .optflag(
            "",
    ...
    let mut target_rustcflags = matches.opt_strs("target-rustcflags");
    if matches.opt_present("trust-vanilla") {
        target_rustcflags.push(TRUST_VANILLA_SENTINEL.to_string());
    }
    ...
    Config {
        ...
        target_rustcflags,
        rustc_supports_trust_verification: matches.opt_present("rustc-supports-trust-verification"),
