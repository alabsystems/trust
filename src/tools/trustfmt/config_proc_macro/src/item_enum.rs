...
fn impl_display(ident: &syn::Ident, variants: &Variants) -> TokenStream {
    let match_patterns = fold_quote(variants.iter(), |variant| {
        let value = config_value_of_variant(variant);
        let variant_ident = &variant.ident;
        match &variant.fields {
            syn::Fields::Unit => quote! {
                #ident::#variant_ident => f.write_str(#value),
            },
            syn::Fields::Unnamed(_) => quote! {
                #ident::#variant_ident(..) => write!(f, "{}(..)", #value),
            },
...
fn impl_serde(ident: &syn::Ident, variants: &Variants) -> TokenStream {
    let arms = fold_quote(variants.iter().filter(|variant| is_unit(variant)), |v| {
        let v_ident = &v.ident;
        let option_value = config_value_of_variant(v);
        quote! {
            #ident::#v_ident => serializer.serialize_str(&#option_value),
        }
    });
                ...
                S: ::serde::ser::Serializer,
            {
                ...
                match self {
                    ...
                    _ => Err(S::Error::custom(format!(
                        "cannot serialize payload-bearing configuration variant {:?} as a string",
                        self,
                    ))),
                }
            }
