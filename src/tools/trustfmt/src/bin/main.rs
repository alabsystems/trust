...
fn main() {
    rustc_driver::install_ice_hook_with_default_compiler(BUG_REPORT_URL, "trustfmt", |_| ());
    ...
    let mut exit_code = match execute(&opts) {
        Ok(code) => code,
        Err(e) => {
            eprintln!("{e:#}");
            1
...
fn make_opts() -> Options {
    ...
    opts.optopt(
        "",
        "config-path",
        "Recursively searches the given path for the trustfmt.toml config file. If not \
    ...
    opts.optmulti(
        "",
        "config",
        "Set options from command line. These settings take priority over .trustfmt.toml",
        "[key1=val1,key2=val2...]",
    );
    if is_nightly {
        opts.optflag(
...
fn is_nightly() -> bool {
    option_env!("CFG_RELEASE_CHANNEL")
        .map_or(true, |c| c == "nightly" || c == "dev" || c == "trust")
}
...
fn display_binary_name(default: &str) -> String {
    env::args_os()
        ...
        .filter(|name| matches!(name.as_str(), "trustfmt" | "rustfmt"))
        .unwrap_or_else(|| default.to_owned())
}

fn parse_cli_args(opts: &Options, args: impl IntoIterator<Item = OsString>) -> Result<Matches> {
    Ok(opts.parse(args)?)
}

// Returned i32 is an exit code
fn execute(opts: &Options) -> Result<i32> {
    let matches = parse_cli_args(opts, env::args_os().skip(1))?;
...
fn print_usage_to_stdout(opts: &Options, reason: &str) {
    ...
    let binary = display_binary_name("trustfmt");
    let msg = format!("{sep}Format Rust code\n\nusage: {binary} [options] <file>...");
...
```
{binary} src/lib.rs src/foo.rs --file-lines '[
...
fn print_version() {
    ...
    if commit_info.is_empty() {
        println!("{binary} {version_number}");
    } else {
        println!("{binary} {version_number}-{commit_info}");
    }
}
...
mod test {
    ...
    #[test]
    fn style_edition_flag_is_registered_once() {
        let usage = make_opts().usage("");
        assert_eq!(usage.matches("--style-edition").count(), 1, "{usage}");
    }

    #[cfg(unix)]
    #[test]
    fn non_unicode_argument_is_reported_instead_of_panicking() {
        use std::os::unix::ffi::OsStringExt as _;

        let invalid = OsString::from_vec(vec![0xff]);
        assert!(parse_cli_args(&make_opts(), [invalid]).is_err());
    }
