...
use crate::comment::{
    ...
    recover_comment_removed, recover_missing_comment_in_span, rewrite_comment,
...
use crate::contracts::{ContractClause, is_comment_only_gap, rewrite_contract_clauses};
use crate::lists::{
...
struct ControlFlow<'a> {
    ...
    nested_if: bool,
    is_loop: bool,
    loop_contract: Option<&'a ast::LoopContract>,
    span: Span,
}
...
fn to_control_flow(expr: &ast::Expr, expr_type: ExprType) -> Option<ControlFlow<'_>> {
    match expr.kind {
        ...
        ast::ExprKind::While(ref cond, ref block, label, ref contract) => {
            let (pat, cond) = extract_pats_and_cond(cond);
...
fn loop_contract_clauses(contract: &ast::LoopContract) -> Vec<ContractClause> {
    contract
        .clauses
        .iter()
        .map(|clause| ContractClause {
            keyword: match clause.kind {
                ast::LoopClauseKind::Invariant => "invariant",
                ast::LoopClauseKind::Decreases => "decreases",
            },
            keyword_span: Some(clause.keyword_span),
            // Trust: preserve the authored predicate together with its
            // optional `by <thm>` citation as one lossless payload.
            payload: clause
                .clause
                .citation
                .map_or(clause.clause.predicate, |citation| {
                    clause.clause.predicate.to(citation.span)
                }),
        })
        .collect()
}
...
impl<'a> ControlFlow<'a> {
    ...
    fn rewrite_cond(
        ...
        let contract_rewrite = self
            .loop_contract
            .map(|contract| {
                rewrite_contract_clauses(
                    context,
                    loop_contract_clauses(contract),
                    cond_span.hi(),
                    shape.indent,
                )
            })
        ...
        let after_condition = if let Some(contract) = &contract_rewrite {
            let trailing_span = mk_sp(contract.last_payload_hi, self.block.span.lo());
            if !is_comment_only_gap(context.snippet(trailing_span)) {
                return Err(RewriteError::Unknown);
            }

            let mut suffix = contract.text.clone();
            let trailing_comment = recover_missing_comment_in_span(
                trailing_span,
                Shape::indented(shape.indent.block_indent(context.config), context.config),
                context,
                last_line_width(&suffix),
            )?;
            suffix.push_str(&trailing_comment);
            suffix.push_str(alt_block_sep);
            suffix
        } else {
            ...
            after_cond_comment.map_or_else(|| block_sep.to_owned(), |comment| comment)
        };

        let used_width = if contract_rewrite.is_some() {
            // The contract rewriter always places the body brace on a new
            // line, so only the outer indentation is occupied there.
            shape.indent.width()
