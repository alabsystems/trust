...
fn run_rustfmt(files: &HashSet<String>, ranges: &[Range]) -> Result<(), FormatDiffError> {
    ...
    let trustfmt = env::var_os("TRUSTFMT").or_else(|| env::var_os("RUSTFMT"));
    let formatter = trustfmt
        .as_deref()
        .unwrap_or_else(|| OsStr::new("trustfmt"));
    let exit_status = run_formatter(formatter, files, &ranges_as_json).or_else(|err| {
        if trustfmt.is_none() && err.kind() == io::ErrorKind::NotFound {
            run_formatter(OsStr::new("rustfmt"), files, &ranges_as_json)
        } else {
            Err(err)
        }
    })?;
    ...
    if !exit_status.success() {
        return Err(FormatDiffError::IoError(io::Error::new(
            io::ErrorKind::Other,
            format!("trustfmt failed with {exit_status}"),
        )));
    }
    Ok(())
}
