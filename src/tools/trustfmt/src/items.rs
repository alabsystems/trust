...
use crate::contracts::{ContractClause, ContractRewrite, rewrite_contract_clauses};
use crate::expr::{
...
pub(crate) struct FnSig<'a> {
    ...
    contract: Option<&'a ast::FnContract>,
    ext: ast::Extern,
...
impl<'a> FnSig<'a> {
    ...
    pub(crate) fn from_fn_kind(
        ...
        match *fn_kind {
            ...
            ) => FnSig::from_method_sig(sig, generics, contract.as_deref(), vis, defaultness),
            visit::FnKind::Fn(
                _,
                vis,
                ast::Fn {
...
impl<'a> FmtVisitor<'a> {
    ...
    pub(crate) fn rewrite_required_fn(
        ...
        let (mut result, ends_with_comment, _) = rewrite_fn_base(
            ...
            indent,
            ident,
            &FnSig::from_method_sig(sig, generics, contract, vis, defaultness),
            span,
            FnBraceStyle::None,
        )?;
...
fn native_contract_clauses(
    contract: Option<&ast::FnContract>,
) -> Result<Vec<ContractClause>, RewriteError> {
    let Some(contract) = contract else {
        return Ok(Vec::new());
    };

    // The parser-authored marker stream is the sole authority for authored
    // order. Source spans are not an identity: macro-produced clauses can
    // legitimately have equal spans, and sorting the six physical AST lanes
    // would then silently reorder proof obligations. `rustc_ast` owns both
    // the marker schema and physical storage, so tools receive validated,
    // lane-bound references rather than supplying positional lane slices.
    let ordered = contract
        .ordered_clause_refs()
        .map_err(|_| RewriteError::Unknown)?;
    let mut clauses = Vec::with_capacity(
        contract.trust_native_requires.len()
            + contract.trust_native_ensures.len()
            + contract.trust_native_decreases.len(),
    );
    for clause in ordered {
        let ast::FnContractClauseRef::Native(native) = clause.clause() else {
            continue;
        };
        let keyword = match clause.marker().kind {
            ast::FnContractClauseKind::Requires => "requires",
            ast::FnContractClauseKind::Ensures => "ensures",
            ast::FnContractClauseKind::Decreases => "decreases",
        };
        clauses.push(ContractClause {
            keyword,
            keyword_span: None,
            // Trust: format the full clause text — predicate plus any
            // `by <thm>` citation (E9).
            payload: native.citation.map_or(native.predicate, |citation| {
                native.predicate.to(citation.span)
            }),
        });
    }
    Ok(clauses)
...
fn rewrite_native_contract(
    context: &RewriteContext<'_>,
    contract: Option<&ast::FnContract>,
    signature_end: BytePos,
    indent: Indent,
) -> Result<Option<ContractRewrite>, RewriteError> {
    rewrite_contract_clauses(
        context,
        native_contract_clauses(contract)?,
        signature_end,
        indent,
    )
}
...
#[cfg(test)]
mod trust_contract_order_tests {
    use super::*;
    ...
    fn native_clause(lo: u32, hi: u32) -> ast::TrustNativeClause {
        ast::TrustNativeClause {
            predicate: span(lo, hi),
            // These fixtures exercise clause-ORDER recovery only; the payload
            // spelling is never read. `sym::dummy` is the designated
            // pre-interned placeholder for exactly this ("use this instead of
            // `sym::empty` for symbols that won't be used"), and being
            // pre-interned it needs no session globals.
    ...
    #[test]
    fn native_contract_order_comes_from_validated_markers() {
        let mut contract = ast::FnContract::default();
        contract.trust_native_requires.push(native_clause(20, 21));
        // Equal spans deliberately make source-position sorting unable to
        // recover the authored identity.
        contract.trust_native_ensures.push(native_clause(20, 21));
        contract.trust_native_decreases.push(native_clause(20, 21));
        contract.clause_order.push(marker(
            0,
        ...
        let clauses = native_contract_clauses(Some(&contract)).unwrap();
        assert_eq!(clauses.len(), 3);
        assert_eq!(clauses[0].keyword, "ensures");
        assert_eq!(clauses[1].keyword, "requires");
        assert_eq!(clauses[2].keyword, "decreases");
    }

    #[test]
    fn native_contract_order_rejects_unmarked_lane_values() {
        let mut contract = ast::FnContract::default();
        contract.trust_native_requires.push(native_clause(20, 21));

        assert!(native_contract_clauses(Some(&contract)).is_err());
    }

    #[test]
    fn native_contract_order_rejects_duplicate_or_non_dense_ordinals() {
        let mut contract = ast::FnContract::default();
        contract.trust_native_requires.push(native_clause(20, 21));
        contract.trust_native_ensures.push(native_clause(22, 23));
        contract.clause_order.push(marker(
            0,
            ast::FnContractClauseKind::Requires,
            ast::FnContractClauseLane::Native,
        ...
        contract.clause_order[1].ordinal = 2;
        assert!(native_contract_clauses(Some(&contract)).is_err());
    }

    #[test]
    fn native_contract_order_rejects_skipped_or_out_of_range_lane_indices() {
        let mut contract = ast::FnContract::default();
        contract.trust_native_requires.push(native_clause(20, 21));
        contract.clause_order.push(marker(
            0,
        ...
        contract.clause_order[0].lane_index = u32::MAX;
        assert!(native_contract_clauses(Some(&contract)).is_err());
    }

    #[test]
    fn native_contract_order_requires_markers_for_non_native_lanes_too() {
        let mut contract = ast::FnContract::default();
        contract.trust_native_requires.push(native_clause(20, 21));
        contract.trust_opaque_ensures.push(span(22, 23));
        contract.clause_order.push(marker(
            0,
            ast::FnContractClauseKind::Requires,
            ast::FnContractClauseLane::Native,
...
fn rewrite_fn_base(
    ...
    let signature_end = match fd.output {
        ast::FnRetTy::Default(..) => params_span.hi(),
        ast::FnRetTy::Ty(ref ty) => ty.span.hi(),
    };
    let native_contract = rewrite_native_contract(context, fn_sig.contract, signature_end, indent)?;
    ...
    if let ast::FnRetTy::Ty(..) = fd.output {
        let ret_should_indent = match context.config.indent_style() {
            ...
            _ => {
                ...
                // and the brace.
                if where_clause.predicates.is_empty() && native_contract.is_none() {
                    sig_length += 2;
                }
    ...
    if let Some(contract) = &native_contract {
        result.push_str(&contract.text);
        // A body brace on the final predicate line is legal, but obscures the
        // signature/body boundary and makes multiline predicates unstable.
        force_new_line_for_brace = true;

        if where_clause.predicates.is_empty() {
            let missing_comment = recover_missing_comment_in_span(
                mk_sp(contract.last_payload_hi, span.hi()),
    ...
    let pos_before_where = native_contract
        .as_ref()
        .map_or(signature_end, |contract| contract.last_payload_hi);
    ...
    // params and `{`.
    if where_clause_str.is_empty() && native_contract.is_none() {
