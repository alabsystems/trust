...
impl Config {
    ...
    /// searches parents for a `trustfmt.toml` see `from_resolved_toml_path`.
    ///
    ...
    ///
    /// Searches for `trustfmt.toml` beginning with `dir`, and
...
// Check for the presence of known config file names in `dir`
//
