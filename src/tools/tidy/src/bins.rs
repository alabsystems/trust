...
mod os_impl {
    ...
    pub fn check(path: &Path, tidy_ctx: TidyCtx) {
        ...
        const ALLOWED: &[&str] = &[
            // Top-level entry points intentionally keep extensionless names.
            "configure",
            "x",
            // Source copy for the local Git hook installed as `.git/hooks/commit-msg`.
            ".hooks/commit-msg",
            // Extensionless tool scripts that are intentionally executable.
            "src/tools/miri/miri",
        ];
