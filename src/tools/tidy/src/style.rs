...
fn trust_test_fixture_path(path: &Path) -> bool {
    path.components().any(|component| component.as_os_str() == "tests")
}

/// Trust: the committed per-root finding counts the ratcheted roots may carry.
///
/// `crates/` and `targo-trust/` are ~1.5M lines that no mechanical style gate
/// ever covered, so their findings predate any decision to enforce style there.
/// Flipping them to hard failures would red the gate on day one against work
/// nobody is doing now, and a gate that cannot be got green is a gate that gets
/// switched off. They therefore enter through a ratchet: the rules are the same
/// rules, the findings are still printed, and the check fails only when a
/// root's count RISES. Lowering it is the only way it moves; `--bless` writes
/// the lower number back.
const TRUST_STYLE_RATCHET: &str = "src/tools/tidy/trust-style-ratchet.txt";

/// How many ratcheted findings get printed before the rest are summarized.
/// Enough to act on, few enough to leave the hard failures readable.
const TRUST_RATCHET_SAMPLE: usize = 25;

/// Where a style finding goes. Ratcheted roots count instead of failing, so a
/// single implementation of the rules serves both enforcement modes.
enum StyleVerdict<'a> {
    Fail(&'a mut RunningCheck),
    Ratchet { check: &'a mut RunningCheck, root: String, found: usize },
}

impl StyleVerdict<'_> {
    fn error(&mut self, msg: String) {
        match self {
            StyleVerdict::Fail(check) => check.error(msg),
            StyleVerdict::Ratchet { check, found, .. } => {
                *found += 1;
                if *found <= TRUST_RATCHET_SAMPLE {
                    check.warning(msg);
                }
            }
        }
...
/// One `<root> <count>` line per ratcheted root, `#` comments ignored.
fn read_trust_style_ratchet(root_path: &Path) -> FxHashMap<String, usize> {
    let path = root_path.join(TRUST_STYLE_RATCHET);
    ...
    contents
        ...
        .filter_map(|line| {
            ...
            let (name, count) = line.split_once(char::is_whitespace)?;
            Some((name.to_string(), count.trim().parse().ok()?))
        })
        .collect()
}

fn write_trust_style_ratchet(root_path: &Path, counts: &FxHashMap<String, usize>) {
    let mut rows: Vec<_> = counts.iter().collect();
    rows.sort();
    let mut out = String::from(
        "# Trust style ratchet: the highest style-finding count each root is\n\
         # allowed to carry. See `TRUST_STYLE_RATCHET` in src/tools/tidy/src/style.rs.\n\
...
pub fn check(path: &Path, tidy_ctx: TidyCtx) {
    run(path, None, tidy_ctx);
}

/// Style over a root that is not yet clean: same rules, ratcheted verdict.
pub fn check_ratcheted(root_path: &Path, path: &Path, tidy_ctx: TidyCtx) {
    run(path, Some(root_path), tidy_ctx);
}

fn run(path: &Path, ratchet_root: Option<&Path>, tidy_ctx: TidyCtx) {
    let name = if ratchet_root.is_some() { "style_ratcheted" } else { "style" };
    let mut check = tidy_ctx.start_check(CheckId::new(name).path(path));
    ...
    fn skip(path: &Path, is_dir: bool) -> bool {
        ...
        if filter_dirs(path) || trust_test_fixture_path(path) {
            return true;
        }
    ...
    let ratchet_name = ratchet_root.map(|root| {
        path.strip_prefix(root).unwrap_or(path).to_string_lossy().replace('\\', "/")
    });
    let counted = {
        let mut verdict = match &ratchet_name {
            None => StyleVerdict::Fail(&mut check),
            Some(root) => {
                StyleVerdict::Ratchet { check: &mut check, root: root.clone(), found: 0 }
            }
        };
        walk_style_rules(path, skip, &mut verdict, &problematic_regex, this_file, codegen_file);
        match verdict {
            StyleVerdict::Fail(_) => None,
            StyleVerdict::Ratchet { root, found, .. } => Some((root, found)),
        }
    };

    let (Some((root, found)), Some(repo_root)) = (counted, ratchet_root) else { return };
    let baseline = read_trust_style_ratchet(repo_root);
    let allowed = baseline.get(&root).copied();
    match allowed {
        Some(allowed) if found > allowed => check.error(format!(
            "{root}: {found} style findings, ratchet allows {allowed}. \
             The ratchet only goes down: fix the new findings, do not raise it."
        )),
        Some(allowed) if found < allowed => {
            if tidy_ctx.is_bless_enabled() {
                let mut lowered = baseline.clone();
                lowered.insert(root.clone(), found);
                write_trust_style_ratchet(repo_root, &lowered);
                check.warning(format!("{root}: ratchet lowered {allowed} -> {found}"));
            } else {
                check.warning(format!(
                    "{root}: {found} style findings, ratchet still allows {allowed}. \
                     Lower it with `x.py test tidy --bless`."
                ));
            }
        }
        Some(_) => {}
        None => check.error(format!(
            "{root} has no row in {TRUST_STYLE_RATCHET}; a ratcheted root without \
             a recorded count enforces nothing. Record {found} with `--bless`."
        )),
    }
}

/// The rule body, identical for both verdict modes. It is a free function only
/// so the ratchet arm can borrow the verdict for exactly the walk and get
/// `check` back afterwards to report the count.
fn walk_style_rules(
    path: &Path,
    skip: impl Send + Sync + 'static + Fn(&Path, bool) -> bool,
    verdict: &mut StyleVerdict<'_>,
    problematic_regex: &regex::RegexSet,
    this_file: &Path,
    codegen_file: &Path,
) {
    walk(path, skip, &mut |entry, contents| {
        ...
        let under_rustfmt = filename.ends_with(".rs") &&
            ...
            // parser to tidy.
            !file.ancestors().any(|a| a.ends_with("tests") && a.join("COMPILER_TESTS.md").exists());

        if contents.is_empty() {
            verdict.error(format!("{}: empty file", file.display()));
        }
        ...
        for (i, line) in contents.split('\n').enumerate() {
            ...
            let mut err = |msg: &str| {
                verdict.error(format!("{}:{}: {msg}", file.display(), i + 1));
            };
            ...
            if is_compiler() {
                ...
                {
                    let mut err = |msg: &str| {
                        verdict.error(format!("{}:{start_line}: {msg}", file.display()));
                    };
        ...
        if leading_new_lines {
            let mut err = |_| {
                verdict.error(format!("{}: leading newline", file.display()));
            };
            suppressible_tidy_err!(err, skip_leading_newlines, "missing leading newline");
        }
        let mut err = |msg: &str| {
            verdict.error(format!("{}: {}", file.display(), msg));
        };
        ...
        if let Directive::Ignore(false) = skip_cr {
            verdict.error(format!("{}: ignoring CR characters unnecessarily", file.display()));
        }
        if let Directive::Ignore(false) = skip_tab {
            verdict.error(format!("{}: ignoring tab characters unnecessarily", file.display()));
        }
        if let Directive::Ignore(false) = skip_end_whitespace {
            verdict.error(format!("{}: ignoring trailing whitespace unnecessarily", file.display()));
        }
        if let Directive::Ignore(false) = skip_trailing_newlines {
            verdict.error(format!("{}: ignoring trailing newlines unnecessarily", file.display()));
        }
        if let Directive::Ignore(false) = skip_leading_newlines {
            verdict.error(format!("{}: ignoring leading newlines unnecessarily", file.display()));
        }
        if let Directive::Ignore(false) = skip_copyright {
            verdict.error(format!("{}: ignoring copyright unnecessarily", file.display()));
        }
