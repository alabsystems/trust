...
pub fn check(
    ...
    for ((feature_name, feature), kind) in all_features_iter {
        ...
        if channel != "nightly" && channel != "trust" && since == Version::CurrentPlaceholder {
            check.error(format!(
...
fn get_version_and_channel(src_path: &Path) -> (Version, String) {
    // Trust: the versions this compares against are `#[stable(since = "...")]`
    // attributes in `library/`, which record the RUST release an item
    // stabilized in. That is Rust's version line, so it must be measured
    // against `src/rust-compat-version` — Trust's own `major.minor.dev` version
    // in `src/version` is a different numbering entirely, and comparing the two
    // reports every stdlib stabilization as being from the future.
    let version_str = t!(std::fs::read_to_string(src_path.join("rust-compat-version")));
