...
fn main() {
    ...
    let targo_trust_path = root_path.join("targo-trust");
    ...
    scope(|s| {
        ...
        check!(seed_freshness, &root_path); // Trust: re-seed cadence invariant
        ...
        // Trust: the Trust-owned trees were outside every mechanical style gate
        // until they entered here, so they carry pre-existing findings that were
        // never anyone's regression. They are checked under a count ratchet
        // (`trust-style-ratchet.txt`) so new findings fail while the backlog
        // drains, rather than reporting thousands of failures nobody can act on.
        check!(style_ratcheted, &root_path, &crates_path);
        check!(style_ratcheted, &root_path, &targo_trust_path);

        // Trust: `[workspace.lints]` is declared but inert unless members opt
        // in. This is the same ratchet shape: it counts the members that have
        // not yet opted in and fails when that number rises.
        check!(workspace_lints, &root_path);
