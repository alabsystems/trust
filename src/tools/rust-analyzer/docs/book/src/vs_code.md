...
`org.freedesktop.Sdk.Extension.trust_cg1`.

If you use a Flatpak SDK for Rust, it must be in your `PATH`:

 * install the SDK extensions with `flatpak install org.freedesktop.Sdk.Extension.{trust_cg1,rust-stable}//25.08`
 * enable SDK extensions in the editor with the environment variable `FLATPAK_ENABLE_SDK_EXT=trust_cg1,rust-stable` (this can be done using flatseal or `flatpak override`)
