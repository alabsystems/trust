...
fn actual_main() -> anyhow::Result<ExitCode> {
    let flags = flags::RustAnalyzer::from_env().unwrap_or_else(|err| {
        if err.is_help() && display_binary_name("trust-analyzer") == "trust-analyzer" {
            println!("{}", err.to_string().replace("rust-analyzer", "trust-analyzer"));
            std::process::exit(0);
        }
        err.exit();
    });
    ...
    match flags.subcommand {
        flags::RustAnalyzerCmd::LspServer(cmd) => 'lsp_server: {
            ...
            if cmd.version {
                println!(
                    "{} {}",
                    display_binary_name("trust-analyzer"),
...
// Blessed `env_mutation` sites (trust-only deny-by-default lint): both
// mutations below happen during single-threaded startup.
...
fn run_server() -> anyhow::Result<()> {
    ...
    let initialize_result = lsp_types::InitializeResult {
        ...
        server_info: Some(lsp_types::ServerInfo {
            name: display_binary_name("trust-analyzer"),
...
fn display_binary_name(default: &str) -> String {
    env::args()
        ...
        .filter(|name| matches!(name.as_str(), "trust-analyzer" | "rust-analyzer"))
