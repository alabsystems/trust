...
impl Tool {
    ...
    pub fn prefer_proxy(self) -> Utf8PathBuf {
        if let Tool::Rustfmt = self {
            return invoke(
                &[
                    cargo_proxy,
                    lookup_rustfmt_env_var,
                    lookup_in_path,
                    cargo_proxy_compat,
                    lookup_in_path_compat,
                ],
                "trustfmt",
            );
        }
    ...
    pub fn path_in(self, path: &Utf8Path) -> Option<Utf8PathBuf> {
        if let Tool::Rustfmt = self {
            return probe_for_binary(path.join("trustfmt"))
                .or_else(|| probe_for_binary(path.join("rustfmt")));
        }
...
fn compat_name(executable_name: &str) -> &str {
...
fn lookup_in_path_compat(executable_name: &str) -> Option<Utf8PathBuf> {
    lookup_in_path(compat_name(executable_name))
}

fn cargo_proxy_compat(executable_name: &str) -> Option<Utf8PathBuf> {
    cargo_proxy(compat_name(executable_name))
}
...
fn lookup_rustfmt_env_var(_: &str) -> Option<Utf8PathBuf> {
    lookup_as_env_var("trustfmt").or_else(|| lookup_as_env_var("rustfmt"))
}
