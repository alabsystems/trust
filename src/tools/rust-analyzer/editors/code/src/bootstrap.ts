...
export async function bootstrap(
    ...
    if (!path) {
        throw new Error(
            "trust-analyzer Language Server is not available. " +
                "Please ensure the Trust toolchain provides a `trust-analyzer` binary.",
        );
    }
...
async function getServer(
    ...
    if (vscode.workspace.workspaceFolders) {
        for (const workspaceFolder of vscode.workspace.workspaceFolders) {
            ...
            // and if the toolchain of this override has a trust-analyzer component
            // if so, use the trust-analyzer component
            ...
            for (const toolchainFile of RUST_TOOLCHAIN_FILES) {
                ...
                let res = await spawnAsync("rustup", ["which", "trust-analyzer"], {
                    env: { ...process.env },
                    cwd: workspaceFolder.uri.fsPath,
                });
                if (res.error || res.status !== 0) {
                    res = await spawnAsync("rustup", ["which", "rust-analyzer"], {
                        env: { ...process.env },
                        cwd: workspaceFolder.uri.fsPath,
                    });
                }
    ...
    if (packageJson.releaseTag === null) return "trust-analyzer";

    // finally, use the bundled one
    const ext = process.platform === "win32" ? ".exe" : "";
    let bundled = vscode.Uri.joinPath(context.extensionUri, "server", `trust-analyzer${ext}`);
    if (!(await fileExists(bundled))) {
        bundled = vscode.Uri.joinPath(context.extensionUri, "server", `rust-analyzer${ext}`);
    }
...
// Given a path to a trust-analyzer executable, resolve its version and return it.
...
// Given a path to two trust-analyzer executables, return the earliest one by date.
...
//  Lowest  - stable
// Example paths:
//  nightly   - ~/.rustup/toolchains/nightly-2022-11-22-aarch64-apple-darwin/bin/trust-analyzer
//  versioned - ~/.rustup/toolchains/1.72.1-aarch64-apple-darwin/bin/trust-analyzer
//  stable    - ~/.rustup/toolchains/stable-aarch64-apple-darwin/bin/trust-analyzer
async function orderFromPath(
    path: string,
    raVersionResolver: (path: string) => Promise<string | undefined>,
): Promise<string> {
    const raVersion = await raVersionResolver(path);
    const raDate = raVersion?.match(
        /^(?:trust-analyzer|rust-analyzer) .*\(.* (\d{4}-\d{2}-\d{2})\)$/,
    );
    if (raDate?.length === 2) {
...
async function hasToolchainFileWithRaDeclared(uri: vscode.Uri): Promise<boolean> {
    try {
        ...
        return (
            toolchainFileContents.match(
                /components\s*=\s*\[.*"(?:trust-analyzer|rust-analyzer)".*\]/g,
            )?.length === 1
        );
    } catch (_) {
        return false;
...
async function getNixOsServer(
    ...
    const dest = vscode.Uri.joinPath(globalStorageUri, `trust-analyzer${ext}`);
    ...
    await vscode.window.withProgress(
        {
            ...
            title: "Patching trust-analyzer for NixOS",
        },
        async (progress, _) => {
            const expression = `
