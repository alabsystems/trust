...
/// This allows the manifest to contain trust-docs and trustc-docs for hosts
...
fn is_nightly_only(pkg: &PkgType) -> bool {
    match pkg {
        PkgType::Miri | PkgType::JsonDocs | PkgType::Enzyme => true,
        PkgType::Rust
        | PkgType::RustSrc
        | PkgType::Rustc
        | PkgType::RustcDev
        ...
        | PkgType::RustStd
        | PkgType::Cargo
        // Trust: `targo-trust` is a shipped stable-channel tool (the verifier
        // CLI), like `Cargo`/`Clippy`/`Rustfmt` — not nightly-only. It was
        // added to the `PkgType` enum without this arm, leaving the match
        // non-exhaustive (E0004) and breaking `build-manifest`, which blocks
        // channel-manifest generation and therefore stage0 seed minting.
        | PkgType::TCargoTrust
        | PkgType::HtmlDocs
...
impl Builder {
    fn build(&mut self) {
        ...
        if channel == "stable" {
            // Trust: channel manifests are named after the product being
            // released, so this is Trust's own version — channel-trust-0.1.0.toml
            // and channel-trust-0.1.toml, not a Rust release number.
            let version = self.versions.trust_version().to_string();
            self.write_channel_files(&version, &manifest);
    ...
    fn build_manifest(&mut self) -> Manifest {
        ...
        manifest.pkg.insert("trust".to_string(), self.rust_package(&manifest));
    ...
    fn add_profiles_to(&mut self, manifest: &mut Manifest) {
        ...
        default.extend([HtmlDocs, Rustfmt, Clippy, TCargoTrust, RustAnalyzer, RustSrc, LlvmTools]);
        profile("default", &default);

        // NOTE: this profile is effectively deprecated; do not add new components to it.
        let mut complete = default;
        complete.extend([RustAnalysis, Miri]);
        profile("complete", &complete);

        // The compiler libraries are not stable for end users, and they're also huge, so we only
        // `trustc-dev` for nightly users, and only in the "complete" profile. It's still possible
        // for users to install the additional component manually, if needed.
        if self.versions.channel() == "nightly" {
            self.extend_profile("complete", &mut manifest.profiles, &[RustcDev]);
            // Do not include the trustc-docs component for now, as it causes
            // conflicts with the trust-docs component when installed. See
            // #75833.
            // self.extend_profile("complete", &mut manifest.profiles, &["trustc-docs"]);
        }
    }
    ...
    fn add_renames_to(&self, manifest: &mut Manifest) {
        ...
        for (from, to) in [
            ...
            ("clippy", "tippy-preview"), // Trust: produced linter component is tippy
            ("clippy-preview", "tippy-preview"),
    ...
    fn target_host_combination(&mut self, host: &str, manifest: &Manifest) -> Option<Target> {
        ...
        for pkg in &PkgType::all() {
            match pkg {
                // trustc/trust-std/targo/docs are all required
                ...
                // so is trust-mingw if it's available for the target
                PkgType::RustMingw => {
        ...
        let mut target_from_compressed_tar = |target_name| {
            ...
            for (substr, fallback_target) in fallback {
                if target_name.contains(substr) {
                    ...
                    // default. It is also possible that `trust-docs` and `trustc-docs` differ in
    ...
    fn write_channel_files(&mut self, channel_name: &str, manifest: &Manifest) {
        ...
        if let Some(git_commit_hash) = manifest.pkg["trust"].git_commit_hash.as_ref() {
            self.write(git_commit_hash, channel_name, "-git-commit-hash.txt");
        } else {
            eprintln!(
                "warning: trust package has no git commit hash; \
                 skipping channel-rust-{channel_name}-git-commit-hash.txt"
            );
        }
    }
