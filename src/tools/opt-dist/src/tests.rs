...
pub fn run_tests(env: &Environment) -> anyhow::Result<()> {
    ...
    // Extract trustc, trust-std, targo and trust-src archives to create the optimized sysroot
    let rustc_dir = extract_dist_dir(&format!("trustc-{version}-{host_triple}"))?.join("trustc");
    let libstd_dir = extract_dist_dir(&format!("trust-std-{version}-{host_triple}"))?
        .join(format!("trust-std-{host_triple}"));
    let cargo_dir = extract_dist_dir(&format!("targo-{version}-{host_triple}"))?.join("targo"); // Trust: produced frontend dist artifact is targo
    let extracted_src_dir = extract_dist_dir(&format!("trust-src-{version}"))?.join("trust-src");
