...
enum EnvironmentCmd {
    ...
    Local {
        ...
        /// Checkout directory of `rustc-perf`, the PGO training corpus.
        ///
        /// Trust does not carry that corpus in-tree, so this is required for any run
        /// that reaches the training stages.
        #[arg(long)]
...
fn execute_pipeline(
    ...
    with_log_group("Building rustc-perf", || {
        // Trust: the training corpus is not carried in-tree, so there is no default
        // location to fall back to — an unset checkout has to fail here rather than
        // deep inside a directory copy.
        let rustc_perf_checkout_dir = env.prebuilt_rustc_perf().context(
            "no rustc-perf checkout: PGO training needs `--rustc-perf-checkout-dir <path>` \
             pointing at a rustc-perf clone",
        )?;
...
fn main() -> anyhow::Result<()> {
    ...
    if is_fast_try_build() {
        ...
        for target in [
            ...
            "trust-source",
            "extended",
            "tippy", // Trust: dist clippy step alias is tippy
            "miri",
            "trustfmt",
            "generate-copyright",
            "bootstrap",
