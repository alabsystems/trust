...
impl<'e> TextRenderer<'e> {
    fn render(input: &str, url: Option<Url>, indent: usize) -> Result<String, Error> {
        crate::validate_link_destinations(input, url.as_ref())?;
    ...
    fn push_md(&mut self) -> Result<(), Error> {
        ...
        // Trust: restores the exact indentation after a footnote definition.
        // The parser does not attach the label to the closing event, so retain
        // the nesting state here instead of guessing at the close. Footnotes
        // are one of the constructs upstream leaves as `unimplemented!()`.
        let mut footnote_indents = Vec::new();
            ...
            match event {
                Event::Start(tag) => {
                    match tag {
                        ...
                        Tag::FootnoteDefinition(label) => {
                            ...
                            self.push_indent(self.indent);
                            self.push_to_line(&marker);
                            footnote_indents.push(self.indent);
                            self.indent += marker.chars().count() + 1;
                            suppress_paragraph = true;
                        }
                        Tag::Table(alignment) => {
                ...
                Event::End(tag_end) => match &tag_end {
                    ...
                    TagEnd::FootnoteDefinition => {
                        self.flush();
                        self.indent = footnote_indents.pop().ok_or_else(|| {
                            anyhow::anyhow!("footnote definition ended without a start")
                        })?;
                        self.hard_break();
                    }
                    TagEnd::Table => {}
                ...
                Event::InlineHtml(t) => self.word.push_str(&t),
                Event::FootnoteReference(label) => {
                    write!(self.word, "[{label}]")?;
                }
                ...
                Event::TaskListMarker(checked) => {
                    self.push_to_line(if checked { "[x]" } else { "[ ]" });
                }
                Event::InlineMath(text) => {
                    write!(self.word, "${text}$")?;
                }
                Event::DisplayMath(text) => {
                    self.flush();
                    self.push_indent(self.indent);
                    self.push_to_line(&format!("$${text}$$"));
                    self.hard_break();
                }
            }
        }
        if !footnote_indents.is_empty() {
            bail!("footnote definition was not closed");
        }
        Ok(())
    }
...
// Trust: pins that the extended-markdown constructs upstream left
// unimplemented now render, rather than aborting the documentation build.
#[cfg(test)]
mod tests {
    use super::TextRenderer;

    #[test]
    fn extended_markdown_is_rendered_without_loss_or_panics() {
        let input = "# extended(1)\n\nA[^note] with <kbd>raw</kbd> and $x + 1$.\n\n- [x] done\n- [ ] pending\n\n$$x^2$$\n\n[^note]: Footnote **body**.\n";
        let output = TextRenderer::render(input, None, 0).unwrap();
