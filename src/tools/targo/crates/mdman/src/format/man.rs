            ...
            match event {
                Event::Start(tag) => {
                    match tag {
                        ...
                        // Trust: upstream leaves the extended-markdown events
                        // below as `unimplemented!()`, so an author writing a
                        // footnote, task list, or math span aborts the doc
                        // build. Render them instead — a documentation tool has
                        // no reason to panic on valid input.
                        Tag::FootnoteDefinition(label) => {
                            self.flush();
                            self.output.push_str(".sp\n.RS 4\n");
                            write!(self.output, "[{}] ", escape(&label)?)?;
                            footnote_depth += 1;
                            suppress_paragraph = true;
                        }
                        Tag::Table(alignment) => {
                ...
                Event::End(tag_end) => {
                    match &tag_end {
                        ...
                        TagEnd::FootnoteDefinition => {
                            if footnote_depth == 0 {
                                bail!("footnote definition ended without a start");
                            }
                            footnote_depth -= 1;
                            self.flush();
                            self.output.push_str(".RE\n");
