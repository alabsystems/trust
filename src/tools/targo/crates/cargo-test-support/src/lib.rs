...
pub trait TestEnvCommandExt: Sized {
    fn test_env(mut self) -> Self {
        let mut inherited_test_path = env::var_os("PATH").unwrap_or_default();
        ...
        if env::var_os("RUSTUP_TOOLCHAIN").is_some() {
            ...
            inherited_test_path = new_path.clone();
        ...
        // Trust's staged compiler verifies by default, but Cargo's inherited
        // fixture programs exercise Cargo semantics rather than serving as a
        // verified-Rust corpus. Bootstrap supplies an explicit TEST_* bundle
        // only for `x test src/tools/targo`. Install it on fixture commands at
        // the final compiler boundary so Cargo still observes the authored
        // rustflags, wrappers, fingerprints, and environment unchanged.
        // Deliberate per-test RUSTC or PATH overrides remain later-wins.
        self = configure_trust_cargo_fixture(
            self,
            env::var_os("TRUST_TARGO_TEST_NO_VERIFY").is_some(),
            env::var_os("TEST_TRUST_SHIM_DIR"),
            Some(inherited_test_path),
            env::vars().map(|(key, _)| key),
        )
        .unwrap_or_else(|err| panic!("invalid Trust Cargo-test shim context: {err}"));
        if cfg!(windows) {
...
/// Trust: bootstrap's compiler shim is driven by these variables, and a
/// fixture that inherits them compiles under a different policy than the test
/// intends. `RUSTC_BOOTSTRAP` is deliberately exempt — upstream fixtures set
/// and clear it on purpose to exercise the unstable-channel gate.
fn is_bootstrap_shim_control_env(key: &str) -> bool {
    ...
    (key.starts_with("RUSTC_") && key != "RUSTC_BOOTSTRAP")
        ...
        || matches!(
            key.as_str(),
            "DUMP_BOOTSTRAP_SHIMS" | "FORCE_ON_BROKEN_PIPE_KILL"
        )
}

fn configure_trust_cargo_fixture<C: TestEnvCommandExt>(
    mut command: C,
    isolation_requested: bool,
    shim_dir: Option<OsString>,
    ...
    let shim_dir = shim_dir.filter(|path| !path.is_empty()).ok_or_else(|| {
        "TEST_TRUST_SHIM_DIR must accompany TRUST_TARGO_TEST_NO_VERIFY".to_owned()
    })?;
    let fixture_path = env::join_paths(
        std::iter::once(PathBuf::from(shim_dir))
            .chain(env::split_paths(&inherited_path.unwrap_or_default())),
    )
    .map_err(|err| format!("could not build Trust-isolated Cargo fixture PATH: {err}"))?;

    // Trust: sweep bootstrap-shim mutation controls before installing the
    // dedicated PATH launcher. Prefix matching is case-folded for Windows, and no
    // canonical private variables are reinstalled in this command's env map.
    // RUSTC_BOOTSTRAP itself remains available because upstream fixtures
    // intentionally inherit or override Rust's unstable-channel escape hatch.
    for key in inherited_keys {
        if is_bootstrap_shim_control_env(&key) {
            command = command.env_remove(&key);
        }
    }
    command = command
        .env("PATH", fixture_path)
        // Do not expose harness authority to fixture programs.
        .env_remove("TRUST_TARGO_TEST_NO_VERIFY")
        .env_remove("TEST_TRUST_SHIM_DIR");
    Ok(command)
}

// Trust: pins the fixture-isolation wiring — that it is inert unless requested,
// that it refuses an incomplete request instead of half-applying, and that a
// deliberate per-test override still wins over it.
#[cfg(test)]
mod trust_targo_test_isolation_tests {
    use std::{env, ffi::OsString, path::PathBuf};

    use cargo_util::ProcessBuilder;

    use super::{configure_trust_cargo_fixture, is_bootstrap_shim_control_env};

    #[test]
    fn sweeps_bootstrap_mutation_controls_but_preserves_rustc_bootstrap_policy() {
        for key in [
            ...
            "FORCE_ON_BROKEN_PIPE_KILL",
            "rustc_host_flags",
            "RuStDoC_lIbDiR",
            "trust_bootstrap_shim_no_verify",
            "trust_bootstrap_no_verify",
        ] {
            assert!(is_bootstrap_shim_control_env(key), "did not sweep {key}");
        }
        for key in [
            "RUSTC_BOOTSTRAP",
            "rustc_bootstrap",
            ...
            assert!(!is_bootstrap_shim_control_env(key), "over-swept {key}");
        }
        assert!(is_bootstrap_shim_control_env("TRUST_NO_VERIFY"));
    }

    #[test]
    fn fixture_wiring_is_explicit_complete_and_later_overridable() {
        let mut command = ProcessBuilder::new("cargo");
        configure_trust_cargo_fixture(&mut command, false, None, None, std::iter::empty()).unwrap();
        assert!(!command.get_envs().contains_key("PATH"));

        let err = configure_trust_cargo_fixture(&mut command, true, None, None, std::iter::empty())
            .unwrap_err();
        assert!(err.contains("TEST_TRUST_SHIM_DIR"));

        let inherited_path = env::join_paths([PathBuf::from("existing-bin")]).unwrap();
        configure_trust_cargo_fixture(
            &mut command,
            true,
            Some(OsString::from("trust-shim-bin")),
            Some(inherited_path),
            ["rustc_host_flags".to_owned(), "TRUST_NO_VERIFY".to_owned()],
        )
        .unwrap();
        let path = env::split_paths(&command.get_env("PATH").unwrap()).collect::<Vec<_>>();
        assert_eq!(
            path,
            [
                PathBuf::from("trust-shim-bin"),
                PathBuf::from("existing-bin")
            ]
        );
        assert_eq!(command.get_envs().get("rustc_host_flags"), Some(&None));
        assert_eq!(command.get_envs().get("TRUST_NO_VERIFY"), Some(&None));
        assert_eq!(
            command.get_envs().get("TRUST_TARGO_TEST_NO_VERIFY"),
            Some(&None)
        );
        assert_eq!(command.get_envs().get("TEST_TRUST_SHIM_DIR"), Some(&None));

        command.env("PATH", "deliberate-override");
        assert_eq!(
            command.get_env("PATH"),
            Some(OsString::from("deliberate-override"))
        );
    }
}
