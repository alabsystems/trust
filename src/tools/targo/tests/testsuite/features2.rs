...
// Trust: `-Zfeatures=ws` used to reach an `unimplemented!()`, turning a typo in
// a CLI flag into a panic. It is an ordinary unsupported-flag error now.
#[cargo_test]
fn unsupported_features_ws_is_a_normal_cli_error() {
    let p = project().file("src/lib.rs", "").build();

    p.cargo("check -Zfeatures=ws")
        .masquerade_as_nightly_cargo(&["features=ws"])
        .with_status(101)
        .with_stderr_data(str![[r#"
[ERROR] -Zfeatures flag `ws` is not supported
