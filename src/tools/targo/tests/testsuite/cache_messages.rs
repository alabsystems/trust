...
fn cached_as_json() {
    ...
    // Trust: the JSON schema extension is additive and skipped when absent;
    // this asserts the compat frontend's output really is unchanged, which is
    // what unmodified ecosystem tooling depends on.
    assert!(
        !orig_cargo_out.contains("trust_compile_target"),
        "plain Cargo JSON must not expose Targo-only target identity fields: {orig_cargo_out}"
    );
    p.cargo("clean").run();
