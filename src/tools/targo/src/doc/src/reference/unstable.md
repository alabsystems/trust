...
```
cargo +trust doc --no-deps -Zrustdoc-map
```
...
default, trustdoc creates links to Trust-owned standard-library docs. To change
this behavior, use the `doc.extern-map.std` setting:
...
A value of `"local"` means to link to the documentation found in the Trust
sysroot. If you are using rustup, this documentation can be installed with
`rustup component add rust-docs --toolchain trust`.

The default value is `"remote"`, which uses the Trust docs root.
