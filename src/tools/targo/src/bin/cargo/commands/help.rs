...
pub fn cli() -> Command {
    subcommand("help")
        // Trust: this text and the errors below name the invoked binary rather
        // than a `cargo` literal; see `frontend_binary_name` in
        // `bin/cargo/main.rs`.
        .about("Displays help for a Cargo-compatible command")
        .arg(
            Arg::new("COMMAND")
                .num_args(1..)
...
pub fn exec(gctx: &mut GlobalContext, args: &ArgMatches) -> CliResult {
    ...
    match find_builtin_cmd(&lookup_parts) {
        ...
        Err(FindError::UnknownSubcommand {
            ...
            let frontend = crate::frontend_binary_name();
            let err = anyhow::format_err!(
                "no such command: `{frontend} {valid_prefix} {invalid}` \n\n\
                 help: view available subcommands with `{frontend} {valid_prefix} --help`",
            );
            return Err(err.into());
        }
    }
