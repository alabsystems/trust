...
pub fn cli() -> Command {
    // Trust: `color_print::cformat!` needs the binary name at expansion time,
    // so usage and long-help are built before the `Command` rather than inline.
    let frontend = crate::frontend_binary_name();
    let usage = color_print::cformat!(
        "\
       <bright-cyan,bold>{frontend} add</> <cyan>[OPTIONS] <<DEP>>[@<<VERSION>>] ...</>
       <bright-cyan,bold>{frontend} add</> <cyan>[OPTIONS]</> <bright-cyan,bold>--path</> <cyan><<PATH>> ...</>
       <bright-cyan,bold>{frontend} add</> <cyan>[OPTIONS]</> <bright-cyan,bold>--git</> <cyan><<URL>> ...</>"
    );
    let dependency_help = format!(
        "Reference to a package to add as a dependency

You can reference a package by:
- `<name>`, like `{frontend} add serde` (latest version will be used)
- `<name>@<version-req>`, like `{frontend} add serde@1` or `{frontend} add serde@=1.0.38`"
    );
    clap::Command::new("add")
        ...
        .after_help(crate::command_help_footer("add"))
