...
mod tests {
    ...
    fn rules_source_dir() -> PathBuf {
        // Trust: these tests inspect the package's source registry, not
        // runtime input. Upstream's cwd-relative helpers assume the test is run
        // from the package directory, which is false under bootstrap. Anchoring
        // at the compile-time manifest root keeps them valid under
        // `--manifest-path`, IDE runners, bootstrap, and arbitrary caller cwd.
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/cargo/diagnostics/rules")
    }

    #[test]
    fn ensure_updated_lints() {
        let dir = rules_source_dir();
        let mut expected = HashSet::new();
        for entry in std::fs::read_dir(&dir).unwrap_or_else(|error| {
            panic!("failed to read lint source dir {}: {error}", dir.display())
        }) {
    ...
    fn ensure_updated_lint_groups() {
        let path = rules_source_dir().join("mod.rs");
        let expected = std::fs::read_to_string(&path).unwrap_or_else(|error| {
            panic!("failed to read lint registry {}: {error}", path.display())
        });
