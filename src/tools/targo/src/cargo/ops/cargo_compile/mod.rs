...
pub fn compile_with_exec<'a>(
    ...
    // Trust: a build that makes no proof claim still has to say which policy it
    // is walking past, or a project can believe its declared level is in force
    // because nothing ever contradicted it.
    crate::util::trust_policy::report_policy_not_honored(ws)?;
