...
enum GeneratedFile {
    ...
    VcsInfo(vcs::VcsInfo),
    // Trust: the package's verification record, already read and courier-checked
    // from disk. Carried as bytes rather than as a path because the check has
    // happened by the time this exists — re-reading at archive time would open a
    // window where the file on disk is not the file that was validated.
    TrustProofCert(Vec<u8>),
}
                    ...
                    let message = messages::PackageList {
                        ...
                        files: BTreeMap::from_iter(ar_files.into_iter().map(|f| {
                            let file = match f.contents {
                                ...
                                FileContents::Generated(
                                    GeneratedFile::VcsInfo(_)
                                    | GeneratedFile::Lockfile(None)
                                    | GeneratedFile::TrustProofCert(_),
...
fn build_ar_list(
    ...
    for src_file in &src_files {
        ...
        match rel_str {
            "Cargo.lock" => continue,
            // Trust: a tracked `.trust/proof.cert` is legitimate — a developer
            // may well commit the record — but it must not enter the archive as
            // an unchecked source file. Skipping it here lets the validated
            // generated entry below be the only copy that ships.
            trust_proof::TRUST_PROOF_CERT_FILE => continue,
    ...
    // Trust: additive by construction. A package with no verification record
    // contributes no entry, no warning, and no error — which is every package
    // that has never been near a Trust toolchain.
    if let Some(cert) = trust_proof::read_proof_cert(pkg)? {
        let rel_str = trust_proof::TRUST_PROOF_CERT_FILE;
        result
            .entry(UncasedAscii::new(rel_str))
            .or_insert_with(Vec::new)
            .push(ArchiveFile {
                rel_path: PathBuf::from(rel_str),
                rel_str: rel_str.to_string(),
                contents: FileContents::Generated(GeneratedFile::TrustProofCert(cert)),
            });
    }
        ...
        match contents {
            ...
            FileContents::Generated(generated_kind) => {
                let contents = match generated_kind {
                    ...
                    // Trust: emitted verbatim. The bytes are the publisher's
                    // record; re-serializing them would make this step look like
                    // it authored the claim, and it did not.
                    GeneratedFile::TrustProofCert(ref bytes) => {
