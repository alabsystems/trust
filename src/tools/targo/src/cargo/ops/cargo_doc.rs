...
fn merge_cross_crate_info(ws: &Workspace<'_>, compilation: &Compilation<'_>) -> CargoResult<()> {
    ...
    for (kind, fingerprint) in fingerprints.iter() {
        ...
        fingerprint.persist(|doc_parts_dirs| {
            let rustdoc_path = ws.gctx().rustdoc()?;
            let mut cmd = ProcessBuilder::new(rustdoc_path);
            if crate::is_targo_invocation() && crate::trust_verified_targo() {
                // Trust: merge-finalize builds its command directly and so
                // bypasses `Compilation::fill_env`. Establish the same closed
                // toolchain loader prefix explicitly before crossing this raw
                // rustdoc execution edge.
                configure_verified_tool_loader_environment(&mut cmd, rustdoc_path)?;
            }
