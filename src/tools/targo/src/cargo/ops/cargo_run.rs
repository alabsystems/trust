...
pub fn run(
    ...
    // Trust: Unix Cargo normally replaces itself with the `run` target. The
    // exact explicit-unverified root owns an in-process authority broker, so
    // replacement would kill the listener while leaking its now-stale address
    // to the target. Keep only that Linux root alive behind a transparent
    // pidfd-backed signal/exit supervisor. Inherited Targo processes do not own
    // the broker and retain upstream exec replacement semantics.
    if crate::util::process_authority::nested_unverified_targo_root_broker_active()? {
        #[cfg(target_os = "linux")]
        {
            process.exec_replace_supervised()
        }
        #[cfg(not(target_os = "linux"))]
        unreachable!("nested-unverified root brokers are Linux-only")
    } else {
