...
enum Chunk {
    ...
    Features,
    LibName,
    // Trust: the dependency's shipped verification record. Display-only — the
    // tree reports what each package claims and judges none of it.
    TrustProof,
}
    ...
    pub fn new(format: &str) -> Result<Pattern, Error> {
        ...
        for raw in Parser::new(format) {
            let chunk = match raw {
                ...
                RawChunk::Argument("proof") => Chunk::TrustProof,
                RawChunk::Argument(a) => {
            ...
            Node::Package {
                ...
                for chunk in &self.pattern.0 {
                    match chunk {
                        ...
                        Chunk::TrustProof => {
                            write!(fmt, "{}", crate::util::trust_proof::describe_proof_cert(package))?;
                        }
                    }
                }
            }
