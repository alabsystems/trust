...
// Trust: the commit-discovery logic runs only in `build.rs`, which no test
// harness executes. Including the source here puts it under the ordinary unit
// tests without linking it into production Targo.
#[cfg(test)]
#[path = "../../build_commit_info.rs"]
