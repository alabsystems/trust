...
fn normalize_toml(
    ...
    let mut normalized_toml = manifest::TomlManifest {
        ...
        lints: None,
        hints: None,
        // Trust: carried unconditionally, unlike the package-only fields
        // above, because a virtual workspace root declares policy for its
        // members and has no `[package]` to hang it from.
        trust: original_toml.trust.clone(),
...
fn prepare_toml_for_publish(
    ...
    let mut manifest = manifest::TomlManifest {
        ...
        // Trust: a published crate keeps the policy it was proved under, so a
        // consumer verifying from the registry sees the same obligations.
        trust: me.trust.clone(),
        workspace: None,
