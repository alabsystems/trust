...
mod imp {
    ...
    // Trust: no unsupported-platform state can be constructed — `current`
    // always returns an error. Encoding that in the type discharges upstream's
    // `unimplemented!()` at compile time instead of leaving a panic a future
    // caller could reach.
    pub struct State(Infallible);
    ...
    pub fn pct_idle(prev: &State, _next: &State) -> f64 {
        match prev.0 {}
    }
}
