...
use crate::util::process_authority::DIAGNOSTICS_SERVER_VAR;
...
fn get_bug_report_url(rustc_workspace_wrapper: &Option<PathBuf>) -> &str {
    get_bug_report_url_with_windows_semantics(rustc_workspace_wrapper, cfg!(windows))
}

/// Trust: upstream matches the wrapper with `file_stem`, which strips any
/// suffix — `clippy-driver.backup` would be credited to Clippy. Whole-name
/// matching, with the Windows semantics split out so both platforms' rules are
/// testable from either host.
fn get_bug_report_url_with_windows_semantics(
    rustc_workspace_wrapper: &Option<PathBuf>,
    windows_semantics: bool,
) -> &str {
    let is_clippy_driver = rustc_workspace_wrapper.as_deref().is_some_and(|path| {
        super::tippy_arg_protocol::executable_path_matches_with_windows_semantics(
            path,
            "clippy-driver",
            windows_semantics,
        )
    ...
    if is_clippy_driver {
        // The inherited driver remains upstream Clippy provenance. Trust's
        // branded tippy-driver and unknown wrappers report to Trust.
...
// Trust: pins that a bug report is attributed by the driver's real identity,
// including the suffix and case variants a stem match would misread.
#[cfg(test)]
mod trust_tests {
    use super::{get_bug_report_url, get_bug_report_url_with_windows_semantics};
    use std::path::PathBuf;

    #[test]
    fn lint_driver_bug_urls_follow_tool_provenance() {
        assert_eq!(
            get_bug_report_url(&Some(PathBuf::from("clippy-driver"))),
            "https://github.com/rust-lang/rust-clippy/issues"
        );
        assert_eq!(
            get_bug_report_url(&Some(PathBuf::from("tippy-driver"))),
            "https://github.com/alabsystems/Trust/issues"
        );
        assert_eq!(
            get_bug_report_url_with_windows_semantics(
        ...
        assert_eq!(
            get_bug_report_url_with_windows_semantics(
                &Some(PathBuf::from("clippy-driver.backup")),
                false,
            ),
            "https://github.com/alabsystems/Trust/issues"
        );
