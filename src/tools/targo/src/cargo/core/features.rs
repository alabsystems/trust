...
pub const SEE_CHANNELS: &str = "Use the Trust `trust` channel, or another Cargo channel with explicit \
     unstable-feature support, for these flags.";
    ...
    fn add(
        ...
        let Some((slot, feature)) = self.status(feature_name) else {
            ...
            if append_see_docs {
                let _ = writeln!(
                    msg,
                    "See the Trust Cargo unstable-features documentation shipped with this toolchain for more information."
                );
            }
            bail!(msg)
        };
        ...
        match feature.stability {
            ...
            Status::Unstable if !nightly_features_allowed => bail!(
                "the cargo feature `{}` requires a Cargo channel with \
                 unstable-feature support, but this is the `{}` channel\n\
                 {}\n{}",
                feature_name,
                channel(),
                SEE_CHANNELS,
...
impl CliUnstable {
    ...
    pub fn parse(
        ...
        if !flags.is_empty() && !nightly_features_allowed {
            bail!(
                "the `-Z` flag is only accepted on Trust/nightly-capable Cargo channels, \
    ...
    fn add(&mut self, flag: &str, warnings: &mut Vec<String>) -> CargoResult<()> {
        ...
        match k {
            ...
            For available unstable features, run `cargo -Z help`\n\
    ...
    pub fn fail_if_stable_opt_custom_z(
        ...
        if !enabled {
            let see = format!(
                "See https://github.com/alabsystems/Trust/issues for more \
                 information about the `{flag}` flag."
            );
            // NOTE: a `config` isn't available here, check the channel directly
            let channel = channel();
            if nightly_features_allowed_on_channel(&channel) {
                bail!(
                ...
                bail!(
                    "the `{flag}` flag is unstable, and only available on Trust/nightly-capable \
                     Cargo channels, but this is the `{channel}` channel\n\
                     {SEE_CHANNELS}\n\
                     {see}"
                );
            }
    ...
    pub fn fail_if_stable_command(
        ...
        let see = format!(
            "See https://github.com/alabsystems/Trust/issues for more \
            information about the `cargo {command}` command."
        );
        if gctx.nightly_features_allowed {
            ...
            bail!(
                "the `cargo {}` command is unstable, and only available on \
                 Trust/nightly-capable Cargo channels, but this is the `{}` channel\n\
                 {}\n\
                 {}",
                command,
                channel(),
...
pub fn nightly_features_allowed_on_channel(channel: &str) -> bool {
    matches!(channel, "nightly" | "dev" | "trust")
}
...
/// Generate a reference to Cargo documentation for the current release channel.
/// `path` is the URL component after `https://doc.rust-lang.org/{channel}/cargo/`
/// for Rust-compatible channels. Trust channels use the locally shipped docs.
pub fn cargo_docs_link(path: &str) -> String {
    if channel() == "trust" {
        return format!("the Trust Cargo documentation shipped with this toolchain ({path})");
    }
...
#[cfg(test)]
mod trust_channel_tests {
    use super::nightly_features_allowed_on_channel;

    #[test]
    fn trust_channel_allows_nightly_features() {
        assert!(nightly_features_allowed_on_channel("trust"));
        assert!(nightly_features_allowed_on_channel("nightly"));
        assert!(nightly_features_allowed_on_channel("dev"));
        assert!(!nightly_features_allowed_on_channel("beta"));
        assert!(!nightly_features_allowed_on_channel("stable"));
    }
}
