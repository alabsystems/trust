...
use cargo_util_schemas::manifest::{Hints, TomlManifest, TomlProfiles, TomlTrust};
use semver::Version;
use serde::Serialize;
use serde::ser;
use url::Url;
...
impl Manifest {
    ...
    /// Trust: the `[trust]` table as this manifest declares it, before any
    /// workspace default is folded in. Reading it from the normalized manifest
    /// keeps one parse of the table for the whole build, which is what lets
    /// policy become a fingerprint input rather than an out-of-band file read.
    pub fn trust_policy(&self) -> Option<&TomlTrust> {
        self.normalized_toml.trust.as_ref()
    }
...
impl VirtualManifest {
    ...
    /// Trust: the `[trust]` table of a virtual workspace root. A virtual
    /// manifest has no package of its own, so this table exists only to supply
    /// defaults to members that leave a key unwritten.
    pub fn trust_policy(&self) -> Option<&TomlTrust> {
        self.normalized_toml.trust.as_ref()
    }
