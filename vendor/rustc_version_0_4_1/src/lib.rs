...
pub fn version_meta_for(verbose_version_string: &str) -> Result<VersionMeta> {
    ...
    let channel = match semver.pre.split('.').next().unwrap() {
        "" => Channel::Stable,
        // Trust uses a repo-owned prerelease tag (for example
        // `1.96.0-trust`) while retaining rustc-compatible channel semantics
        // for ecosystem build scripts that depend on rustc_version.
        "dev" | "trust" => Channel::Dev,
        "beta" => Channel::Beta,
