...
## `tests/ui/compile-time`

Tests guarding against pathological compile-time blowups (many locals, deep
method chains, trait-impl overlap at scale).
...
## `tests/ui/monomorphization`

Tests for monomorphization-time behavior: mono-item collection, recursion
depth limits, and codegen-time diagnostics.
...
Tests for the Trust verifier surface: contracts (`requires`/`ensures`),
clean islands, proof monitors, R1 flips, and verification diagnostics.
