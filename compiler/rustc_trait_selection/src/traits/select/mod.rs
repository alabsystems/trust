...
// Trust: dep-node-carrying cache storage for the cross-call caches below —
// hits must replay the original computation's dep-graph reads.
use rustc_middle::traits::cache::WithDepNodeCache;
...
// Trust: a cap on where-clause evaluation chain length
// (`MAX_PREDICATE_EVALUATION_CHAIN`, #865, rust-lang#95819/#108714) was
// REMOVED here. It silently returned `EvaluatedToAmbig` after 2048 loop
// iterations — a cacheable, wrong verdict that could persist crate-wide via
// the evaluation caches, and a drop-in violation: a single obligation list is
// linear in the size of the user's code (e.g. an auto-trait check over a very
// wide struct), which upstream compiles fine. The exponential *re-evaluation*
// the cap targeted is handled by the cross-call caches below; depth is
// bounded by `check_recursion_limit`, exactly like upstream.

// Trust: A depth-64 throttle on non-trait predicate evaluation
// (`MAX_NON_TRAIT_PREDICATE_DEPTH`, #865/rust-lang#77173) was REMOVED here.
// Its goal — bounding the exponential obligation forest of recursive
// associated-type structures — is already met by the cross-call
// `predicate_evaluation_cache`/`projection_match_cache` below (which dedupe
// the forest to linear work) plus projection cycle detection. The
// `exponential-recursive-assoc-type-77173` regression test is `check-pass`
// at 15 nesting levels — far below 64 — so it passes via those caches and
// never depended on the throttle. Capping non-trait predicates at 64 (half
// the global recursion limit) instead broke legitimate deep auto-trait
// evaluation: e.g. `Send` for wgpu's `Mutex<RawMutex, WeakVec<TextureView>>`
// returned an UNCACHEABLE `EvaluatedToAmbig` at depth 64, defeating
// memoization and inflating the parent trait recursion past the recursion
// limit where stock rustc resolves it well under 128. Non-trait predicates
// are now bounded by `check_recursion_limit` (called at the top of
// `evaluate_predicate_recursively`) exactly like every other predicate and
// like upstream, with the caches providing Trust's extra breadth protection.
...
pub struct SelectionContext<'cx, 'tcx> {
    ...
    // evaluation result is independent of local inference state. Entries carry
    // the `DepNodeIndex` of the anon task that computed them, so hits replay
    // the original computation's dep-graph reads (same as the upstream
    // evaluation/selection caches) — a plain map would under-record
    // dependencies and let stale results go green under incremental.
    predicate_evaluation_cache:
        WithDepNodeCache<ty::ParamEnvAnd<'tcx, ty::Predicate<'tcx>>, EvaluationResult>,
    ...
    // blowup. We cache only fully known projection matches, keyed by the pieces
    // that affect matching and normalization. Dep-node-carrying for the same
    // reason as `predicate_evaluation_cache`.
    projection_match_cache:
        WithDepNodeCache<ProjectionMatchCacheKey<'tcx>, ProjectionMatchesProjection>,
}

#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
struct ProjectionMatchCacheKey<'tcx> {
...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
    pub fn new(infcx: &'cx InferCtxt<'tcx>) -> SelectionContext<'cx, 'tcx> {
        SelectionContext {
            ...
            predicate_evaluation_cache: Default::default(),
            // Trust: Initialize HRTB projection match cache (rust-lang#99188).
            projection_match_cache: Default::default(),
        }
    }
    ...
    fn evaluate_predicate_recursively<'o>(
        ...
        // Cheap pre-filters run BEFORE the resolve fold: a predicate's clause
        // kind is invariant under inference-var resolution and type/const
        // params never resolve away, so trait clauses and param-containing
        // predicates can never produce a cache key — skip the
        // OpportunisticVarResolver fold for them (this path runs once per
        // obligation at every recursion level).
        let may_use_predicate_cache = !matches!(
            obligation.predicate.kind().skip_binder(),
            ty::PredicateKind::Clause(ty::ClauseKind::Trait(_))
        ) && !obligation.predicate.has_param();
        let cache_key = if may_use_predicate_cache {
            let resolved_pred = self.infcx.resolve_vars_if_possible(obligation.predicate);
            // Opaque types count as "global" (`HAS_TY_OPAQUE` is not a free local
            // name), but in TypingMode::Analysis their evaluation depends on the
            // infcx's mutable opaque-type storage — the same reason upstream's
            // can_use_global_caches refuses them. Coroutines likewise.
            let is_non_trait_global = resolved_pred.is_global()
                && !resolved_pred.has_non_region_infer()
                && !resolved_pred.has_opaque_types()
                && !resolved_pred.has_coroutines();
            if is_non_trait_global {
                let key = obligation.param_env.and(resolved_pred);
                if let Some(cached) = self.predicate_evaluation_cache.get(&key, self.tcx()) {
                    debug!(
        ...
        // Trust: the former depth-64 non-trait predicate throttle was removed
        // here (#865/rust-lang#77173) — see the note at the top of this file.
        // Non-trait predicates (projection, WF, subtype, coerce,
        // const-evaluatable) are bounded by `check_recursion_limit` above plus
        // the cross-call evaluation caches, matching upstream depth semantics
        // while keeping Trust's breadth memoization.

        // Trust: when the cross-call cache applies, compute under an anon
        // `TraitSelect` task so the cache entry can replay this evaluation's
        // dep-graph reads on later hits, and track cycle participation via a
        // save/reset/merge protocol on the root's min-reached-depth cell so a
        // result computed under an unproven coinductive premise (a cycle head
        // still on the stack) is never cached beyond this evaluation stack —
        // while results whose cycles completed entirely within this
        // computation (premise discharged) stay cacheable.
        let tcx = self.tcx();
        let infcx = self.infcx;
        let cycle_cache = previous_stack.cache;
        let saved_min_cycle_depth = if cache_key.is_some() {
            Some(cycle_cache.min_cycle_reached_depth.replace(usize::MAX))
        } else {
            None
        };
        let mut compute = || {
            ensure_sufficient_stack(|| {
                ...
                match bound_predicate.skip_binder() {
                    ...
                    ty::PredicateKind::Subtype(p) => {
                        ...
                        match self.infcx.subtype_predicate(
                            &obligation.cause,
                    ...
                    ty::PredicateKind::Coerce(p) => {
                        let p = bound_predicate.rebind(p);
                        // Does this code ever run?
                        match self.infcx.coerce_predicate(
                            &obligation.cause,
                            obligation.param_env,
                            p,
                        ) {
                    ...
                    ty::PredicateKind::Clause(ty::ClauseKind::WellFormed(term)) => {
                        ...
                        for stack_term in previous_stack.cache.wf_args.borrow().iter().rev() {
                            ...
                            // Trust: a WF-arg cycle hit — whether it resolves
                            // coinductively (plain Ok below) or not, results in
                            // this subtree rest on the in-progress premise at
                            // the recorded depth and must not be cross-call
                            // cached by frames enclosing that depth.
                            cache.note_cycle_reached_depth(stack_term.1);
                            ...
                            ProjectAndUnifyResult::Holds(mut subobligations) => {
                                'compute_res: {
                                    ...
                                    for subobligation in subobligations.iter_mut() {
                                        subobligation
                                            .set_depth_from_parent(obligation.recursion_depth);
                                    }
        ...
        let (result, dep_node) = if cache_key.is_some() {
            // Trust: mirror `evaluation_probe` around the cacheable
            // computation: region-constraint / opaque-type registrations made
            // while evaluating a (global) predicate normally leak into the
            // ambient infcx and get folded into the result by the ENCLOSING
            // evaluation_probe — but a cache hit replays only the value, not
            // the side effects, so the `OkModuloRegions`/`OkModuloOpaqueTypes`
            // signal must be baked into the value itself. The probe rollback
            // is what the enclosing probe would eventually do anyway; the
            // signal still propagates upward through the result lattice.
            let (result, dep_node) =
                tcx.dep_graph.with_anon_task(tcx, DepKind::TraitSelect, || {
                    infcx.probe(|snapshot| {
                        let outer_universe = infcx.universe();
                        let result = compute()?;

                        if infcx.leak_check(outer_universe, Some(snapshot)).is_err() {
                            return Ok(EvaluatedToErr);
                        }
                        ...
                        if infcx.region_constraints_added_in_snapshot(snapshot) {
                            Ok(result.max(EvaluatedToOkModuloRegions))
                        } else {
                            Ok(result)
                        }
            ...
            (result, Some(dep_node))
        } else {
            (compute(), None)
        };

        // Trust: merge the subtree's cycle tracking back into the ambient
        // cell (enclosing cacheable frames need to see these events too) and
        // judge premise-dependence: a cycle whose head sits at or above the
        // enclosing trait frame means this result rests on a premise still on
        // the stack; a cycle contained strictly below completed (premise
        // discharged) before this computation returned.
        let mut premise_dependent = false;
        if let Some(saved) = saved_min_cycle_depth {
            let subtree_min = cycle_cache.min_cycle_reached_depth.get();
            cycle_cache.min_cycle_reached_depth.set(saved.min(subtree_min));
            let boundary = previous_stack.head.map_or(0, |h| h.depth);
            premise_dependent = subtree_min <= boundary;
        }
        ...
        // Never cache: stack-dependent results (only valid relative to the
        // evaluation stack that produced them — the same rule
        // insert_evaluation_cache applies), or premise-dependent results (a
        // coinductive cycle hit returns a plain `EvaluatedToOk` that rests on
        // the premise that a cycle head still on the stack will hold; if that
        // head later fails, the cached entry would replay a wrong verdict —
        // the map analogue of upstream's `reached_depth` caching rule).
        if let (Some(key), Ok(eval_result), Some(dep_node)) = (cache_key, &result, dep_node)
            && !eval_result.is_stack_dependent()
            && !premise_dependent
        {
            self.predicate_evaluation_cache.insert(key, dep_node, *eval_result);
        }
    ...
    pub(super) fn match_projection_projections(
        ...
        let cache_key = self.projection_match_cache_key(
            obligation,
            env_predicate,
            potentially_unnormalized_candidates,
        );
        if let Some(key) = cache_key
            && let Some(cached) = self.projection_match_cache.get(&key, self.tcx())
        {
            debug!("match_projection_projections: cache hit for {:?}", key);
            return cached;
        }

        // Trust: when the cache applies, compute under an anon `TraitSelect`
        // task so the cache entry can replay this computation's dep-graph
        // reads on later hits. (Unlike the predicate cache, no coinductive
        // taint check is needed: the nested evaluation below runs under a
        // FRESH ProvisionalEvaluationCache root, so it cannot depend on any
        // in-progress premise of the ambient stack.)
        let tcx = self.tcx();
        let mut compute = || {
            let mut nested_obligations = PredicateObligations::new();
            ...
            let (is_match, cacheable) = match self
                .infcx
                .at(&obligation.cause, obligation.param_env)
                .eq(DefineOpaqueTypes::No, obligation.predicate, infer_projection)
            {
                Ok(InferOk { obligations, value: () }) => {
                    match self.evaluate_predicates_recursively(
                        TraitObligationStackList::empty(&ProvisionalEvaluationCache::default()),
                        ...
                        Ok(result) => (
                            ...
                            matches!(
                                result,
                                EvaluatedToOk | EvaluatedToOkModuloRegions | EvaluatedToErr
                            ),
                        ),
                        Err(_) => (false, false),
                    }
                        ...
                        p.has_non_region_infer()
                            && match p.kind() {
                                ...
                                ty::GenericArgKind::Type(ty) => {
                                    self.infcx.shallow_resolve(ty) != ty
                                }
        ...
        let ((result, cacheable), dep_node) = if cache_key.is_some() {
            let (result, dep_node) =
                tcx.dep_graph.with_anon_task(tcx, DepKind::TraitSelect, compute);
            (result, Some(dep_node))
        } else {
            (compute(), None)
        };

        if let (Some(key), Some(dep_node)) = (cache_key, dep_node)
            && cacheable
            && matches!(result, ProjectionMatchesProjection::No | ProjectionMatchesProjection::Yes)
        {
            self.projection_match_cache.insert(key, dep_node, result);
        }

        result
    }

    fn projection_match_cache_key(
        &self,
        obligation: &ProjectionTermObligation<'tcx>,
        env_predicate: PolyProjectionPredicate<'tcx>,
        potentially_unnormalized_candidates: bool,
    ) -> Option<ProjectionMatchCacheKey<'tcx>> {
        let param_env = obligation.param_env;
        let projection_term = obligation.predicate;
        // Trust: opaques and coroutines are rejected for the same reason
        // `predicate_evaluation_cache` refuses them — in TypingMode::Analysis
        // their meaning depends on the infcx's mutable opaque-type storage,
        // which can change between insert and hit within one SelectionContext.
        if param_env.has_infer()
            || param_env.has_placeholders()
            || param_env.has_opaque_types()
            || param_env.has_coroutines()
            || projection_term.has_infer()
            || projection_term.has_placeholders()
            || projection_term.has_opaque_types()
            || projection_term.has_coroutines()
            || env_predicate.has_infer()
            || env_predicate.has_placeholders()
            || env_predicate.has_opaque_types()
            || env_predicate.has_coroutines()
        {
            return None;
        }

        Some(ProjectionMatchCacheKey {
            param_env,
            projection_term,
            env_predicate,
            potentially_unnormalized_candidates,
...
impl<'o, 'tcx> TraitObligationStack<'o, 'tcx> {
    ...
    fn update_reached_depth(&self, reached_depth: usize) {
        ...
        // Trust: record the cycle participation on the shared root cache so
        // the cross-call predicate cache refuses results computed under an
        // in-progress premise, while still caching results whose cycles
        // completed beneath the enclosing frame. See
        // `ProvisionalEvaluationCache::min_cycle_reached_depth`.
        self.previous.cache.note_cycle_reached_depth(reached_depth);
        let mut p = self;
        while reached_depth < p.depth {
            debug!(?p.fresh_trait_pred, "update_reached_depth: marking as cycle participant");
            p.reached_depth.set(p.reached_depth.get().min(reached_depth));
            let Some(previous) = p.previous.head else {
                break;
            };
            p = previous;
        }
...
struct ProvisionalEvaluationCache<'tcx> {
    ...
    // Trust: the minimum trait-stack depth reached by any cycle participation
    // recorded since the innermost cache-eligible frame reset this cell
    // (bumped by `update_reached_depth` and the WF-arg cycle arm;
    // `usize::MAX` = no cycle seen). The cross-call predicate cache uses a
    // save/reset/merge protocol around each cacheable computation: the result
    // may be cached only if every cycle it participated in had its head
    // STRICTLY BELOW the enclosing trait frame — i.e. the cycle head
    // completed (its premise was discharged) within the computation itself.
    min_cycle_reached_depth: Cell<usize>,
}
...
impl<'tcx> Default for ProvisionalEvaluationCache<'tcx> {
    fn default() -> Self {
        Self {
            ...
            min_cycle_reached_depth: Cell::new(usize::MAX), // Trust: see field docs
        }
    }
}
...
impl<'tcx> ProvisionalEvaluationCache<'tcx> {
    ...
    // Trust: record a cycle participation reaching `depth` — see the field
    // docs on `min_cycle_reached_depth`.
    fn note_cycle_reached_depth(&self, depth: usize) {
        self.min_cycle_reached_depth.set(self.min_cycle_reached_depth.get().min(depth));
    }
