...
struct DiagCtxtInner {
    ...
    /// Lifetime-monotonic census of diagnostic side effects attempted through
    /// this context, including suppressed expectations and stashed diagnostics.
    /// Consumers may use an unchanged epoch as a conservative proof that a
    /// computation emitted no diagnostic state. Concurrent unrelated
    /// diagnostics can only cause a false "changed" result.
    diagnostic_side_effect_epoch: usize,
    ...
    /// Used to suggest the right `<driver> --explain <error code>` command.
...
pub struct DiagCtxtFlags {
    ...
    /// The driver name to suggest in `--explain <error code>` notes.
    ///
    /// Trust normally reports `trustc`, but upstream-compatibility runs use
    /// vanilla mode and must keep upstream rustc diagnostics byte-for-byte.
    pub explain_binary: &'static str,
}

impl DiagCtxtFlags {
    fn explain_binary(self) -> &'static str {
        if self.explain_binary.is_empty() { "trustc" } else { self.explain_binary }
    }
}
...
impl DiagCtxt {
    ...
    pub fn reset_err_count(&self) {
        ...
        let DiagCtxtInner {
            ...
            diagnostic_side_effect_epoch: _,
            emitter: _,
            must_produce_diag,
            has_printed,
        ...
        *deduplicated_warn_count = 0;
        // Deliberately do not reset `diagnostic_side_effect_epoch`. External
        // parser users may reuse this context after clearing its diagnostics,
        // and a later emission must never reuse an epoch that a consumer could
        // still hold as a snapshot.
...
impl<'a> DiagCtxtHandle<'a> {
    ...
    pub fn stash_diagnostic(
        ...
        let mut inner = self.inner.borrow_mut();
        inner.diagnostic_side_effect_epoch = inner
            .diagnostic_side_effect_epoch
            .checked_add(1)
            .expect("diagnostic side-effect epoch overflow");
        inner
            .stashed_diagnostics
            .entry(key)
            .or_default()
    ...
    /// A lifetime-monotonic snapshot of diagnostic state changes. If the value is
    /// unchanged across a computation, that computation did not emit, stash,
    /// or fulfill a diagnostic through this context. A change may come from
    /// another query running concurrently, so callers must treat it only as a
    /// conservative "not side-effect-free" signal.
    pub fn diagnostic_side_effect_epoch(&self) -> usize {
        self.inner.borrow().diagnostic_side_effect_epoch
    }
        ...
        if can_show_explain && are_there_diagnostics {
            ...
            if !error_codes.is_empty() {
                ...
                if error_codes.len() > 1 {
                    ...
                    let explain_binary = inner.flags.explain_binary();
                    let msg2 = format!(
                        "For more information about an error, try `{explain_binary} --explain {}`.",
                        &error_codes[0]
                    );
                    ...
                    let explain_binary = inner.flags.explain_binary();
                    let msg = format!(
                        "For more information about this error, try `{explain_binary} --explain {}`.",
                        &error_codes[0]
                    );
...
impl DiagCtxtInner {
    fn new(emitter: Box<DynEmitter>) -> Self {
        Self {
            ...
            deduplicated_err_count: 0,
            deduplicated_warn_count: 0,
            diagnostic_side_effect_epoch: 0,
            emitter,
            must_produce_diag: None,
            has_printed: false,
            suppressed_expected_diag: false,
    ...
    fn emit_diagnostic(
        ...
        self.diagnostic_side_effect_epoch = self
            .diagnostic_side_effect_epoch
            .checked_add(1)
            .expect("diagnostic side-effect epoch overflow");
