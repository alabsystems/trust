...
pub fn register_builtin_macros(resolver: &mut dyn ResolverExpand) {
    ...
    // Trust: the trust-spec attribute surface. The resolver's spec-contract
    // hook substitutes these for the `trust::requires` / `trust::ensures`
    // proc-macro passthroughs, so Trust-origin clauses stay distinguishable
    // from upstream `core::contracts` clauses in the parser.
    let trust_requires = SyntaxExtensionKind::Attr(Arc::new(contracts::ExpandTrustRequires));
    register(sym::trust_contracts_requires, trust_requires);
    let trust_ensures = SyntaxExtensionKind::Attr(Arc::new(contracts::ExpandTrustEnsures));
    register(sym::trust_contracts_ensures, trust_ensures);
}
