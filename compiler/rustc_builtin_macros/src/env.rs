...
/// Returns `true` if an environment variable from `env!` could be one set by
/// the build tool (targo, or its Cargo compatibility surface).
fn is_cargo_env_var(var: &str) -> bool {
    // Trust-first: `TARGO_*` is the canonical spelling targo sets for compiled
    // crates; `CARGO_*` is the retained twin for foreign crates.io code.
    var.starts_with("TARGO_")
