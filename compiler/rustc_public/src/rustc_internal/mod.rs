    ...
    ($args:expr, $callback:expr $(, $with_tcx:ident)?) => {{
            ...
            F: FnOnce($($crate::optional!($with_tcx TyCtxt))?) -> ControlFlow<B, C> + Send,
        {
            ...
            pub fn run(&mut self, args: &[String]) -> Result<C, CompilerError<B>> {
                // Trust: nested rustc_public/rustc_driver analysis compilations
                // are exempted from the verifier via the inherited `TRUST_NO_VERIFY`
                // env the --trust-vanilla harness sets (docs/test-env-vars.md) — the
                // ONE nested-propagation mechanism, covering every API uniformly.
