...
type SettledValTree<'tcx> = (EvalToValTreeResult<'tcx>, usize);

fn ensure_node_budget<'tcx>(
    num_nodes: usize,
    additional: usize,
) -> Result<(), ValTreeCreationError<'tcx>> {
    if additional > VALTREE_MAX_NODES.saturating_sub(num_nodes) {
...
fn reserve_nodes<'tcx>(
    num_nodes: &mut usize,
    additional: usize,
) -> Result<(), ValTreeCreationError<'tcx>> {
    ensure_node_budget(*num_nodes, additional)?;
    *num_nodes += additional;
    Ok(())
}
...
fn branches<'tcx>(
    ...
    settled: &mut FxHashMap<MPlaceTy<'tcx>, SettledValTree<'tcx>>,
    ...
    let branch_count = field_count
        .checked_add(variant.is_some() as usize)
        .ok_or(ValTreeCreationError::NodesOverflow)?;
    // Every aggregate contributes its `Branch` node, and every child contributes
    // at least one node. Check that lower bound before allocating the child list.
    ensure_node_budget(
        *num_nodes,
        branch_count.checked_add(1).ok_or(ValTreeCreationError::NodesOverflow)?,
    )?;
    reserve_nodes(num_nodes, 1)?;
...
fn slice_branches<'tcx>(
    ...
    let capacity = usize::try_from(n).map_err(|_| ValTreeCreationError::NodesOverflow)?;
    // Charge the aggregate `Branch` and require room for at least one node per
    // element before reserving attacker-controlled capacity.
    ensure_node_budget(
        *num_nodes,
        capacity.checked_add(1).ok_or(ValTreeCreationError::NodesOverflow)?,
    )?;
    reserve_nodes(num_nodes, 1)?;
...
fn const_to_valtree_inner<'tcx>(
    ...
    if let Some(&(result, cached_nodes)) = settled.get(place) {
        return match result {
            Ok(valtree) => {
                reserve_nodes(num_nodes, cached_nodes)?;
                Ok(valtree)
            }
            Err(error) => Err(error),
        };
    ...
    let settled_nodes = result.is_ok().then_some(*num_nodes - initial_nodes).unwrap_or(0);
    debug_assert!(result.is_err() || settled_nodes > 0);
    settled.insert(place.clone(), (result, settled_nodes));
    result
}
