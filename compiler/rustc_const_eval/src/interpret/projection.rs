    ...
    M: Machine<'tcx, Provenance = Prov>,
{
    ...
    pub fn project_field<P: Projectable<'tcx, M::Provenance>>(
        ...
        let (meta, offset) = if field_layout.is_unsized() {
            ...
            // struct. Also, extern-tailed types are unsized but thin, so they have no metadata
            // to consult and the statically computed offset is already the field offset.
