...
// env_mutation.rs
#[derive(Diagnostic)]
pub(crate) enum EnvMutationDiag {
    #[diag(
        "call to `{$fn_path}` mutates the process-global environment: process-global environment mutation races parallel threads and env-gated readers"
    )]
    #[help(
        "route the mutation through a lock-scoped helper that sets and restores the variable under a global lock, and bless that single call site with `#[allow(env_mutation)]`"
    )]
    Call { fn_path: &'static str },
    #[diag(
        "use of `{$fn_path}` as a function value: process-global environment mutation races parallel threads and env-gated readers"
    )]
    #[help(
        "route the mutation through a lock-scoped helper that sets and restores the variable under a global lock, and bless that single call site with `#[allow(env_mutation)]`"
    )]
    FnValue { fn_path: &'static str },
}
...
pub(crate) enum UnknownLintSuggestion {
    #[suggestion(
        "{$from_rustc ->
            [true] a lint with a similar name exists in `trustc` lints
            *[false] did you mean
        }",
        code = "{replace}",
        applicability = "maybe-incorrect"
    ...
    #[help(
        "{$from_rustc ->
            [true] a lint with a similar name exists in `trustc` lints: `{$replace}`
...
#[note("for more information, see the Trust Reference's destructors section")]
