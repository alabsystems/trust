...
struct AllUninitRepeatLocal<'fx, 'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> {
    fx: &'fx FunctionCx<'a, 'tcx, Bx>,
    local: mir::Local,
    definitions: usize,
    all_uninit_definitions: usize,
    repeat_uses: usize,
    other_uses: usize,
}

impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> Visitor<'tcx>
    for AllUninitRepeatLocal<'_, 'a, 'tcx, Bx>
{
    fn visit_assign(
        &mut self,
        place: &mir::Place<'tcx>,
        ...
        if place.as_local() == Some(self.local) {
            self.definitions += 1;
            if self.fx.rvalue_is_known_all_uninit_definition(rvalue) {
                self.all_uninit_definitions += 1;
            }
        } else {
    ...
    fn visit_rvalue(&mut self, rvalue: &mir::Rvalue<'tcx>, location: mir::Location) {
        if let mir::Rvalue::Repeat(operand, _) = rvalue
            && let Some(place) = operand.place()
            && place.as_local() == Some(self.local)
        {
            self.repeat_uses += 1;
            return;
        }
    ...
    fn visit_terminator(&mut self, terminator: &mir::Terminator<'tcx>, location: mir::Location) {
        if let mir::TerminatorKind::Call { func, args, destination, .. } = &terminator.kind {
            ...
            if destination.as_local() == Some(self.local) {
                self.definitions += 1;
                if self.fx.call_is_maybe_uninit_uninit(func, args) {
                    self.all_uninit_definitions += 1;
                }
            } else {
                self.visit_place(
    ...
    fn visit_local(&mut self, local: mir::Local, context: PlaceContext, _location: mir::Location) {
        if local == self.local && context.is_use() {
            self.other_uses += 1;
        }
    }
}
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    pub(crate) fn codegen_rvalue(
        ...
        match *rvalue {
            ...
            mir::Rvalue::Repeat(ref elem, count) => {
                ...
                // Repeating an all-uninit value leaves the destination uninitialized.
                if self.operand_is_known_all_uninit_for_repeat(elem) {
                    return;
                }
    ...
    fn operand_is_known_all_uninit_for_repeat(&self, operand: &mir::Operand<'tcx>) -> bool {
        match operand {
            mir::Operand::Constant(const_op) => {
                self.eval_mir_constant(const_op).all_bytes_uninit(self.cx.tcx())
            }
            mir::Operand::Copy(place) | mir::Operand::Move(place) => {
                place.as_local().is_some_and(|local| self.local_is_only_repeated_all_uninit(local))
            }
            mir::Operand::RuntimeChecks(_) => false,
        }
    }

    fn local_is_only_repeated_all_uninit(&self, local: mir::Local) -> bool {
        let mut visitor = AllUninitRepeatLocal {
            fx: self,
            local,
            definitions: 0,
            all_uninit_definitions: 0,
            repeat_uses: 0,
            other_uses: 0,
        };
        visitor.visit_body(self.mir);
        visitor.definitions == 1
            && visitor.all_uninit_definitions == 1
            && visitor.repeat_uses == 1
            && visitor.other_uses == 0
    }

    fn rvalue_is_known_all_uninit_definition(&self, rvalue: &mir::Rvalue<'tcx>) -> bool {
        match rvalue {
            mir::Rvalue::Use(mir::Operand::Constant(const_op), _) => {
                self.eval_mir_constant(const_op).all_bytes_uninit(self.cx.tcx())
            }
            _ => false,
        }
    ...
    fn call_is_maybe_uninit_uninit(
        &self,
        ...
        let ty::FnDef(def_id, _) = *func.ty().kind() else {
            return false;
        };
        self.cx.tcx().is_diagnostic_item(sym::maybe_uninit_uninit, def_id)
    }
