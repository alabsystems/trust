...
impl<'a, 'tcx> TerminatorCodegenHelper<'tcx> {
    ...
    fn do_call<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        let needs_foreign_unwind_pad = callee_abi.is_some_and(|abi| !abi.is_rustic_abi())
            && !base::wants_new_eh_instructions(fx.cx.tcx().sess)
            && !base::wants_wasm_eh(fx.cx.tcx().sess);

        let unwind_block = match unwind {
            mir::UnwindAction::Cleanup(cleanup) if needs_foreign_unwind_pad => {
                let anchor = self.emit_foreign_unwind_anchor(bx);
                Some(self.foreign_unwind_landing_pad_for(fx, cleanup, anchor))
            }
    ...
    fn emit_foreign_unwind_anchor<Bx: BuilderMethods<'a, 'tcx>>(&self, bx: &mut Bx) -> Bx::Value {
        let anchor = bx.alloca(Size::from_bytes(1), Align::ONE);
        let zero = bx.cx().const_u8(0);
        bx.store_with_flags(zero, anchor, Align::ONE, MemFlags::VOLATILE);
        anchor
    }

    fn foreign_unwind_landing_pad_for<Bx: BuilderMethods<'a, 'tcx>>(
        &self,
        fx: &mut FunctionCx<'a, 'tcx, Bx>,
        cleanup: mir::BasicBlock,
        anchor: Bx::Value,
    ) -> Bx::BasicBlock {
        let cleanup_llbb = Bx::append_block(fx.cx, fx.llfn, "foreign_unwind_pad");
        let mut cleanup_bx = Bx::build(fx.cx, cleanup_llbb);

        let llpersonality = fx.cx.eh_personality();
        let (exn0, exn1) = cleanup_bx.cleanup_landing_pad(llpersonality);

        let slot = fx.get_personality_slot(&mut cleanup_bx);
        slot.storage_live(&mut cleanup_bx);
        Pair(exn0, exn1).store(&mut cleanup_bx, slot);

        let i8_ty = cleanup_bx.cx().type_i8();
        cleanup_bx.volatile_load(i8_ty, anchor, Align::ONE);
        cleanup_bx.br(fx.llbb(cleanup));
        cleanup_llbb
    }
