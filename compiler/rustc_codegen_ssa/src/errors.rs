    ...
    fn into_diag(mut self, dcx: DiagCtxtHandle<'_>, level: Level) -> Diag<'_, G> {
        ...
        if contains_undefined_ref {
            ...
            if rustc_session::utils::was_invoked_from_cargo() {
                diag.note(msg!("use the `cargo:rustc-link-lib` directive to specify the native libraries to link with targo-compatible build scripts; see the Trust Cargo build script documentation for that directive"));
            }
        }
        diag
    }
