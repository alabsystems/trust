...
impl<'tcx> crate::MirPass<'tcx> for SimplifyComparisonIntegral {
    ...
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        // Trust: cheap syntactic pre-scan — a candidate needs a SwitchInt
        // whose own block assigns the discriminant from an Eq/Ne comparison.
        // This late in the pipeline (after GVN/MatchBranchSimplification/
        // JumpThreading) most bodies have zero such shapes; skip the full
        // SsaLocals analysis (the dominant cost) for them. Strict superset of
        // find_optimizations' filter, which additionally requires SSA-ness
        // and a constant integral operand.
        let has_candidate = body.basic_blocks.iter().any(|bb| {
            let Some((discr, _)) = bb.terminator().kind.as_switch() else { return false };
            let Some(place_switched_on) = discr.place() else { return false };
            bb.statements.iter().any(|stmt| {
                matches!(
                    &stmt.kind,
                    StatementKind::Assign((lhs, Rvalue::BinaryOp(BinOp::Eq | BinOp::Ne, _)))
