    ...
    B: Deref<Target = Body<'tcx>>,
{
    ...
    // Trust: the cheap structural shape checks run FIRST — any const whose
    // body is not literally `_0 = const X; return` (arithmetic, calls,
    // multiple statements) bails here without paying the
    // impossible-predicates evaluation (fresh InferCtxt + ObligationCtxt per
    // call) or the opaque_types_defined_by query below. This path runs for
    // every const-like def (array lengths, enum discriminants, const generic
    // args) at all opt levels. Both call paths have mir_built computed
    // already, so forcing the body first costs nothing extra.
