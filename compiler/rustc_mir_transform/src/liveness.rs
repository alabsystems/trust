...
/// Trust: whether any contract clause uses the exact parameter identifier.
/// Reads the authored clause surface straight from HIR — the `FnContract`
/// clause spans plus their token-rendered `payload` spellings, the very
/// inputs the `trust_contracts` provider lowers — and MUST NOT force the
/// `trust_contracts` query itself: that query's provider emits contract
/// lowering errors ("invalid `requires` clause: ..."), and `check_liveness`
/// runs inside `run_required_analyses`, BEFORE the
/// `front_end_errors_before_trust_hooks` snapshot in
/// `rustc_interface::passes::analysis`. Forcing the query here reclassifies
/// those verification-lane errors as front-end errors, which makes the
/// front-end-clean gate skip `trust_ensure_whole_crate_verification` — Clean
/// island checking and the E9 citation sweep — for the ENTIRE crate. Reading
/// HIR is non-emitting by construction, and the lowering errors keep
/// surfacing at the query's original post-snapshot force sites (the
/// TrustVerify pass and the citation sweep).
///
/// The lexical scan is identifier-position aware: it excludes
/// member/associated-name positions, comments, and literals; raw substring
/// matching is never used. Fail-open direction is a MISSING warning (a name
/// coincidentally shared with an unrelated clause identifier), never a
/// spurious one.
fn trust_contract_clauses_mention(tcx: TyCtxt<'_>, def_id: DefId, name: Symbol) -> bool {
    let Some(local) = def_id.as_local() else { return false };
    if !rustc_middle::mir::trust_contract::TrustContractBundle::def_may_have_contracts(tcx, local) {
        return false;
    }
    let Some(body) = tcx.hir_maybe_body_owned_by(local) else { return false };
    let Some(contract) = body.contract else { return false };
    let source_map = tcx.sess.source_map();
    let name = name.as_str();
    // A native clause's `payload` symbol is the faithful spelling authority
    // (macro expansion can make the span unrecoverable); the clause-span
    // source scan is the conservative catch-all — attribute-origin clauses
    // carry no payload symbol. Checking both over-approximates mentions,
    // which keeps this analysis fail-safe: a missed mention would produce a
    // spurious warning, so we over- rather than under-approximate.
    let clause_mentions = |span: Span, payload: Option<Symbol>| {
        payload.is_some_and(|text| contract_source_mentions_identifier(text.as_str(), name))
            || source_map
                .span_to_snippet(span)
                .is_ok_and(|text| contract_source_mentions_identifier(&text, name))
    };
    contract
        .requires
        .iter()
        .chain(contract.ensures)
        .chain(contract.decreases)
        .any(|clause| clause_mentions(clause.span, clause.payload))
        || contract
            .loop_clauses
            .iter()
            .any(|clause| clause_mentions(clause.payload_span, Some(clause.payload)))
}

fn contract_source_mentions_identifier(text: &str, name: &str) -> bool {
    use rustc_lexer::{FrontmatterAllowed, TokenKind};

    let mut offset = 0usize;
    let tokens = rustc_lexer::tokenize(text, FrontmatterAllowed::No)
        .filter_map(|token| {
            let start = offset;
            ...
            .then(|| (token.kind, &text[start..offset]))
        })
        .collect::<Vec<_>>();

    tokens.iter().enumerate().any(|(index, (kind, spelling))| {
...
impl<'a, 'tcx> AssignmentResult<'a, 'tcx> {
    ...
    fn report_fully_unused(&mut self) {
        ...
        for (index, place) in self.checked_places.iter() {
            ...
            // Trust: a parameter whose only use is inside a trust contract
            // clause (`fn f(x: u64) requires x > 0 {}`) is
            // NOT unused — the clause is part of the function's specification
            // surface. Vanilla Rust has no trust contracts, so this changes
            // nothing for it. Query-owned `Var` nodes are preferred; the
            // authored-span fallback is tokenized and identifier-position
            // aware for opaque/synthetically-named native predicates.
            if matches!(self.body.local_kind(local), LocalKind::Arg)
                && trust_contract_clauses_mention(tcx, self.body.source.def_id(), name)
            {
                continue;
            }
    ...
    fn report_unused_assignments(self) {
        ...
        for (index, statements) in self.assignments.into_iter_enumerated() {
            ...
            // A contract mention consumes the value passed at function entry,
            // but it does not consume later assignments in the Rust body. The
            // fully-unused pass above suppresses `unused_variables` for this
            // same exact case; mirror that decision only for the synthetic
            // `Param` access so `unused_assignments` remains independent.
            let place = self.checked_places.places[index];
            let trust_contract_uses_param =
                matches!(self.body.local_kind(place.local), LocalKind::Arg)
                    && trust_contract_clauses_mention(tcx, self.body.source.def_id(), name);
            ...
            for (source_info, Access { live, kind, is_direct, location }) in statements.into_iter()
            {
                ...
                let should_report = !live
                    && (is_direct || !is_maybe_drop_guard)
                    && !(kind == AccessKind::Param && trust_contract_uses_param);
...
// Trust: the clause-source matching cases are Trust's and exercise a private
// helper, so they stay in this module's own tree.
#[cfg(test)]
mod tests;
