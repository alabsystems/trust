...
impl<'tcx> crate::MirPass<'tcx> for SsaRangePropagation {
    ...
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        // Trust: a range can change MIR only when the exact source Place is read
        // by another Operand. Count operand uses with a cheap, read-only walk and
        // retain only Assert/SwitchInt/Assume sources with at least two uses (the
        // source's own operand is one). This is a strict superset of the real
        // opportunities below: SSA-ness, dominance, and unique-predecessor gates
        // can only remove candidates. It therefore skips SsaLocals + dominators
        // for the common branch-shaped body whose condition is never reused, and
        // prevents the ranges map becoming write-only for unrelated sources.
        let candidates = range_propagation_candidates(body);
        ...
        let mut range_set =
            RangeSet::new(tcx, typing_env, body, &ssa, &body.local_decls, dominators, &candidates);
...
#[derive(Default)]
struct CandidateScan<'tcx> {
    operand_uses: FxHashMap<Place<'tcx>, u32>,
    // Preserve MIR traversal order so finishing the scan never iterates a hash
    // set inside a query (rustc's potential-query-instability lint). The set is
    // membership-only defensive dedup.
    range_sources: Vec<Place<'tcx>>,
    seen_sources: FxHashSet<Place<'tcx>>,
}

impl<'tcx> CandidateScan<'tcx> {
    fn record_source(&mut self, operand: &Operand<'tcx>) {
        if let Some(place) = operand.place()
            && place.is_stable_offset()
            && self.seen_sources.insert(place)
        {
            self.range_sources.push(place);
        }
    }
}

impl<'tcx> Visitor<'tcx> for CandidateScan<'tcx> {
    fn visit_operand(&mut self, operand: &Operand<'tcx>, _location: Location) {
        if let Some(place) = operand.place()
            && place.is_stable_offset()
        {
            let count = self.operand_uses.entry(place).or_default();
            *count = count.saturating_add(1);
        }
    }

    fn visit_statement(&mut self, statement: &Statement<'tcx>, location: Location) {
        if let StatementKind::Intrinsic(NonDivergingIntrinsic::Assume(operand)) = &statement.kind {
            self.record_source(operand);
        }
    ...
    fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, location: Location) {
        match &terminator.kind {
            TerminatorKind::Assert { cond, .. } => self.record_source(cond),
            // RangeSet intentionally declines larger switches; mirror that
            // gate so their discriminants cannot keep the expensive path live.
            TerminatorKind::SwitchInt { discr, targets } if targets.all_targets().len() < 16 => {
                self.record_source(discr);
            }
            _ => {}
        }
...
fn range_propagation_candidates<'tcx>(body: &Body<'tcx>) -> FxHashSet<Place<'tcx>> {
    let mut scan = CandidateScan::default();
    scan.visit_body(body);
    scan.range_sources
        .into_iter()
        .filter(|place| scan.operand_uses.get(place).is_some_and(|uses| *uses >= 2))
        .collect()
}
...
struct RangeSet<'tcx, 'body, 'a> {
    ...
    /// Raw syntactic superset of places that can actually be rewritten.
    candidates: &'a FxHashSet<Place<'tcx>>,
...
impl<'tcx, 'body, 'a> RangeSet<'tcx, 'body, 'a> {
    ...
    fn insert_range(&mut self, place: Place<'tcx>, location: Location, range: WrappingRange) {
        ...
        if self.candidates.contains(&place) {
