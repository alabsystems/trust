...
impl<'tcx> crate::MirPass<'tcx> for Validator {
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let mut cfg_checker = CfgChecker {
            ...
            can_unwind,
            locals_with_def: DenseBitSet::new_empty(body.local_decls.len()),
            first_control_read: IndexVec::from_elem_n(None, body.local_decls.len()),
        };
        cfg_checker.visit_body(body);
        cfg_checker.check_cleanup_control_flow();
        cfg_checker.check_local_definedness();
...
struct CfgChecker<'a, 'tcx> {
    ...
    can_unwind: bool,
    // Trust: locals that appear in at least one `MutatingUse` ANYWHERE in the body, including
    // unreachable blocks. Recording a write we may never reach can only suppress a report, never
    // invent one, which is the direction a validator must err in. See `check_local_definedness`.
    locals_with_def: DenseBitSet<Local>,
    // Trust: for each local, the first CONTROL-FLOW read seen in a reachable block — a `SwitchInt`
    // discriminant or an `Assert` condition. Kept so the diagnostic can point at the read.
    first_control_read: IndexVec<Local, Option<Location>>,
}

impl<'a, 'tcx> CfgChecker<'a, 'tcx> {
    /// Trust: control flow must not depend on a local that nothing in the body ever writes.
    ///
    /// MIR is not SSA. A `Local` is a storage slot named by an index, and a read is tied to its
    /// definition by nothing but that index — the body records no edge from a definition to its
    /// users. So a pass can delete a defining statement while a use survives, and the result is
    /// still a well-formed `Body`: every local declared, every place well-typed, the CFG intact.
    /// Nothing else in this file would notice.
    ///
    /// That is not hypothetical. `MatchBranchSimplification`'s contiguous-range fold popped the
    /// statement defining a comparison temp on the assumption it was single-use; `GVN` had already
    /// CSE'd it into a multi-use temp, so three blocks were left switching on an uninitialized
    /// local. It miscompiled `rustc_abi::Integer::fit_unsigned`, which gave every enum with a
    /// multi-byte discriminant a one-byte tag, and the compiler it built rejected `rustix` with
    /// `E0080` on a promoted `&FileType`. Validation ran on that body — `pass_manager` makes it
    /// mandatory at the `Runtime(Optimized)` phase change regardless of `-Zvalidate-mir` — and had
    /// nothing to say, because this file had no notion of definedness at all.
    ///
    /// # Why only control-flow reads
    ///
    /// The general form — *any* read of a never-written local — was implemented first and is too
    /// strict. It rejects `core::fmt::builders::field_with::{closure#0}`, where a block is
    /// CFG-reachable but semantically dead: it is the `Ok` arm of the `match residual` in
    /// `FromResidual for Result`, whose payload is the uninhabited `Infallible`. Binding a payload
    /// that can never exist is a legitimate shape, and `traversal::reachable_as_bitset` is pure
    /// CFG reachability, so it cannot tell that arm from a live one.
    ///
    /// Restricting to `SwitchInt` discriminants and `Assert` conditions keeps the case that
    /// matters — *execution being routed by an undefined value* — and drops the dead-arm payload
    /// reads that produced the false positive. It still catches the defect above exactly: that bug
    /// was three `switchInt(copy _2)` with `_2` written nowhere.
    ///
    /// The check is deliberately FLOW-INSENSITIVE: "is there a write anywhere" rather than "does a
    /// write reach this read". That is the cheap question and the safe one — it cannot fire on a
    /// legitimately conditionally-initialized local, needs no `MoveData`, and costs one bitset test
    /// per local. `MaybeUninitializedPlaces` asks the precise question, is move-path indexed, and
    /// is far too expensive to run after every optimization pass.
    ///
    /// Writes in unreachable blocks still count as writes: suppressing a report is safe, inventing
    /// one is not. `_0` and the argument locals are established by the ABI rather than by a
    /// statement, so they are excluded.
    fn check_local_definedness(&self) {
        for (local, first_read) in self.first_control_read.iter_enumerated() {
            let Some(location) = *first_read else { continue };
            // `_0` is index 0 and the arguments are `1..=arg_count`.
            if local.index() <= self.body.arg_count {
                continue;
            }
            if !self.locals_with_def.contains(local) {
                self.fail(
                    location,
                    format!(
                        "control flow here depends on local {local:?}, which is never written \
                         anywhere in the body; a pass deleted its defining statement while this \
                         use survived"
                    ),
                );
            }
...
impl<'a, 'tcx> Visitor<'tcx> for CfgChecker<'a, 'tcx> {
    fn visit_local(&mut self, local: Local, context: PlaceContext, location: Location) {
        if self.body.local_decls.get(local).is_none() {
            ...
            // `fail` returns when errors were already emitted, and the bookkeeping below indexes
            // by `local` — so stop here rather than index a local that has no declaration.
            return;
        }
        // Trust: definedness bookkeeping — record every context that could WRITE this local.
        // `NonUse` (storage markers, `VarDebugInfo`) writes nothing and is ignored; ordinary reads
        // are collected from terminators instead, see `check_local_definedness`.
        if let PlaceContext::MutatingUse(_) = context {
            self.locals_with_def.insert(local);
        }
    }
    ...
    fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, location: Location) {
        // Trust: record CONTROL-FLOW reads for `check_local_definedness` — the operands whose
        // value decides where execution goes next.
        if self.reachable_blocks.contains(location.block) {
            let control_operand = match &terminator.kind {
                TerminatorKind::SwitchInt { discr, .. } => Some(discr),
                TerminatorKind::Assert { cond, .. } => Some(cond),
                _ => None,
            };
            if let Some(local) =
                control_operand.and_then(|op| op.place()).and_then(|place| place.as_local())
                && self.first_control_read[local].is_none()
            {
                self.first_control_read[local] = Some(location);
            }
        }
