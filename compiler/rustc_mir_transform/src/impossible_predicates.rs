...
pub(crate) fn has_impossible_predicates(tcx: TyCtxt<'_>, def_id: DefId) -> bool {
    ...
    tracing::trace!(?predicates);
    // Trust: the filter above removes every clause referencing the item's own
    // generics, so most bodies reach here with NO predicates at all — and
    // `traits::impossible_predicates` has no empty-input early-out (it always
    // builds a fresh InferCtxt + ObligationCtxt). An empty list can never be
    // impossible; skip the whole setup. This runs per body at all opt levels.
