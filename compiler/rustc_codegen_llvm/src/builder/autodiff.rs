...
// need to match those.
// FIXME(ZuseAY): This logic is a bit more complicated than it should be, can we simplify it
...
fn match_args_from_caller_to_enzyme<'ll, 'tcx>(
    ...
    while activity_pos < inputs.len() {
        ...
        if duplicated {
            ...
            // FIXME(ZuseAY): We should add support for Vec here too, but it's less urgent since
            ...
            if slice {
                ...
                // FIXME(ZuseAY): We will upstream a safety check later which asserts that
...
// FIXME(ZuseAY): `outer_fn` should include upstream safety checks to
...
pub(crate) fn generate_enzyme_call<'ll, 'tcx>(
    ...
    // FIXME(ZuseAY): the CC/Addr/Vis values are best effort guesses, we should look at tests and
