...
struct LoweringContext<'a, 'hir> {
    ...
    // Trust: one checker binding per `#[ensures]` clause (empty = no
    // contract); `return` lowering chains a check per entry.
    contract_ensures: Vec<(Span, Ident, HirId)>,

    /// Trust: first-class loop clauses (`while c invariant P decreases e`)
    /// collected while lowering the CURRENT body's expressions; drained into
    /// the body's `hir::FnContract` when the body is recorded. Saved/restored
    /// per body so a closure's loop clauses attach to the closure's own body.
    trust_loop_clauses: Vec<hir::LoopClause>,

    /// Trust: exact AST-expression to HIR-expression identity for typed
    /// function-contract clauses in the CURRENT body. Contract clause spans
    /// are diagnostic only: macro expansion may assign equal spans to
    /// distinct predicates. Saved/restored per body so lowering an ensures
    /// closure cannot consume or contaminate its enclosing function's map.
    trust_contract_clause_exprs: NodeMap<HirId>,
...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    fn new(tcx: TyCtxt<'hir>, resolver: &'a ResolverAstLowering<'hir>, owner: NodeId) -> Self {
        ...
        Self {
            ...
            trust_loop_clauses: Vec::new(),
            trust_contract_clause_exprs: NodeMap::default(),
...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    fn with_new_scopes<T>(&mut self, scope_span: Span, f: impl FnOnce(&mut Self) -> T) -> T {
        ...
        let old_contract = mem::take(&mut self.contract_ensures);
