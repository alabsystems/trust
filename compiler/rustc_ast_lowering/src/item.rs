...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    fn lower_item_kind(
        ...
        match i {
            ...
            // Trust: a Clean island lowers to a span-only HIR marker — no
            // body owner (nothing to codegen or resolve). The verify pass
            // recovers the island text through the source map and hands it
            // to the Clean parser + kernel (two-language design E10).
            ItemKind::CleanIsland(content_span) => {
                hir::ItemKind::CleanIsland { content_span: self.lower_span(*content_span) }
            }
    ...
    fn lower_foreign_item(&mut self, i: &ForeignItem) -> &'hir hir::ForeignItem<'hir> {
        ...
        let (ident, kind) = match &i.kind {
            ForeignItemKind::Fn(Fn { sig, ident, generics, contract, define_opaque, .. }) => {
                self.reject_bodyless_contract(
                    contract.as_deref(),
                    i.span,
        ...
        let (ident, generics, kind, has_value) = match &i.kind {
            ...
            AssocItemKind::Fn(Fn {
                ...
                self.reject_bodyless_contract(
                    contract.as_deref(),
                    i.span,
                    "required trait method declarations",
                );
    ...
    /// Bodyless declarations currently have no HIR body on which to retain a
    /// [`hir::FnContract`]. Reject an authored contract loudly at lowering
    /// rather than allowing its predicate or citation to disappear from the
    /// verification inventory.
    fn reject_bodyless_contract(
        &self,
        ...
        self.dcx().span_err(
            authored_span,
            format!(
                "contract clauses are not supported on {declaration_kind}; \
                 bodyless contract metadata cannot be preserved for verification"
            ),
        );
    }
    ...
    fn record_body(
        ...
        value: hir::Expr<'hir>,
        contract: Option<&'hir hir::FnContract<'hir>>,
    ) -> hir::BodyId {
        let body = hir::Body { params, value: self.arena.alloc(value), contract };
        let id = body.id();
    ...
    pub(super) fn lower_body(
        ...
        self.lower_body_with_contract(None, f)
    }

    fn lower_body_with_contract(
        &mut self,
        ...
        // Trust: loop clauses are collected per body while its expressions
        // lower (a closure's clauses belong to the closure's own body).
        let prev_loop_clauses = std::mem::take(&mut self.trust_loop_clauses);
        // Typed contract predicates likewise belong to exactly one body. In
        // particular, lowering an ensures closure recursively lowers the
        // closure body; that nested body must not steal the enclosing
        // clause's AST -> HIR identity map.
        let prev_contract_clause_exprs = std::mem::take(&mut self.trust_contract_clause_exprs);
        let (parameters, result) = f(self);
        let loop_clauses = std::mem::replace(&mut self.trust_loop_clauses, prev_loop_clauses);
        let contract_clause_exprs =
            std::mem::replace(&mut self.trust_contract_clause_exprs, prev_contract_clause_exprs);
        // The signature clauses come from the AST contract; the loop clauses
        // from the walk above. Either alone produces a HIR contract, so no
        // authored clause can silently drop (design §1.2-6).
        let hir_contract = self.lower_hir_contract(contract, loop_clauses, contract_clause_exprs);
        let body_id = self.record_body(parameters, result, hir_contract);
    ...
    pub(super) fn lower_fn_body(
        ...
        self.lower_body_with_contract(contract, |this| {
            let params =
    ...
    fn lower_maybe_coroutine_body(
        ...
        //
        // Trust: `lower_contract` never runs for coroutine bodies, so any
        // requires/ensures clause EXPRESSIONS here would leave the closure
        // DefIds collected during def collection as phantom owners — an ICE
        // at crate-metadata encoding (`No HirId for DefId(..::{closure#N})`).
        // The stable attribute surface is already rejected at expansion by
        // the contracts builtin macro; this guards the `contracts_internals`
        // keyword syntax with the same real error instead of the ICE. Opaque
        // trust-spec clauses are spans-only (no expressions, no DefIds) and
        // remain allowed.
        if let Some(contract) = contract {
            let typed_clause_span = contract
                .requires_clauses
                .first()
                .map(|e| e.span)
                .or_else(|| contract.requires.as_ref().map(|e| e.span))
                .or_else(|| contract.ensures_clauses.first().map(|e| e.span))
                .or_else(|| contract.ensures.as_ref().map(|e| e.span));
            if let Some(span) = typed_clause_span {
                self.dcx().span_err(
                    span,
    ...
    fn error_on_invalid_abi(&self, abi: StrLit) {
        ...
        self.dcx().emit_err(InvalidAbi {
            ...
            command: "trustc --print=calling-conventions".to_string(),
        });
    }
