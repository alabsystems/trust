...
pub(crate) fn build_mir_inner_impl<'tcx>(tcx: TyCtxt<'tcx>, def: LocalDefId) -> Body<'tcx> {
    ...
    let body = match tcx.thir_body(def) {
        ...
        Ok((thir, expr)) => {
            ...
            let mir = build_mir(&thir);
            // Trust: P1 — lower source (THIR) directly to trust-ir + differential gate vs MIR
            // (parallel to MIR). THIR is still fresh here, before unsafeck steals it. The freshly
            // built `mir` is passed by ref so the oracle builds the MIR-side trust-ir from it
            // directly — never `optimized_mir` (which would be a query cycle here).
            if tcx.sess.trust_ir_lower_enabled()
                || trust_thir_lower::crate_module::emit_requested(tcx)
            {
                // Keep the direct-lane snapshots scoped together; they are all consumed before
                // the freshly built MIR leaves this query.
                {
                    ...
                    // finalize. Extracted here once; never re-extracted in `record`.
                    let (report, mir_snapshot) =
                        trust_thir_lower::differential::compare(tcx, def, &lowered, &mir);
                    // The measurement-only collect-all lowering is needed by either the
                    // debug scorecard or coverage.json. Compute it at most once even when
                    // both consumers are enabled; the old split ownership re-lowered the
                    // same failed body independently in this hook and in record().
                    //
                    // Trust: this producer condition must stay a superset of every consumer
                    // condition in `crate_module::record`. `--emit=trust-ir` publishes the
                    // coverage companion without the `-Z trust-dump=ir` dev flag, so omitting
                    // it here does not disable the artifact — it publishes a
                    // structurally-valid, silently EMPTY `collect_primary`/`collect_cascade`
                    // histogram, which reads as "this body demanded no leaf shapes" rather
                    // than "nobody measured". An empty producer table must never be
                    // indistinguishable from a clean one. The second lowering it buys
                    // back is bounded to bodies that already FAILED, in a run whose whole
                    // purpose is to produce that artifact; a verifier compile without
                    // `--emit=trust-ir` still never pays it.
                    let collect_debug = tracing::enabled!(tracing::Level::DEBUG);
                    let collect_requested = !report.unsupported.is_empty()
                        && (collect_debug
                            || tcx.sess.opts.unstable_opts.trust_dump.ir.is_some()
                            || trust_thir_lower::crate_module::emit_requested(tcx));
                    let mut collect_all = if collect_requested {
                        trust_thir_lower::lower_module_collect_all_from_thir(tcx, def, &thir, expr)
                    } else {
                        Vec::new()
                    };
                    let collect_all_overflowed = collect_all.len() > 4096;
                    // Trust (A2): under `-Ztrust-ir-lower=strict`, a body the producer cannot
                    // fully lower is an ERROR rather than a coverage-only skip that quietly ships
                    // built MIR. Without this the producer's coverage cannot be ratcheted: a
                    // regression and a shape nobody has implemented yet are the same observation
                    // — a `debug!` event and a row in a sidecar.
                    //
                    // Strict deliberately admits NO tags rather than carrying an in-compiler
                    // allow-list. The measured tag population is large (114 distinct reasons over
                    // 768 of 1972 sampled bodies, reports/2026-07-25-trust-ir-producer-scorecard-
                    // postfix.md), so an allow-list would start as a transcription of the current
                    // gaps and would have to be edited DOWN — the same shape as a ledger that is
                    // emptied by adding exceptions. The unit that grows instead is the set of
                    // files strict is run over.
                    if tcx.sess.trust_ir_lower_strict() && !report.unsupported.is_empty() {
                        let mut shapes: Vec<String> = report
                            .unsupported
                            .iter()
                            .map(|(at, what)| format!("{what} at {}", at.as_str()))
                            .collect();
                        shapes.sort();
                        shapes.dedup();
                        tcx.dcx().span_err(
                            tcx.def_span(def),
                            format!(
                                "-Ztrust-ir-lower=strict: the direct trust-ir producer could not \
                                 fully lower this body ({} unsupported shape(s)): {}",
                    ...
                    if !report.unsupported.is_empty() {
                        tracing::debug!(
                            ...
                            "trust-ir-lower: unsupported THIR shapes"
                        );
                        // Trust: producer ratchet — one event per unsupported reason so the
                        // scorecard (scripts/trust_ir_producer_scorecard.py) can histogram
                        // tags, not just counts.
                        for (at, what) in &report.unsupported {
                            tracing::debug!(
                                ?def,
                                at = at.as_str(),
                                what,
                                "trust-ir-lower: unsupported shape"
                            );
                        }
                        // Trust (v2 Phase 0b, RFC docs/TRUST_IR_V2.md §4): the COLLECT-ALL
                        // second pass — re-lower the FAILED body with the sibling-walk seams
                        // enabled so the events below approach its FULL leaf demand (the strict
                        // events above are the first-fail prefix; `Call(unsupported arg)` alone
                        // masks 98.6% of its subtree tags). Measurement-only: the collect-all
                        // lowering is discarded except its tag vector (`lower_module_collect_all`
                        // returns tags alone), and this branch only runs for a body the strict
                        // pass already failed. One aggregated event per (tag, class): class is
                        // "cascade" for unbound-local echoes (the downstream shadow of an
                        // earlier failed binding), "primary" otherwise. Gated on the debug log
                        // actually being collected (scorecard runs set RUSTC_LOG) — a normal
                        // verifier compile never pays the second lowering.
                        if collect_debug {
                            let mut agg: std::collections::BTreeMap<&str, u64> =
                                std::collections::BTreeMap::new();
                            for (_at, what) in &collect_all {
                                let w: &str = what;
                                let count = agg.entry(w).or_default();
                            ...
                            for (what, n) in agg {
                                ...
                                tracing::debug!(
                                    ...
                                    n,
                                    class,
                                    "trust-ir-lower: collect-all tag"
                                );
                            }
                        }
                    }
                    // coverage.json has always retained at most the first 4096
                    // events plus an overflow marker. Preserve that contract
                    // after the debug consumer (which historically saw the full
                    // scorecard stream) has finished.
                    collect_all.truncate(4096);
                    // Trust: P1.4 — semantic-equivalence (differential interpretation) verdict.
                    // Logged for every body: `mode` distinguishes Agreed (proved equal) from
                    // MirOracle (real divergence verdict) from NotRun (coverage-only); `note`
                    // carries the reason (e.g. oracle incapacity vs THIR defect).
                    //
                    // Trust (B9-A): SUPPRESS this event for bodies DEFERRED to the crate-finalize
                    // seam (clean + call-bearing). The seam emits their one real verdict at
                    // finalize; emitting a NotRun "direct call" event here too would (via the
                    // scorecard's first-event-wins) shadow the seam verdict and inflate the
                    // clean-skip-direct-call bucket by ~1064. `deferred_to_seam` is the SINGLE
                    // source of truth shared with `record` (must-fix 5) — deferred bodies are
                    // clean by definition, so the orphan check
                    // `unsupported_event_without_differential_event` stays 0.
                    let deferred = trust_thir_lower::differential::deferred_to_seam(&lowered);
                    if !deferred {
                        tracing::debug!(
                            ?def,
                            equal = report.equal,
                            ...
                            mode = ?report.mode,
                            note = report.notes.last().map(|s| s.as_str()).unwrap_or(""),
                            "trust-ir-lower: differential"
                        );
                    }
                    // Trust: P1.2 slice 1 — derived-vs-built MIR differential (the trust-ir ->
                    // MIR shim). Log-only: the derived Body is never fed to borrowck / codegen /
                    // queries. This structural-symbolic gate reaches a verdict on exactly the
                    // arithmetic/control-flow bodies the interpreter differential above must
                    // skip (oracle-Undef blindness).
                    let derived_report = trust_thir_lower::mir_differential::compare_derived(
                        tcx,
                        def,
                        &lowered,
                        &mir,
                    ...
                    tracing::debug!(
                        ?def,
                        verdict = ?derived_report.verdict,
                        detail = derived_report.detail.as_str(),
                        markers_exact = derived_report.markers_exact,
                        markers = derived_report.markers_detail.as_str(),
                        norms = ?derived_report.norms,
                        tally = ?derived_report.tally,
                        "trust-ir-lower: derived-MIR differential"
                    );
                    // Trust: P1 item 5 (the flip) — remember the trust-ir Module for bodies
                    // whose derived-MIR differential is green. `optimized_mir`
                    // (rustc_mir_transform::trust_ir_flip) re-derives MIR from it and consumes
                    // it on the codegen path; borrowck stays on the `mir` built here.
                    // Session-gated + fail-closed inside `record_green` (`-Ztrust-ir-flip=no`
                    // escape hatch, debuginfo, coverage) + the per-BODY exact-marker gate
                    // (`markers_exact` is required whenever the session emits lifetime
                    // markers, i.e. at -O — the flip flips what it can prove per level).
                    if derived_report.verdict
                        == trust_thir_lower::mir_differential::DerivedVerdict::DerivedAgreed
                    {
                        trust_thir_lower::flip_registry::record_green(
                            tcx,
                            def,
                            &lowered,
                            derived_report.markers_exact,
                        );
                    }
                    // Trust: P1 Phase-0 — hand the per-body result to the crate-level registry.
                    // It is assembled into ONE crate `trust_ir::Module` (+ coverage sidecar) at the
                    // `rustc_interface::passes::analysis` seam; the artifact is dumped there when
                    // `-Z trust-dump=ir:<dir>` is also set. Must happen HERE: this hook is the only seam
                    // where the THIR-side lowering reliably exists per body (THIR is stolen by
                    // unsafeck afterwards).
                    //
                    // Trust (B9-A): `record` is now UNCONDITIONAL under `-Z trust-ir-lower` — the
                    // assembled crate module also drives the crate-seam call-linking differential
                    // ([`crate_module::run_seam_differentials`]), not only the dump. `mir_snapshot`
                    // (the MIR-side `extract_function`, taken once by `compare`) rides along so the
                    // seam can bundle a call-bearing body against its callees.
                    trust_thir_lower::crate_module::record(
                        tcx,
                        def,
                        lowered,
                        &report,
