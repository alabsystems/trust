...
pub(crate) fn thir_body<'tcx>(
    ...
    // Trust (P1 follow-on): serve the check-THIR from an ACCEPTED witness replay
    // instead of rebuilding it. The entry exists only after
    // `trust_build_check_thir` passed the SAME taint gate and the linear checker
    // accepted the root (trust_replay.rs), and type_consts are never minted, so
    // this is a pure performance reuse of an already-validated THIR. Empty on any
    // miss/reject => normal build below (fail-safe). Downstream consumers get a
    // normal `&Steal<Thir>` with unchanged steal semantics.
    if let Some((thir, expr)) = tcx.trust_witness_thir_take(owner_def) {
        return Ok((tcx.alloc_steal_thir(thir), expr));
    }
...
// Trust (typeck-moonshot P1, Phase 2): build a THROWAWAY THIR from a
// caller-supplied (re-interned witness) `TypeckResults` instead of the memoized
// `tcx.typeck(def)`, returning an OWNED `Thir` (no `alloc_steal_thir`, so the
// `thir_body` query cache is untouched). This is the check-THIR the linear
// witness checker validates before any replayed `typeck` result is used. It is a
// faithful copy of `thir_body` above, differing only in (1) the `TypeckResults`
// source (`new_with_results` vs `new`) and (2) the owned return.
pub(crate) fn trust_build_check_thir<'tcx>(
    tcx: TyCtxt<'tcx>,
    def: LocalDefId,
    results: &'tcx ty::TypeckResults<'tcx>,
) -> Result<(Thir<'tcx>, ExprId), ErrorGuaranteed> {
    debug_assert!(!tcx.is_type_const(def.to_def_id()), "trust_build_check_thir on type_const");

    let body = tcx.hir_body_owned_by(def);
    let mut cx: ThirBuildCx<'tcx> = ThirBuildCx::new_with_results(tcx, def, results);
    if let Some(reported) = cx.typeck_results.tainted_by_errors {
        return Err(reported);
    }

    // Lower the params before the body's expression so errors from params are shown first.
    let owner_id = tcx.local_def_id_to_hir_id(def);
    if let Some(fn_decl) = tcx.hir_fn_decl_by_hir_id(owner_id) {
        let closure_env_param = cx.closure_env_param(def, owner_id);
        let explicit_params = cx.explicit_params(owner_id, fn_decl, &body);
        cx.thir.params = closure_env_param.into_iter().chain(explicit_params).collect();

        // The resume argument may be missing, in that case we need to provide it here.
        // It will always be `()` in this case.
        if tcx.is_coroutine(def.to_def_id()) && body.params.is_empty() {
            cx.thir.params.push(Param {
                ty: tcx.types.unit,
                pat: None,
                ty_span: None,
    ...
    // The returned `ExprId` is unused: the checker walks every `thir.exprs` node.
    // Return the root ExprId too so the accepted check-THIR can be reused as
    // the body's real THIR (the single-build follow-on); the checker itself
    // walks every `thir.exprs` node and does not need this expr.
    let expr = cx.mirror_expr(body.value);
    Ok((cx.thir, expr))
}
...
impl<'tcx> ThirBuildCx<'tcx> {
    fn new(tcx: TyCtxt<'tcx>, def: LocalDefId) -> Self {
        Self::new_with_results(tcx, def, tcx.typeck(def))
    }

    // Trust (typeck-moonshot P1, Phase 2): same as `new` but with a
    // caller-supplied `typeck_results` (the re-interned witness candidate)
    // instead of `tcx.typeck(def)`, so `trust_build_check_thir` can build a
    // throwaway check-THIR from the candidate without touching the `typeck`
    // query cache. The body below is verbatim `new`, only `typeck_results` is a
    // parameter now.
    fn new_with_results(
        tcx: TyCtxt<'tcx>,
        def: LocalDefId,
