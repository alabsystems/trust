...
// Trust: machine-checked unsafe-operation catch-completeness lock.
//
// `docs/unsafe-coverage-completeness.md` asserts Trust's verifier recognizes every
// rustc unsafe-operation kind ("proves no others exist", detection direction). That
// was a HAND-maintained table that silently goes stale the moment rustc adds a new
// `UnsafeOpKind`. The exhaustive match below (NO wildcard arm) makes the
// correspondence machine-checked: it fails to compile if rustc introduces a new
// unsafe-op kind, forcing Trust to classify it instead of leaving a silent hole.
// It is intentionally never called — dead code is still exhaustiveness-checked, so
// this is a pure compile-time completeness lock with zero runtime effect.
#[allow(dead_code)]
enum TrustUnsafeCatch {
    /// A dedicated authoritative Trust detector emits a kind-specific `[unsafe:*]`
    /// obligation (see `rustc_mir_transform::trust_verify` / `trust_vcgen`).
    SpecificDetector,
    /// The compiler-side call scan (`fn_sig().safety()` / target-feature check)
    /// catches it.
    CallScan,
    /// The separation engine models or flags it.
    SeparationEngine,
    /// Caught only by the conservative every-unsafe-block finding (so it does NOT
    /// escape detection), with no dedicated kind-specific detector. For the kinds
    /// currently classified this way this is the APPROPRIATE design, not a gap
    /// awaiting a dedicated detector:
    /// they are the `incomplete` unstable features `unsafe_fields` /
    /// `unsafe_binders`, detected by rustc at THIR level where the field/binder
    /// info exists — by the time Trust's MIR-level verifier runs, that signal is
    /// lowered away, so a faithful MIR detector is not feasible. The conservative
    /// net is the correct mechanism for them until/unless they stabilize and gain
    /// a stable MIR representation. The machine-checked lock keeps this honest: a
    /// NEW variant that IS MIR-detectable must not silently land here.
    ConservativeNetOnly,
}

// Trust: the lock itself. Adding a `UnsafeOpKind` variant without a corresponding
// arm here is a COMPILE ERROR — the mechanism that keeps the completeness claim
// honest as upstream rustc evolves.
#[allow(dead_code)]
fn trust_unsafe_op_catch(kind: &UnsafeOpKind) -> TrustUnsafeCatch {
    use TrustUnsafeCatch::*;
    match kind {
        CallToUnsafeFunction(_) => CallScan,
        CallToFunctionWith { .. } => CallScan,
        UseOfInlineAssembly => SpecificDetector,
        AccessToUnionField => SpecificDetector,
        UseOfMutableStatic => SpecificDetector,
        UseOfExternStatic => SpecificDetector,
        MutationOfLayoutConstrainedField => SpecificDetector,
        BorrowOfLayoutConstrainedField => SpecificDetector,
        DerefOfRawPointer => SeparationEngine,
        // Trust: 1.99 — upstream removed `InitializingTypeWith` and added
        // `CallDropExplicitly(DefId)` (explicit `Drop::drop` call). A call-shaped
        // THIR check, so the MIR call-scan detector covers it.
        CallDropExplicitly(_) => CallScan,
        // `incomplete` unstable features (unsafe_fields / unsafe_binders), detected
        // by rustc at THIR level; the signal is lowered away before Trust's
        // MIR-level verifier runs, so a faithful kind-specific MIR detector is not
        // feasible. The conservative every-unsafe-block net catches them (no escape)
        // and is the correct mechanism here. Surfaced + classified by this lock.
        UseOfUnsafeField => ConservativeNetOnly,
        InitializingTypeWithUnsafeField => ConservativeNetOnly,
        UnsafeBinderCast => ConservativeNetOnly,
    }
}
