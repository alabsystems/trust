...
// for now it works.
// Blessed `env_mutation` site: single-threaded build script restoring the
// environment bootstrap saw. Build scripts do not receive `--cfg bootstrap`,
// so allow the trust-only lint by name and tolerate compilers without it.
...
fn main() {
    ...
    for lib in &quoted_split(cmd) {
        ...
        let name = if let Some(stripped) = lib.strip_prefix("-l") {
            ...
            } else if lib.ends_with(".dylib") || lib.ends_with(".so") {
                // Trust: an external llvm-config (e.g. Homebrew's) may emit a
                // system lib as an absolute dylib path (e.g.
                // /opt/homebrew/opt/z3/lib/libz3.dylib) rather than `-lz3`.
                // Reduce it to a search path plus the bare stem so the link line
                // is well-formed; the stem is then subject to the filters below
                // (in particular z3 is dropped — Trust doesn't use LLVM's Z3).
                println!("cargo:rustc-link-search=native={}", path.parent().unwrap().display());
                let name = path.file_stem().unwrap().to_str().unwrap();
                name.trim_start_matches("lib")
            } else {
        ...
        // Trust: Z3 is LLVM's optional SMT solver (LLVM_ENABLE_Z3_SOLVER, e.g.
        // in Homebrew's LLVM). `llvm-config --system-libs` lists it, but Trust
        // never invokes LLVM's Z3 path — verification uses AY exclusively, so the
        // Z3Solver object is never pulled from libLLVMSupport.a and `-lz3` is
        // spurious (it only makes tool links fail when libz3 isn't on the search
        // path). Drop it so the toolchain carries no Z3 dependency.
        if name == "z3" || name.starts_with("z3.") {
            continue;
        }
