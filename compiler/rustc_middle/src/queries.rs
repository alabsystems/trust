...
rustc_queries! {
    ...
    /// An immutable snapshot of a local function's elaborated MIR for Trust verification.
    ///
    /// `optimized_mir` ensures this query before stealing the source body. This lets
    /// recursive Trust-IR closure construction inspect any local function without peeking
    /// at the mutable state of a `Steal` or depending on query execution order.
    query mir_for_trust_verification(key: LocalDefId) -> &'tcx mir::Body<'tcx> {
        no_hash
        desc { "caching MIR of `{}` for Trust verification", tcx.def_path_str(key) }
    }

    /// Compact direct-local-callee adjacency used by Trust's query-cycle oracle.
    /// Unlike `mir_for_trust_verification`, this retains no full MIR body.
    query trust_mir_direct_local_callees(key: LocalDefId) -> &'tcx [LocalDefId] {
        desc { "collecting direct local Trust callees of `{}`", tcx.def_path_str(key) }
        cache_on_disk
    }

    /// Sorted local functions in mutual (two-or-more-function) call SCCs.
    /// A direct self edge is deliberately excluded because rustc can optimize it
    /// without forming an `optimized_mir` query cycle. Computing every SCC once
    /// avoids a transitive graph walk for each optimized function.
    query trust_mir_mutually_recursive_functions(_: ()) -> &'tcx [LocalDefId] {
        desc { "computing mutually recursive local Trust functions" }
        cache_on_disk
    }
    ...
    /// Trust (B): the direct source(THIR) -> trust-ir lowering for one body.
    ///
    /// This exists so the Module has DEP-GRAPH IDENTITY. It was previously produced only as a
    /// side effect of `mir_built` executing, into a Session-owned registry; on an incremental
    /// replay where `mir_built` is cache-green, no Module existed and the compiler silently fell
    /// back to built MIR. An artifact that vanishes on a warm cache is an overlay, not a path.
    ///
    /// Only the LOWERING lives here. The differential stays at the `mir_built` hook, because it
    /// compares against the freshly built body by reference — reaching for `optimized_mir` there
    /// would be a query cycle.
    ///
    /// `None` is a body the producer declined; it is not an error and not evidence of anything.
    query trust_ir_of(key: LocalDefId) -> Option<&'tcx trust_ir::Module> {
        // `no_hash` because `trust_ir::Module` is a foreign type and the orphan rule forbids a
        // `HashStable` impl for it here. Invalidation is unaffected — it flows through this
        // query's dep edges (THIR, and the options the producer reads), not through the result
        // hash. What is lost is only the "recomputed to the same value" early-out.
        no_hash
        desc { "lowering `{}` from THIR to trust-ir", tcx.def_path_str(key) }
    }

    /// Returns compiler-native Trust contracts for an item.
    query trust_contracts(key: DefId) -> &'tcx mir::trust_contract::TrustContractBundle<'tcx> {
        desc { "retrieving Trust contracts for `{}`", tcx.def_path_str(key) }
        arena_cache
        // Upstream replaced `cache_on_disk_if { <pred> }` with the unconditional
        // `cache_on_disk` modifier; the disk-cache eligibility is now handled by the
        // query system (this query is only invoked for items that have contracts).
        cache_on_disk
        separate_provide_extern
    }
