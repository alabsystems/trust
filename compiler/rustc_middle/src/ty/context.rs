...
impl<'tcx> CtxtInterners<'tcx> {
    fn new(arena: &'tcx WorkerLocal<Arena<'tcx>>) -> CtxtInterners<'tcx> {
        ...
        CtxtInterners {
            ...
            // the interner in functions like `intern_ty`, then compile a large crate — bigger
            // workloads give more accurate ratios.
            // Trust: no benchmark harness is named because the toolchain carries no
            // compile-benchmark corpus of its own.
...
pub struct GlobalCtxt<'tcx> {
    ...
    /// Trust (typeck-moonshot P1): typeck roots recorded as mintable during a
    /// `-Z trust-witness=mint:<dir>` compile; drained by the analysis seam to commit
    /// the per-crate witness store. Inert unless the mint flag is set.
    pub trust_witness_minted: Lock<rustc_data_structures::fx::FxIndexSet<LocalDefId>>,

    /// Trust (typeck-moonshot P1 follow-on): the check-THIR built from an
    /// ACCEPTED witness replay, keyed by root, drained by `thir_body` so the
    /// replay path builds THIR ONCE (the checker's build is reused for MIR).
    /// Empty unless the linear checker accepted this root, so a miss/reject
    /// leaves it empty and `thir_body` rebuilds normally (fail-safe).
    pub trust_witness_thir_lookaside:
        Lock<FxHashMap<LocalDefId, (crate::thir::Thir<'tcx>, crate::thir::ExprId)>>,
}
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn create_global_ctxt<T>(
        ...
        let gcx = gcx_cell.get_or_init(|| GlobalCtxt {
            ...
            current_gcx,
            jobserver_proxy,
            trust_witness_minted: Default::default(),
            trust_witness_thir_lookaside: Default::default(),
        });
    ...
    /// Compatibility predicate for the retired deferred-verification preview.
    ///
    /// The preview skipped borrow checking and related diagnostic sweeps, so its
    /// command-line option and implementation were removed. A few shared pipeline
    /// seams still use this predicate to guard their old branches; keeping it
    /// permanently false makes those seams fail closed until they are removed.
    pub fn trust_defer_verification(self) -> bool {
        false
    }
    ...
    /// Trust (typeck-moonshot): the selected witness lane.
    fn trust_witness_lane(self) -> &'tcx rustc_session::config::TrustWitness {
        &self.sess.opts.unstable_opts.trust_witness
    }

    /// Trust (typeck-moonshot, A1-A3): shadow-mint parity measurement of the precise
    /// edit-loop key. Mint+replay use the precise key, but a replayed candidate is
    /// only byte-diffed against real typeck (never trusted) — see trust_replay.
    pub fn trust_witness_precise(self) -> bool {
        self.sess.opts.unstable_opts.trust_witness_precise
    }

    /// Trust (typeck-moonshot): the auto-mode default store, `<out-dir>/trust-witness`.
    /// `None` when there is no explicit `--out-dir` (a bare `-o file`), so the auto
    /// lane stays inert on ad-hoc single-file compiles and engages on real builds.
    fn trust_witness_default_dir(self) -> Option<std::path::PathBuf> {
        self.sess.io.output_dir.as_ref().map(|d| d.join("trust-witness"))
    }

    /// Trust (typeck-moonshot P1): is the witness MINT lane active this compile?
    /// Cheap (no allocation), safe to call per-root. Riders: non-incremental (the
    /// on-disk `typeck` cache is the incremental alternative, and returning a
    /// replayed result into that on-disk-cached provider would be a cache-poison
    /// vector — see the soundness audit), no `-Zunpretty`, no `-Zvalidate-mir`.
    /// Off ⇒ inert, output byte-identical to upstream.
    pub fn trust_witness_mint_active(self) -> bool {
        let lane = self.trust_witness_lane();
        self.trust_witness_riders_ok()
            && lane.mints()
            && (lane.explicit_store().is_some() || self.sess.io.output_dir.is_some())
    }

    /// Trust (typeck-moonshot P1): is the witness REPLAY lane active this compile?
    pub fn trust_witness_replay_active(self) -> bool {
        let lane = self.trust_witness_lane();
        self.trust_witness_riders_ok()
            && lane.replays()
            && (lane.explicit_store().is_some() || self.sess.io.output_dir.is_some())
    ...
    /// The mint store directory (explicit lane store, else
    /// `<out-dir>/trust-witness`).
    pub fn trust_witness_mint_dir(self) -> Option<std::path::PathBuf> {
        if !self.trust_witness_mint_active() {
            return None;
        }
        self.trust_witness_lane()
            .explicit_store()
            .cloned()
            .or_else(|| self.trust_witness_default_dir())
    }

    /// The replay store directory (explicit lane store, else
    /// `<out-dir>/trust-witness`); the managed lane mints and replays through
    /// one store, so a mint in one compile is replayed by the next.
    pub fn trust_witness_replay_dir(self) -> Option<std::path::PathBuf> {
        if !self.trust_witness_replay_active() {
            return None;
        }
        self.trust_witness_lane()
            .explicit_store()
    ...
    /// Router (typeck-moonshot): replay pays a fixed decode + check-THIR + checker
    /// cost, so it only wins on bodies whose fresh-typeck cost clears that floor.
    /// Gate mint + replay on a cheap O(1) HIR node-count proxy for body cost, so
    /// the lane engages AUTOMATICALLY only where it helps — small bodies fall
    /// straight through to normal typeck (no trivial-body slowdown). Tunable via
    /// `TRUST_WITNESS_MIN_NODES` (default 64).
    pub fn trust_witness_body_big_enough(self, def_id: LocalDefId) -> bool {
        fn min_nodes() -> usize {
            static MIN: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
            *MIN.get_or_init(|| {
                std::env::var("TRUST_WITNESS_MIN_NODES")
                    .ok()
                    .and_then(|s| s.parse::<usize>().ok())
                    .unwrap_or(64)
            })
        }
        self.opt_hir_owner_nodes(def_id).is_some_and(|o| o.nodes.len() >= min_nodes())
    }

    /// The router only applies to the managed lane: an explicit
    /// `-Ztrust-witness=mint:<dir>` mints every eligible root (measurement +
    /// deterministic harnesses depend on it), while `auto` skips small bodies
    /// to stay net-positive.
    pub fn trust_witness_mint_router_skip(self, def_id: LocalDefId) -> bool {
        self.trust_witness_lane().is_auto() && !self.trust_witness_body_big_enough(def_id)
    }

    /// The router only applies to the managed lane: an explicit
    /// `-Ztrust-witness=replay:<dir>` replays every stored root.
    pub fn trust_witness_replay_router_skip(self, def_id: LocalDefId) -> bool {
        self.trust_witness_lane().is_auto() && !self.trust_witness_body_big_enough(def_id)
    }

    fn trust_witness_riders_ok(self) -> bool {
        let opts = &self.sess.opts;
        opts.incremental.is_none() && opts.pretty.is_none() && !opts.unstable_opts.validate_mir
    }

    /// Trust (typeck-moonshot P1): record a mintable typeck root (from the
    /// `typeck_root` provider wrapper). Idempotent; only under the mint flag.
    pub fn trust_witness_record_mint(self, def_id: LocalDefId) {
        self.trust_witness_minted.lock().insert(def_id);
    }

    /// Trust (typeck-moonshot P1): drain the recorded mintable roots for the
    /// analysis-seam crate-store commit.
    pub fn trust_witness_drain_minted(self) -> Vec<LocalDefId> {
        self.trust_witness_minted.lock().drain(..).collect()
    }

    /// Trust (P1 follow-on): stash the ACCEPTED replay check-THIR so `thir_body`
    /// reuses it (single THIR build on the replay path). Checker-accept branch only.
    pub fn trust_witness_thir_install(
        self,
        def_id: LocalDefId,
        thir: crate::thir::Thir<'tcx>,
        expr: crate::thir::ExprId,
    ) {
        self.trust_witness_thir_lookaside.lock().insert(def_id, (thir, expr));
    }

    /// Trust (P1 follow-on): take the stashed check-THIR for `def_id`, if any.
    pub fn trust_witness_thir_take(
        self,
        def_id: LocalDefId,
    ) -> Option<(crate::thir::Thir<'tcx>, crate::thir::ExprId)> {
        self.trust_witness_thir_lookaside.lock().remove(&def_id)
    }

    /// Trust (P1 follow-on): reclaim any un-taken check-THIR entries; returns the count.
    pub fn trust_witness_drain_thir_lookaside(self) -> usize {
        let mut m = self.trust_witness_thir_lookaside.lock();
        let n = m.len();
        m.clear();
        n
    }
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    // Trust: rust-lang#100914 — accessor for the `large_stack_frame` lint limit.
    pub fn stack_frame_limit(self) -> Limit {
        self.limits(()).stack_frame_limit
    }
