...
pub(crate) fn target_reserves_x7(target_features: &FxIndexSet<Symbol>) -> bool {
    target_features.contains(&sym::reserve_x7)
}
...
fn not_reserved_x7(
    ...
    if target_reserves_x7(target_features) {
        Err("x7 is a reserved register on this target")
    } else {
        Ok(())
    }
}
