...
impl TargetTuple {
    ...
    /// Exact custom-target contents captured by [`TargetTuple::from_path`].
    /// Consumers that authenticate target semantics must hash these bytes,
    /// rather than reopening the mutable path and introducing a TOCTOU gap.
    pub fn json_contents(&self) -> Option<&str> {
        match self {
            TargetTuple::TargetTuple(_) => None,
            TargetTuple::TargetJson { contents, .. } => Some(contents),
        }
    }
