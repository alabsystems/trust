        ...
        if fn_abi.ret.layout.is_aggregate() && fn_abi.ret.layout.is_sized() {
            ...
            if t.abi_return_struct_as_int || opts.reg_struct_return {
                ...
                if fn_abi.ret.layout.is_single_fp_element(cx) {
                    ...
                    // Trust (rust-lang#102174 / tRust#921): Clang's i386 SysV ABI
                    // returns EVERY struct <= 8 bytes in an integer register, rounding
                    // the size up to the next register width (3 -> i32; 5/6/7 -> i64),
                    // not just the exact power-of-two sizes. Upstream fell through to
                    // `make_indirect()` (sret) for 3/5/6/7-byte structs, diverging from
                    // the platform C ABI; round up so trustc matches Clang.
                    match fn_abi.ret.layout.size.bytes() {
                        1 => fn_abi.ret.cast_to(Reg::i8()),
                        2 => fn_abi.ret.cast_to(Reg::i16()),
                        3..=4 => fn_abi.ret.cast_to(Reg::i32()),
                        5..=8 => fn_abi.ret.cast_to(Reg::i64()),
                        _ => fn_abi.ret.make_indirect(),
                    }
                }
            } else {
