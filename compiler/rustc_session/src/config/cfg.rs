...
pub(crate) fn disallow_cfgs(sess: &Session, user_cfgs: &Cfg) {
    ...
    for cfg in user_cfgs {
        match cfg {
            ...
            // Trust: `cfg(trust_verify)` tracks whether the verifier is
            // active; a user `--cfg trust_verify` under `-Z trust-verify=off`
            // would activate Trust-only attributes with the verifier off.
            (sym::trust_verify, None) => disallow(cfg, "-Z trust-verify=off"),
            _ => {}
        }
    }
}
...
pub(crate) fn default_configuration(sess: &Session) -> Cfg {
    ...
    // Trust: expose `cfg(trust_verify)` whenever the verification pass is active,
    // so a crate can apply Trust-only tool attributes —
    // `#[cfg_attr(trust_verify, trust::backing)]`, `#[cfg_attr(trust_verify,
    // trust::single_writer)]`; explicit vanilla/bootstrap builds omit it via
    // `-Ztrust-verify=off`.
    // The shared Session predicate is also used by the MIR pass and proof-only
    // type-system hooks, so the cfg cannot drift into a state where attributes
    // are present but their verifier consumer is absent (or vice versa).
    if sess.trust_verification_enabled() {
        ins_none!(sym::trust_verify);
    }
...
impl CheckCfg {
    ...
    pub fn fill_well_known(&mut self, current_target: &Target) {
        ...
        // Trust: `cfg(trust_verify)` is injected by `default_configuration`
        // whenever verification is active (the default); without this entry the
        // documented `#[cfg_attr(trust_verify, trust::…)]` pattern trips the
        // `unexpected_cfgs` lint under cargo's default `--check-cfg`.
        ins!(sym::trust_verify, no_values);
