...
pub fn validate_crate_name(sess: &Session, crate_name: Symbol, span: Option<Span>) {
    ...
    for c in invalid_crate_name_chars(crate_name.as_str()) {
...
/// Return the characters rejected by the crate-name grammar used by
/// [`validate_crate_name`]. Pre-Session compiler boundaries use this to reject
/// unsafe artifact identities without duplicating the grammar.
pub fn invalid_crate_name_chars(crate_name: &str) -> impl Iterator<Item = char> + '_ {
    crate_name.chars().filter(|&c| !c.is_alphanumeric() && c != '_')
}
