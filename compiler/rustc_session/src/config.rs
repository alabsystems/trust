...
use rustc_span::{
    FileName, RealFileName, RemapPathScopeComponents, SourceFileHash, SourceFileHashAlgorithm,
    Symbol, sym,
};
...
/// Trust: whether compiler-integrated verification runs.
///
/// Spelled as a two-valued domain rather than an off-switch boolean because the
/// batteries-on answer must be the one a reader sees without composing a
/// negation: `-Ztrust-verify=off` says what it does, while every caller of a
/// `no-`-prefixed switch has to write the double negative to say "on".
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
pub enum TrustVerify {
    #[default]
    On,
    Off,
}
...
/// Trust: how far the direct source(THIR) → trust-ir frontend goes.
///
/// `Off`/`On` are the historical two states this option carried as an
/// `Option<bool>`. `Strict` adds the answer that was missing: a body the
/// producer cannot fully lower is a compile ERROR rather than a coverage-only
/// skip that quietly ships built MIR. Without it, producer coverage cannot be
/// ratcheted — a regression and a shape nobody has implemented yet are the same
/// observation.
///
/// One option with three values rather than a bool plus a `-Ztrust-ir-strict`,
/// for the reason recorded on `trust_policy`: three mutually exclusive answers
/// to one question, encoded as separate booleans, are an enum with an invalid
/// state, and only a hand-written cross-check keeps them honest.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum TrustIrLower {
    Off,
    On,
    Strict,
}
...
impl TrustIrLower {
    /// Does the frontend run at all? `Strict` implies `On`.
    pub const fn runs(self) -> bool {
        matches!(self, Self::On | Self::Strict)
    }
...
/// Trust: how strictly an active verification run enforces its own verdicts.
///
/// The three settings are mutually exclusive by construction. Encoding them as
/// independent booleans made an invalid pair representable, so the domain check
/// that rejected it had to be maintained by hand and could only run after the
/// options were already built.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
pub enum TrustPolicy {
    /// The default. Fail-closed enforcement is ON — a refutation, and any
    /// unproved obligation with NO runtime fallback, still fails the build and a
    /// forced counterexample still escalates.
    ///
    /// What it does NOT do is fail on a completeness gap that rustc itself still
    /// checks at runtime. Such a row is bit-for-bit vanilla Rust semantics, so
    /// rejecting it buys no safety and costs drop-in. Ruled by Andrew 2026-07-25
    /// ("B for now so that we can dogfood, but eventually A"); rationale and the
    /// exit criterion for returning to full strictness live in
    /// `docs/design/2026-07-25-completeness-gap-policy-ruling.md`.
    ///
    /// Fail-CLOSED remains a property of the proof LABEL — an honest UNKNOWN is
    /// never relabelled `Proved` — which is what `DESIGN_PHILOSOPHY.md` requires;
    /// only the exit code for that label changes.
    #[default]
    Strict,
    /// Full static discharge: EVERY unproved obligation fails, including one
    /// whose operation keeps a runtime check. This is the release gate
    /// (`targo trust certify`), not a development default.
    Certify,
    /// Nonfatal coverage reporting: every obligation is still verified and
    /// reported, but neither fail-closed enforcement nor forced-counterexample
    /// escalation applies. Selected by `targo trust --allow-l0-gaps`.
    Advisory,
    /// Narrow demotion: reachable Rust panic refutations and supported
    /// capability gaps become explicit assumption rows, but only for functions
    /// containing no source or inlined `unsafe`. UB, hardened, contract,
    /// functional, and unmarked failures stay strict.
    MemorySafe,
}

impl TrustPolicy {
    /// Whether fail-closed enforcement is active at all.
    ///
    /// True for both `Strict` (the default) and `Certify`: they differ only in
    /// whether a completeness gap WITH a runtime fallback is fatal, not in
    /// whether refutations and no-fallback gaps are.
    pub const fn is_strict(self) -> bool {
        matches!(self, Self::Strict | Self::Certify)
    }

    /// Whether every unproved obligation must fail, including one that keeps a
    /// runtime check. Only `targo trust certify` selects this.
    pub const fn requires_full_discharge(self) -> bool {
        matches!(self, Self::Certify)
    }

    /// Whether proof dispatch may be skipped entirely (the `mir-only` dump).
    pub const fn is_advisory(self) -> bool {
        matches!(self, Self::Advisory)
    }

    pub const fn is_memory_safe(self) -> bool {
        matches!(self, Self::MemorySafe)
    }
}

/// Trust: which typeck-witness lane runs, and against which store.
///
/// Lane and store are one decision: a store directory is meaningless without a
/// lane, and the managed lane owns its own store. Separate `auto`/`mint`/
/// `replay` switches let a caller name a mint directory while the managed lane
/// silently replayed from a different one.
#[derive(Clone, Debug, Default, PartialEq, Eq, Hash)]
pub enum TrustWitness {
    /// Managed: mint and replay both run against `<out-dir>/trust-witness`,
    /// router-gated to bodies where replay beats fresh typeck. Inert without an
    /// `--out-dir`.
    #[default]
    Auto,
    Off,
    /// Mint every eligible root into this store; replay does not run.
    Mint(PathBuf),
    /// Replay every stored root from this store; mint does not run.
    Replay(PathBuf),
}

impl TrustWitness {
    /// The store a caller named explicitly, if any. `None` in the managed and
    /// disabled lanes, where the store is derived or absent.
    pub fn explicit_store(&self) -> Option<&PathBuf> {
        match self {
            Self::Mint(dir) | Self::Replay(dir) => Some(dir),
    ...
    pub const fn mints(&self) -> bool {
        matches!(self, Self::Auto | Self::Mint(_))
    }

    pub const fn replays(&self) -> bool {
        matches!(self, Self::Auto | Self::Replay(_))
    }
}

/// Trust: the closed set of verification artifact sinks `-Ztrust-dump` can
/// publish. One option owns all of them because a dump request is one decision
/// ("publish this artifact here"), and because the `mir-only` mode is
/// meaningless without the MIR sink it truncates — separate options let the two
/// drift into an invalid pair that only a later cross-check could reject.
#[derive(Clone, Debug, Default, PartialEq, Eq, Hash)]
pub struct TrustDump {
    /// `mir:<dir>` / `mir-only:<dir>` — each extracted per-function MIR
    /// verification input.
    pub mir: Option<PathBuf>,
    /// `mir-only:<dir>` — publish the extracted inputs and stop, without solver
    /// or certifier dispatch.
    pub mir_only: bool,
    /// `native-bundle:<dir>` — each finalized native TrustIr verification bundle.
    pub native_bundle: Option<PathBuf>,
    /// `ir:<dir>` — the assembled crate-level TrustIr lowering artifacts.
    pub ir: Option<PathBuf>,
}

impl TrustDump {
    /// The `ir:` sink is the only one that publishes under a crate-name-derived
    /// lease acquired before typed options exist, so the raw argv guard and the
    /// typed path agree on one spelling.
    pub const IR_REQUEST_PREFIX: &'static str = "ir:";

    /// Every configured sink directory, for the shared non-empty-path check.
    pub fn paths(&self) -> impl Iterator<Item = &PathBuf> {
        [self.mir.as_ref(), self.native_bundle.as_ref(), self.ir.as_ref()].into_iter().flatten()
    }
}
...
define_output_types! {
    ...
    #[doc = "Trust: the crate-level trust-ir `Module` (binary codec); the canonical-text"]
    #[doc = "and coverage.json companions are written alongside the .bin. This lane is the"]
    #[doc = "direct THIR producer, whose declared capability is structural parity only —"]
    #[doc = "the module carries executable IR and the coverage record of what could not be"]
    #[doc = "lowered, never obligations or proof certificates. An empty proof inventory in"]
    #[doc = "the artifact means `not wired`, never `verified`."]
    #[doc = "An analysis-only artifact (like `metadata`): requesting it alone stops the"]
    #[doc = "pipeline before codegen, and it never affects any other emit kind."]
    TrustIr => {
        shorthand: "trust-ir",
        extension: "trust-ir.bin",
        description: "Generates a file containing the crate's trust-ir module (binary codec)",
        default_filename: "CRATE_NAME.trust-ir.bin",
        is_text: false,
        compatible_with_cgus_and_single_output: true
    },
}
...
impl OutputTypes {
    ...
    /// Returns `true` when trust-ir is the only requested primary artifact.
    ///
    /// Dependency information is a side artifact and does not make this a mixed
    /// output lane.
    pub fn trust_ir_only(&self) -> bool {
        self.0.contains_key(&OutputType::TrustIr)
            && self.0.keys().all(|k| matches!(k, OutputType::TrustIr | OutputType::DepInfo))
    }
    ...
    pub fn should_codegen(&self) -> bool {
        self.0.keys().any(|k| match *k {
            ...
            | OutputType::Object
            | OutputType::Exe => true,
            // Trust-ir is assembled during analysis and never requires codegen.
            OutputType::Metadata | OutputType::DepInfo | OutputType::TrustIr => false,
        })
    }
    ...
    pub fn should_link(&self) -> bool {
        self.0.keys().any(|k| match *k {
            ...
            | OutputType::Object
            | OutputType::DepInfo
            // Trust-ir is an analysis artifact and is never linked.
            | OutputType::TrustIr => false,
            OutputType::Exe => true,
        })
    }
}
...
#[cfg(test)]
#[path = "config/tests/trust_output_types.rs"]
mod trust_output_types_tests;
...
impl Default for Options {
    fn default() -> Options {
        ...
        Options {
            ...
            trust_solver_content_fingerprint: String::new(),
            unstable_opts,
            prints: Vec::new(),
            cg: Default::default(),
...
impl UnstableOptions {
    pub fn dcx_flags(&self, can_emit_warnings: bool) -> DiagCtxtFlags {
        DiagCtxtFlags {
            ...
            // Trust: under `-Z ui-testing` the compiler runs Trust's blessed test
            // corpus, whose expectations the release porter rewrites to the
            // canonical Trust name (`trustc --explain`, adapter rule
            // `explain_tool_name` in trust-upstream-compat). Emit that canonical
            // `trustc` there regardless of the invoked alias, so a test run driven
            // through the `rustc` drop-in symlink (file_stem `rustc` ⇒ otherwise
            // `rustc --explain`) still matches the trustc-blessed .stderr. Real,
            // non-ui-testing invocations keep the drop-in, invocation-derived name.
            explain_binary: if self.ui_testing { "trustc" } else { invoked_explain_binary() },
        }
    }
...
fn invoked_explain_binary() -> &'static str {
    let invoked_name = std::env::args_os()
        .next()
        .and_then(|arg| std::path::Path::new(&arg).file_stem().map(|name| name.to_owned()))
        .and_then(|name| name.into_string().ok());

    explain_binary_for_invoked_name(invoked_name.as_deref())
}

fn explain_binary_for_invoked_name(invoked_name: Option<&str>) -> &'static str {
    match invoked_name {
        // rustdoc itself has no --explain option; diagnostics must direct users
        // to the paired compiler while preserving the selected toolchain alias.
        Some("rustc" | "rustdoc") => "rustc",
        Some("trustc" | "trustdoc") => "trustc",
        _ => "trustc",
    }
}

#[cfg(test)]
#[path = "config/tests/explain_binary.rs"]
mod trust_explain_binary_tests;
...
// Trust frontend authority is one inventory, and the option table is where it
// lives (`TRUST_AUTHORITY_CONTROLS`). Public compiler embedding APIs use it to
// remove official-evidence authority; the canonical driver snapshots it before
// invoking callbacks. Reading both from the declaration removes the failure
// mode this type used to have: a `-Ztrust-*` option protected at one boundary,
// forgotten at the other, and noticed only by a test nobody had run.
#[derive(Clone)]
pub struct TrustFrontendAuthoritySnapshot {
    trust_solver_content_fingerprint: String,
    unstable_opts: Box<UnstableOptions>,
}

impl TrustFrontendAuthoritySnapshot {
    #[allow(rustc::bad_opt_access)]
    pub fn capture(opts: &Options) -> Self {
        Self {
            trust_solver_content_fingerprint: opts.trust_solver_content_fingerprint.clone(),
            unstable_opts: Box::new(opts.unstable_opts.clone()),
        }
    }

    /// The first authority-bearing control a callback changed, if any.
    #[allow(rustc::bad_opt_access)]
    pub fn changed_option(&self, opts: &Options) -> Option<String> {
        if self.trust_solver_content_fingerprint != opts.trust_solver_content_fingerprint {
            return Some(COMPILER_POPULATED_SOLVER_IDENTITY.to_string());
        }
        self.unstable_opts.first_changed_trust_authority(&opts.unstable_opts)
    }

    /// Return the first caller-populated Trust control that a public embedded
    /// frontend cannot safely inherit. Authority-only controls are excluded:
    /// either incoming value is overwritten by [`Self::deauthorize`].
    #[allow(rustc::bad_opt_access)]
    pub fn first_nondefault_evidence_control(opts: &Options) -> Option<String> {
        let defaults = Options::default();
        if opts.trust_solver_content_fingerprint != defaults.trust_solver_content_fingerprint {
            return Some(COMPILER_POPULATED_SOLVER_IDENTITY.to_string());
        }
        // `-Ztrust-ir-lower` is a request, not a state, and the driver stamps
        // its resolved answer into the field before an embedded frontend ever
        // sees it. Both "unset" and "explicitly off" therefore arrive here as
        // configurations deauthorization can inherit unchanged; only a caller
        // that asked for lowering has populated evidence authority. Comparing
        // against the declared default instead would reject every canonicalized
        // compile, which is why it is classified `AuthorityOnly` in the table
        // and carries this one explicit rejection.
        if opts.unstable_opts.trust_ir_lower.is_some_and(TrustIrLower::runs) {
            return Some("-Ztrust-ir-lower".to_string());
        }
        opts.unstable_opts.first_nondefault_trust_evidence_control()
    }

    /// Remove integrated Trust verification, direct TrustIR lowering, and any
    /// compiler-populated solver identity from an embedded frontend
    /// configuration.
    #[allow(rustc::bad_opt_access)]
    pub fn deauthorize(opts: &mut Options) {
        opts.unstable_opts.trust_verify = TrustVerify::Off;
        // Record the resolved answer, not an absent request: with verification
        // forced off, an unset request would still read as "the toolchain
        // decides" to anything that inspects the field later.
        opts.unstable_opts.trust_ir_lower = Some(TrustIrLower::Off);
        opts.trust_solver_content_fingerprint.clear();
    }
}

pub(crate) const COMPILER_POPULATED_SOLVER_IDENTITY: &str =
    "compiler-populated Trust solver content fingerprint";

/// Classifies compiler entry points by whether they can initialize or reuse a
/// process-global codegen backend.
///
/// This marker covers compiler-owned entry points only. Arbitrary host code
/// that calls a backend or LLVM API directly remains outside this bookkeeping
/// boundary and cannot be made evidence-inert by this guard.
#[doc(hidden)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CompilerBackendRoute {
    BackendCapable,
    EvidenceInert,
}

const BACKEND_AFTER_EVIDENCE_INERT: &str = "backend-capable compiler frontend cannot enter after a NoTrustEvidence compiler invocation in this process; backend, LLVM, and proc-macro state are process-global";
const EVIDENCE_INERT_AFTER_BACKEND: &str = "NoTrustEvidence compiler frontend cannot enter after a backend-capable compiler invocation in this process; LLVM/backend state is process-global";
const EVIDENCE_INERT_PROCESS_REUSE: &str = "NoTrustEvidence compiler frontend cannot enter more than once in a process; backend, LLVM, and proc-macro state may persist after the invocation";

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
enum CompilerBackendRouteState {
    #[default]
    Pristine,
    BackendCapableSeen,
    EvidenceInertDedicated,
}

impl CompilerBackendRouteState {
    fn enter(&mut self, route: CompilerBackendRoute) -> Result<(), &'static str> {
        match (*self, route) {
            (Self::Pristine, CompilerBackendRoute::BackendCapable) => {
                *self = Self::BackendCapableSeen;
                Ok(())
            }
            (Self::BackendCapableSeen, CompilerBackendRoute::BackendCapable) => Ok(()),
            (Self::EvidenceInertDedicated, CompilerBackendRoute::BackendCapable) => {
                Err(BACKEND_AFTER_EVIDENCE_INERT)
            }
            (Self::Pristine, CompilerBackendRoute::EvidenceInert) => {
                // This is deliberately permanent: backend OnceLocks, loaded
                // plugins, and proc-macro dylibs cannot be reliably unloaded.
                *self = Self::EvidenceInertDedicated;
                Ok(())
            }
            (Self::BackendCapableSeen, CompilerBackendRoute::EvidenceInert) => {
                Err(EVIDENCE_INERT_AFTER_BACKEND)
            }
            (Self::EvidenceInertDedicated, CompilerBackendRoute::EvidenceInert) => {
...
static COMPILER_BACKEND_ROUTE_STATE: Mutex<CompilerBackendRouteState> =
    Mutex::new(CompilerBackendRouteState::Pristine);

/// Proof that a compiler-owned route performed its process-global transition.
/// Strict driver-to-interface handoff borrows this token so recursive public
/// calls cannot masquerade as implementation nesting.
#[doc(hidden)]
#[must_use = "the route guard must span any structural compiler handoff"]
pub struct CompilerBackendRouteGuard {
    route: CompilerBackendRoute,
    // Structural handoff is local to the invoking thread.
    _not_send_or_sync: PhantomData<Rc<()>>,
}

impl CompilerBackendRouteGuard {
    /// Whether this token authorizes the named structural route handoff.
    #[doc(hidden)]
    pub fn validates(&self, route: CompilerBackendRoute) -> bool {
        self.route == route
    }
}

fn enter_compiler_backend_route_with_state(
    route: CompilerBackendRoute,
    state: &'static Mutex<CompilerBackendRouteState>,
) -> Result<CompilerBackendRouteGuard, &'static str> {
    state.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).enter(route)?;

    Ok(CompilerBackendRouteGuard { route, _not_send_or_sync: PhantomData })
}

/// Enter a compiler-owned backend route.
///
/// Evidence-inert routes fail closed after any backend-capable compiler route
/// has entered this process. Backend-capable routes likewise cannot overlap an
/// evidence-inert process. Repeated ordinary embedded/canonical routes remain
/// supported, but an evidence-inert outer invocation permanently dedicates a
/// pristine process and cannot be repeated there.
#[doc(hidden)]
pub fn enter_compiler_backend_route(
    route: CompilerBackendRoute,
) -> Result<CompilerBackendRouteGuard, &'static str> {
    enter_compiler_backend_route_with_state(route, &COMPILER_BACKEND_ROUTE_STATE)
}

/// Legacy ambient inputs that used to alter Trust proof obligations, solver
/// routing, verdicts, or generated MIR without participating in dependency
/// tracking. Diagnostic-only switches intentionally remain outside this list.
pub const LEGACY_TRUST_SEMANTIC_ENV_VARS: &[&str] = &[
    ...
    "TRUST_DISABLE_TV",
    "TRUST_DIVREM_PERTURB",
    "TRUST_DUMP_MIR",
    "TRUST_DUMP_NATIVE_BUNDLE",
    "TRUST_DUMP_ONLY",
    "TRUST_ELIDE_CERTIFIED_CHECKS",
    "TRUST_ENABLED",
    "TRUST_ENUM_CTOR_PERTURB",
    "TRUST_ENUM_PAYLOAD_PERTURB",
    "TRUST_ENUM_RESHAPE_PERTURB",
    "TRUST_FLOAT_FLIP_PERTURB",
    "TRUST_HARDENED",
    ...
    "TRUST_RECHECK_FRONTIER_OVERRIDE",
    "TRUST_REFREAD_PERTURB",
    "TRUST_RESULT_CACHE_CAP",
    ...
    "TRUST_STR_RETURN_PERTURB",
    "TRUST_SWITCH_PERTURB",
...
pub const UNTRACKED_TRUST_CODEGEN_ENV_PREFIXES: &[&str] = &["TCG_", "TRUST_CG_"];

fn environment_name_eq(key: &str, expected: &str, windows_semantics: bool) -> bool {
    if windows_semantics { key.eq_ignore_ascii_case(expected) } else { key == expected }
}

fn environment_name_starts_with(key: &str, prefix: &str, windows_semantics: bool) -> bool {
    if windows_semantics {
        key.get(..prefix.len()).is_some_and(|candidate| candidate.eq_ignore_ascii_case(prefix))
    } else {
        key.starts_with(prefix)
    }
}

fn is_untracked_trust_codegen_env_with_windows_semantics(
    key: &OsStr,
    windows_semantics: bool,
) -> bool {
    key.to_str().is_some_and(|key| {
        UNTRACKED_TRUST_CODEGEN_ENV_PREFIXES
            .iter()
            .any(|prefix| environment_name_starts_with(key, prefix, windows_semantics))
    })
}

pub fn is_untracked_trust_codegen_env(key: &OsStr) -> bool {
    is_untracked_trust_codegen_env_with_windows_semantics(key, cfg!(windows))
}

fn is_legacy_trust_semantic_env_with_windows_semantics(
    key: &OsStr,
    windows_semantics: bool,
) -> bool {
    key.to_str().is_some_and(|key| {
        LEGACY_TRUST_SEMANTIC_ENV_VARS
            .iter()
            .any(|expected| environment_name_eq(key, expected, windows_semantics))
    })
}

pub fn forbidden_no_official_evidence_environment_key(
    keys: impl IntoIterator<Item = OsString>,
) -> Option<OsString> {
    keys.into_iter().find(|key| is_untracked_trust_codegen_env(key))
}

#[allow(rustc::bad_opt_access)]
pub fn forbidden_trust_environment_key(
    opts: &Options,
    keys: impl IntoIterator<Item = OsString>,
) -> Option<OsString> {
    let reject_legacy_semantics = crate::trust_ir_lower_enabled_by_flags(
        opts.unstable_opts.trust_verify,
        opts.unstable_opts.trust_ir_lower,
    );
    keys.into_iter().find(|key| {
        is_untracked_trust_codegen_env(key)
            || (reject_legacy_semantics
                && is_legacy_trust_semantic_env_with_windows_semantics(key, cfg!(windows)))
    })
}

pub fn trust_environment_error(key: &OsStr) -> String {
    format!(
        "environment variable `{}` is an untracked Trust semantic/codegen control; remove it and use the corresponding tracked compiler option",
        key.to_string_lossy(),
    )
}

/// Return the first LLVM control that can load code or alter the linked LLVM
/// backend outside an evidence-inert frontend's fixed backend route.
#[allow(rustc::bad_opt_access)]
pub fn evidence_inert_llvm_control(opts: &Options) -> Option<&'static str> {
    if !opts.unstable_opts.llvm_plugins.is_empty() {
        Some("-Zllvm-plugins")
    } else if !opts.unstable_opts.autodiff.is_empty() {
        Some("-Zautodiff")
    } else if opts.unstable_opts.autodiff_post_passes.is_some() {
        Some("-Zautodiff-post-passes")
    } else if !opts.cg.llvm_args.is_empty() {
        Some("-Cllvm-args")
    } else {
        None
    }
...
/// Reject LLVM arguments supplied by a custom target while retaining the
/// compiler-owned arguments of exact built-in targets.
pub fn evidence_inert_target_llvm_args_error(
    target: &TargetTuple,
    has_llvm_args: bool,
) -> Option<String> {
    let exact_builtin = target.json_contents().is_none()
        && rustc_target::spec::TARGETS.iter().any(|builtin| *builtin == target.tuple());
    (has_llvm_args && !exact_builtin).then(|| {
        format!(
            "NoTrustEvidence compiler frontend rejects LLVM arguments from custom target `{}`: backend plugin/loading controls must come only from the linked compiler's exact built-in target table",
            target.tuple(),
        )
    })
}
...
#[cfg(test)]
#[path = "config/tests/frontend_authority.rs"]
mod trust_frontend_authority_tests;

pub fn trust_target_spec_binding_error(
    target: &TargetTuple,
    expected_sha256: Option<&str>,
    authenticated_session: bool,
) -> Option<String> {
    let Some(contents) = target.json_contents() else {
        if expected_sha256.is_some() {
            return Some(
                "-Ztrust-verify-target-spec-sha256 requires an explicit custom JSON --target"
                    .to_string(),
            );
        }
        if authenticated_session
            && !rustc_target::spec::TARGETS.iter().any(|builtin| *builtin == target.tuple())
        {
            // A non-built-in tuple is not necessarily a target triple. Target::search
            // can resolve it through RUST_TARGET_PATH or the sysroot's
            // lib/rustlib/<name>/target.json fallback. That search happens after this
            // boundary and only rustc observes which file wins, so Cargo's proof
            // inventory cannot bind either its origin or its bytes. Fail before the
            // search rather than treating a named custom target as a digest-free
            // built-in target.
            return Some(format!(
                "authenticated Trust verification rejects named non-built-in target `{}`: RUST_TARGET_PATH and sysroot custom-target fallback are outside the proof identity; pass an explicit .json --target so Cargo can bind its exact contents",
                target.tuple()
            ));
        }
        return None;
    ...
    let Some(expected) = expected_sha256 else {
        return authenticated_session.then(|| {
            "an authenticated -Ztrust-verify-session with a custom JSON target requires Cargo's exact -Ztrust-verify-target-spec-sha256 content binding"
                .to_string()
        });
    };
    let observed = SourceFileHash::new_in_memory(SourceFileHashAlgorithm::Sha256, contents)
        .to_string()
        .strip_prefix("sha256=")
        .expect("SHA-256 SourceFileHash display has its algorithm prefix")
        .to_string();
    (observed != expected).then(|| {
        format!(
            "custom JSON target bytes parsed by rustc do not match Cargo's authenticated SHA-256 binding (expected={expected}, observed={observed})"
        )
    })
}

#[cfg(test)]
#[path = "config/tests/target_spec_binding.rs"]
mod trust_target_spec_binding_tests;
...
pub fn build_session_options(early_dcx: &mut EarlyDiagCtxt, matches: &getopts::Matches) -> Options {
    ...
    if let Some(error) = trust_target_spec_binding_error(
        &target_triple,
        unstable_opts.trust_verify_target_spec_sha256.as_deref(),
        unstable_opts.trust_verify_session.is_some(),
    ) {
...
pub mod nightly_options {
    ...
    pub fn check_nightly_options(
        ...
        for opt in flags.iter() {
            ...
            match opt.stability {
                OptionStability::Unstable => {
                    if opt.name == "Z" && z_options_are_stable_trust_compat(matches) {
                        continue;
                    }
        ...
        if nightly_options_on_stable > 0 {
            early_dcx
                .early_help("consider switching to the Trust toolchain: `rustup default trust`");
            ...
            early_dcx.early_note("Trust keeps the inherited unstable option model; use the Trust toolchain for `-Z` workflows.");
            early_dcx.early_fatal(format!(
    ...
    fn z_options_are_stable_trust_compat(matches: &getopts::Matches) -> bool {
        let z_options = matches.opt_strs("Z");
        stable_trust_compat_z_options(&z_options)
    }

    fn stable_trust_compat_z_options(z_options: &[String]) -> bool {
        !z_options.is_empty()
            && z_options.iter().all(|option| {
...
pub(crate) mod dep_tracking {
    ...
    use super::{
        ...
        SymbolManglingVersion, TrustDump, TrustIrLower, TrustPolicy, TrustVerify, TrustWitness,
        WasiExecModel,
    };
    use crate::lint;
