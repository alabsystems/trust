...
symbols! {
    ...
    Keywords {
        ...
        Reuse:              "reuse",
        Safe:               "safe",
        // Trust: internal clause keywords injected when a `trust::requires` /
        // `trust::ensures` attribute (the trust-spec surface) is expanded, so
        // Trust-origin contract clauses stay distinguishable from upstream
        // `core::contracts` clauses (`contract_requires` / `contract_ensures`)
        // all the way into the parser. Trust-origin payloads may use spec-only
        // compatibility spec syntax (`result`, `old()`, bounded quantifiers,
        // `==>`) that must never reach
        // name resolution or typeck.
        TrustContractEnsures: "trust_contract_ensures",
        TrustContractRequires: "trust_contract_requires",
        Union:              "union",
        Yeet:               "yeet",
        // tidy-alphabetical-end
    }
    ...
    Symbols {
        ...
        builtin_syntax,
        bundle,
        // Trust: the `by <thm>` citation suffix on contract clauses
        // (two-language design E9) - contextual, never reserved.
        by,
        c_dash_variadic,
        c_str_literals,
        c_unwind,
        ...
        cfi_encoding,
        char,
        // Trust: the `clean { … }` parser-island keyword (two-language design
        // E10) — contextual, never reserved.
        clean,
        ...
        declare_lint_pass,
        decorated,
        // Trust: first-class loop clause keyword (two-language design E5).
        decreases,
        ...
        exhaustive_patterns,
        existential_type,
        // Trust: spec-only quantifier in trust-spec contract clauses.
        exists,
        exp2f16,
        exp2f32,
        exp2f64,
        ...
        intra_doc_pointers,
        intrinsics,
        // Trust: first-class loop clause keyword (two-language design E4).
        invariant,
        ...
        offset_of_slice,
        ok_or_else,
        // Trust: spec-only pre-state operator in trust-spec contract clauses.
        old,
        old_name,
        ...
        truncf128,
        trust,
        trust_certified_monitor_check,
        // Trust: builtin attr macros the resolver substitutes for the
        // trust-spec `trust::requires` / `trust::ensures` proc-macro
        // passthroughs, keeping Trust-origin contract clauses separate from
        // upstream `contracts_requires` / `contracts_ensures`.
        trust_contracts_ensures,
        trust_contracts_requires,
        // Trust: rustc-private extractor contract-conversion fixtures.
        trust_test_contract_not_lowered: "not_lowered",
        trust_test_contract_x_gt_zero: "x > 0",
        trust_verify,
        try_blocks,
