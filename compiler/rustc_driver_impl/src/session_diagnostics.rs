...
#[derive(Diagnostic)]
#[diag(
    ".rlink file was produced by Trust compiler version `{$rustc_version}`, but the current version is `{$current_version}`"
)]
...
#[derive(Diagnostic)]
#[diag("please make sure that you have updated to the latest Trust toolchain snapshot")]
...
#[derive(Diagnostic)]
#[diag("{$compiler} {$version} running on {$triple}")]
