...
unsafe extern "C" fn print_stack_trace(signum: libc::c_int) {
    ...
    raw_errln!("error: trustc interrupted by {signame}, printing backtrace\n");
    ...
    if (cyclic || stack.len() > random_depth()) && signum == libc::SIGSEGV {
        ...
        // trustc only arrived in this signal handler because bad things happened
        // and this message is for explaining it's not the programmer's fault
        raw_errln!("note: trustc unexpectedly overflowed its stack! this is a bug");
        written += 1;
    }
    ...
    raw_errln!(
        "note: we would appreciate a report at https://github.com/alabsystems/Trust/issues"
    );
    written += 1;
    if signum == libc::SIGSEGV {
        ...
        raw_errln!(
            "help: you can increase trustc's stack size by setting RUST_MIN_STACK={new_size}"
        );
        written += 1;
    }
    if written > 24 {
