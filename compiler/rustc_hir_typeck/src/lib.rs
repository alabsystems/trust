...
fn typeck_root<'tcx>(tcx: TyCtxt<'tcx>, def_id: LocalDefId) -> &'tcx ty::TypeckResults<'tcx> {
    // Trust (typeck-moonshot P1, Phase 3): CHECKED witness-replay. Only fires for
    // a typeck ROOT under `-Z trust-witness=replay:<dir>`; the hook decodes,
    // re-interns, builds a throwaway check-THIR, and runs the MANDATORY linear
    // checker. `Some` only on an accepted check; any miss / decode / reintern /
    // check failure returns `None`, so we fall through to real typeck (FAIL-SAFE,
    // CHECKED not TRUSTED). Root-guarded so children re-demand `tcx.typeck(root)`
    // and hit the replayed root, exactly like `typeck_with_inspect`'s child
    // redirect. Inert (branch skipped) unless `-Z trust-witness=replay:<dir>` is set.
    // In PRECISE mode the replay is never trusted directly (the precise edit-loop
    // key is unsound without the full A1-A3 forest-replay) — it is only measured
    // against real typeck below. So skip the trust path when precise.
    if tcx.trust_witness_replay_active()
        && !tcx.trust_witness_precise()
        && tcx.typeck_root_def_id_local(def_id) == def_id
    {
        if let Some(r) = tcx.trust_witness_try_replay(def_id) {
            return r;
        }
    }
    // Trust witness minting may skip this provider on replay, so a root that
    // emitted a typeck warning or fulfilled a lint expectation is not
    // replayable unless those side effects are serialized too. Snapshot the
    // monotonic diagnostic epoch around cold typeck. Parallel unrelated
    // diagnostics can conservatively exclude extra roots, never admit one
    // that lost output.
    let diagnostic_epoch_before = (tcx.trust_witness_mint_active()
        && tcx.typeck_root_def_id_local(def_id) == def_id)
        .then(|| tcx.dcx().diagnostic_side_effect_epoch());
    let r = typeck_with_inspect(tcx, def_id, None);
    let typeck_was_diagnostic_free = diagnostic_epoch_before
        .is_some_and(|before| tcx.dcx().diagnostic_side_effect_epoch() == before);
    // Trust (typeck-moonshot P1, Phase 0): record ROOTS for the witness mint
    // lane. This crate takes no `trust-witness` edge — the crate-store commit
    // (in rustc_mir_build) filters recorded roots by the enabled-set predicate
    // and encodability, so here we gate only on the mint flag, root-ness, and
    // error-freedom. Inert (never records) unless `-Z trust-witness=mint:<dir>` is set.
    // Children fold into their root (typeck_with_inspect returns the root's
    // results for a child def_id), so recording only the direct root call keeps
    // the store per-root. Pure side-effect: never changes `r`.
    if tcx.trust_witness_mint_active()
        && tcx.typeck_root_def_id_local(def_id) == def_id
        && r.tainted_by_errors.is_none()
        // Concurrent rank-8 fix: replay skips this provider, so a root that emitted
        // a typeck warning or fulfilled a lint expectation is not replayable unless
        // those side effects are serialized — don't mint it.
        && typeck_was_diagnostic_free
        // Router (AUTO mode only): skip bodies too small for a later replay to
        // beat fresh typeck. An explicit `-Ztrust-witness=mint:<dir>` mints every root.
        && !tcx.trust_witness_mint_router_skip(def_id)
    {
        tcx.trust_witness_record_mint(def_id);
    }
    // Trust (A1-A3): PRECISE-key shadow-mint parity MEASUREMENT. The edit-loop precise
    // key is NEVER trusted — this only records whether it retrieved a witness that
    // byte-matches real typeck `r`, so its divergence rate (the negative-information
    // hole) can be measured over a corpus with zero behavioral risk. Always returns `r`.
    if tcx.trust_witness_precise()
        && tcx.typeck_root_def_id_local(def_id) == def_id
        && std::env::var_os("TRUST_WITNESS_STATS").is_some()
    {
        if let Some(matched) = tcx.trust_witness_parity_check(def_id, r) {
            eprintln!(
                "TRUST_REPLAY PARITY-{} {def_id:?}",
                if matched { "MATCH" } else { "DIVERGE" }
            );
        }
    }
    r
...
fn report_unexpected_variant_res(
    ...
    let mut err = tcx
        ...
        .with_code(err_code);
    match res {
        Res::Def(DefKind::Fn | DefKind::AssocFn, _) if err_code == E0164 => err
            .with_span_label(span, "`fn` calls are not allowed in patterns")
            .with_help("for more information, see the Trust Book's patterns chapter"),
...
fn fatally_break_rust(tcx: TyCtxt<'_>, span: Span) -> ! {
    ...
    // Trust: the shipped driver is `trustc`; keep the ICE version note
    // consistent with the panic-hook banner (which already says trustc).
    diag.note(format!("trustc {} running on {}", tcx.sess.cfg_version, config::host_tuple(),));
