...
pub enum TokenKind {
    ...
    /// `?`
    Question,
    /// A standalone `'` punctuation token. Used by proc macros and by the
    /// Trust lexer for an unambiguous post-state prime; real Rust lifetimes
    /// remain represented by [`TokenKind::Lifetime`].
    SingleQuote,
    /// `(`
    OpenParen,
    /// `)`
