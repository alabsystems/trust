...
pub enum ExprKind {
    ...
    ///
    /// Trust: the fourth field carries first-class loop clauses
    /// (`while expr invariant P decreases e { block }`, two-language design
    /// E4/E5) — grammar positions vanilla Rust rejects, so `None` for all
    /// vanilla programs.
    While(Box<Expr>, Box<Block>, Option<Label>, Option<Box<LoopContract>>),
...
// Trust: the first-class clause vocabulary and its authored-order
// reconstruction are Trust's own addition to the AST; they live in their
// own file so upstream merges of this one never touch them.
mod trust_contract;
pub use self::trust_contract::*;

#[derive(Clone, Encodable, Decodable, Debug, Default, Walkable)]
pub struct FnContract {
    /// tRustc: first-class function contract syntax. Runtime checking is a
    /// lowering choice; verification must preserve this structure through a
    /// typed contract query instead of recovering source text later.
    ...
    pub requires_clauses: ThinVec<Box<Expr>>,
    pub ensures_clauses: ThinVec<Box<Expr>>,
    /// Trust: OPAQUE trust-spec clauses (`trust::requires` / `trust::ensures`
    /// payloads written in compatibility spec syntax — `result`, `old()`,
    /// bounded `forall`/`exists`, `==>` —
    /// or otherwise not admissible to the typed lane). Only the payload SPAN is
    /// kept: the clause is verifier vocabulary, not Rust, so it must never be
    /// name-resolved, def-collected, or type-checked. The span flows into
    /// `rustc_hir::FnContract` where the `trust_contracts` query recovers the
    /// predicate text from the source map (its snippet lane).
    pub trust_opaque_requires: ThinVec<Span>,
    pub trust_opaque_ensures: ThinVec<Span>,
    /// Trust: FIRST-CLASS signature clauses
    /// (`fn f(..) requires P ensures Q decreases e`).
    /// These are verifier-language predicates, not Rust expressions, so they
    /// remain span-only just like opaque attribute clauses. They are stored
    /// separately so lowering/query code can give native `ensures result ...`
    /// its bare-result semantics without accepting a malformed bare
    /// `#[trust::ensures(result ...)]` attribute as though it were a closure.
    pub trust_native_requires: ThinVec<TrustNativeClause>,
    pub trust_native_ensures: ThinVec<TrustNativeClause>,
    /// Trust: FIRST-CLASS function-recursion termination measure
    /// (`fn f(..) decreases e { .. }`, ratified E5). The parser enforces at
    /// most one clause; a vector keeps the physical lane independently
    /// checkable by the parser-authored ordering metadata.
    pub trust_native_decreases: ThinVec<TrustNativeClause>,
    /// One marker per authored clause, in exact parser encounter order.
    ///
    /// This metadata has no AST children or spans to visit; lowering validates
    /// every ordinal and lane reference before trusting it.
    #[visitable(ignore)]
    pub clause_order: ThinVec<FnContractClauseMarker>,
}
...
pub enum ItemKind {
    ...
    /// Trust: a `clean { … }` PARSER ISLAND (two-language design E10) — Clean
    /// (Lean) declarations embedded in a Rust file. The payload is the island
    /// body's SPAN only: the text is verifier vocabulary, never name-resolved,
    /// def-collected, type-checked, or code-generated by rustc; the verify
    /// pass recovers it through the source map, parses it with the real Clean
    /// parser, and kernel-checks the declarations (rejects fail the build).
    CleanIsland(Span),
    /// A type alias (`type`).
    ///
    /// E.g., `type Foo = Bar<u8>;`.
...
impl ItemKind {
    ...
    pub fn article(&self) -> &'static str {
        ...
        match self {
            ...
            | GlobalAsm(..) | CleanIsland(..) | TyAlias(..) | Struct(..) | Union(..)
            | Trait(..) | TraitAlias(..) | MacroDef(..) | Delegation(..) | DelegationMac(..) => "a",
    ...
    pub fn descr(&self) -> &'static str {
        match self {
            ...
            ItemKind::CleanIsland(..) => "Clean island",
