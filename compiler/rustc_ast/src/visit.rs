...
macro_rules! common_visitor_and_walkers {
    ($(($mut: ident))? $Visitor:ident$(<$lt:lifetime>)?) => {
        ...
        impl_visitable_list!(<$($lt)? $($mut)?>
            ...
            ThinVec<TyPat>,
            ThinVec<EiiImpl>,
            // Trust: opaque trust-spec contract clause spans on `FnContract`
            // (span-only; visiting a `Span` is a no-op for `Visitor` and
            // `visit_span` for `MutVisitor`).
            ThinVec<Span>,
            // Trust: native contract clauses (predicate + canonical citation).
            ThinVec<TrustNativeClause>,
        );
        ...
        impl WalkItemKind for ItemKind {
            ...
            fn walk<$($lt,)? V: $Visitor$(<$lt>)?>(
                ...
                match self {
                    ...
                    // Trust: a Clean island is span-only verifier vocabulary —
                    // nothing for AST visitors to walk.
        ...
        impl_walkable!(|&$($mut)? $($lt)? self: Expr, vis: &mut V| {
            ...
            match kind {
                ...
                // Trust: LoopContract/LoopClause are metadata-only Walkable
                // nodes. Their Span fields participate in mutable remapping,
                // while no verifier payload is represented or visited as a
                // Rust expression.
                ExprKind::While(subexpression, block, opt_label, contract) =>
                    visit_visitable!($($mut)? vis, subexpression, block, opt_label, contract),
