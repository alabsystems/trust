...
struct Lexer<'psess, 'src> {
    ...
    /// Trust: end position of the last identifier (or fallback prime) token,
    /// used to recognize post-state prime notation (`balance'`, two-language
    /// design D2 / ruling Q9): a `'` immediately adjacent to an identifier
    /// that does not begin a valid char literal or lifetime.
    trust_last_ident_end: Option<BytePos>,
...
impl<'psess, 'src> Lexer<'psess, 'src> {
    ...
    fn next_token_from_cursor(&mut self) -> (Token, bool) {
        ...
        loop {
            ...
            // Trust: post-state prime notation (two-language design D2,
            // ruling Q9). A `'` immediately adjacent to a preceding
            // identifier that does NOT form a valid char literal or lifetime
            // can only be a lex error in vanilla Rust (unterminated or
            // multi-codepoint char literal), so reinterpreting it as a
            // one-char prime token changes no valid program. `x'a'` stays an
            // identifier followed by a char literal; `x'static` stays an
            // identifier followed by a lifetime; `balance' == amount` lexes
            // as `balance` `'` `==` `amount`. The remainder after the quote
            // is re-lexed (the char-literal scan would otherwise have
            // swallowed it). Chained primes (`x''`) fold one at a time.
            if let rustc_lexer::TokenKind::Literal {
                kind: LiteralKind::Char { terminated },
                suffix_start,
            } = token.kind
                && self.trust_last_ident_end == Some(start)
                && Self::trust_char_literal_is_prime_candidate(
                ...
                )
            {
                ...
                self.trust_last_ident_end = Some(self.pos);
                let span = self.mk_sp(start, self.pos);
                // Keep primes identity-distinct from every legal Rust
                // lifetime. In particular, a user-written `'__trust_prime`
                // must never alias this recovery path in token matching,
                // diagnostics, pretty-printing, or proc-macro conversion.
                return (Token::new(token::SingleQuote, span), preceded_by_whitespace);
            }
            ...
            let kind = match token.kind {
                ...
                rustc_lexer::TokenKind::UnknownPrefix => {
                    // Trust: `ident'` where the quote does NOT begin a valid
                    // char literal or lifetime is post-state prime notation
                    // (D2/Q9), not a reserved prefix: pass the identifier
                    // through silently; the quote then lexes as a prime via
                    // the Char-arm fold above. (`x'a'` / `x'static` keep
                    // their vanilla treatment, including the 2021+
                    // reserved-prefix diagnostics.)
                    if !self.trust_prime_follows() {
            ...
            let span = self.mk_sp(start, self.pos);
            // Trust: prime-notation adjacency tracking (see above).
            self.trust_last_ident_end =
                if matches!(kind, token::Ident(..)) { Some(self.pos) } else { None };
            return (Token::new(kind, span), preceded_by_whitespace);
        }
    }

    /// Trust: whether the character at the current position begins a prime
    /// (Q9): a `'` that does NOT lex as a valid lifetime or char literal.
    /// Probed with a fresh sub-lexer so the main cursor is untouched.
    fn trust_prime_follows(&self) -> bool {
        let rel = (self.pos - self.start_pos).to_usize();
        let rest = &self.src[rel..];
        if !rest.starts_with('\'') {
            return false;
        }
        let mut probe = Cursor::new(rest, FrontmatterAllowed::No);
        let tok = probe.advance_token();
        match tok.kind {
            rustc_lexer::TokenKind::Literal {
                kind: LiteralKind::Char { terminated },
                suffix_start,
            } => Self::trust_char_literal_is_prime_candidate(
                &rest[..tok.len as usize],
                terminated,
                suffix_start,
                tok.len,
            ),
    ...
    /// Trust: whether a raw char-literal token is eligible to be split into a
    /// prime. This is the single validity predicate used both while probing an
    /// `UnknownPrefix` and while folding its following token, so the two paths
    /// cannot disagree. Valid chars and all suffixed char literals remain
    /// vanilla Rust; only otherwise-invalid quote sequences are candidates.
    fn trust_char_literal_is_prime_candidate(
        text: &str,
        terminated: bool,
        suffix_start: u32,
        token_len: u32,
        ...
        if suffix_start != token_len {
            // Suffixed char literal: vanilla lexing already owns this case.
            return false;
        }
        debug_assert!(text.starts_with('\'') && text.ends_with('\''));
        let inner = &text[1..text.len() - 1];
        let mut valid = true;
        check_for_errors(inner, Mode::Char, |_, err| {
            if err.is_fatal() {
                valid = false;
            }
        });
