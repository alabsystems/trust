...
#[test]
fn function_contract_markers_preserve_global_interleaving_across_all_lanes() {
    create_default_session_globals_then(|| {
        let psess = ParseSess::new();
        let source = "fn ordered(x: u32) -> u32 \
            trust_contract_ensures { |ret| *ret >= x } \
            requires x > 0 \
            decreases x \
            trust_contract_requires { forall(i, 0..x, i <= x) } \
            ensures result >= x \
            contract_requires { x > 0 } \
            trust_contract_ensures { result >= x } \
            { x }";
        let item = parse_item_from_source_str(
            filename(psess.source_map(), "contract-order.rs"),
            source.to_owned(),
            &psess,
        )
        .unwrap()
        ...
        let ast::ItemKind::Fn(function) = &item.kind else {
            panic!("contract-order fixture must parse as a function");
        };
        let contract = function.contract.as_deref().expect("all clauses must be retained");

        use ast::ast::{FnContractClauseKind as Kind, FnContractClauseLane as Lane};
        let markers = contract
            .clause_order
            .iter()
            .map(|marker| (marker.ordinal, marker.kind, marker.lane, marker.lane_index))
            .collect::<Vec<_>>();
        assert_eq!(
            markers,
            [
                (0, Kind::Ensures, Lane::Typed, 0),
                (1, Kind::Requires, Lane::Native, 0),
                (2, Kind::Decreases, Lane::Native, 0),
                (3, Kind::Requires, Lane::Opaque, 0),
                (4, Kind::Ensures, Lane::Native, 0),
                (5, Kind::Requires, Lane::Typed, 0),
                (6, Kind::Ensures, Lane::Opaque, 0),
            ]
        );
        assert_eq!(contract.requires_clauses.len(), 1);
        assert_eq!(contract.ensures_clauses.len(), 1);
        assert_eq!(contract.trust_opaque_requires.len(), 1);
        assert_eq!(contract.trust_opaque_ensures.len(), 1);
        assert_eq!(contract.trust_native_requires.len(), 1);
        assert_eq!(contract.trust_native_ensures.len(), 1);
        assert_eq!(contract.trust_native_decreases.len(), 1);
        assert_eq!(
            psess
                .source_map()
                .span_to_snippet(contract.trust_native_decreases[0].predicate)
                .unwrap(),
            "x"
        );
        assert_eq!(
            contract.trust_native_decreases[0].payload.as_str(),
            "x",
            "the parser must capture the token-rendered payload spelling"
        );
        assert!(
            item_to_string(&item).contains("trust_decreases_native()"),
            "rustc_ast_pretty must retain a visible marker for native function decreases"
        );
    });
}

#[test]
fn duplicate_function_decreases_is_rejected() {
    create_default_session_globals_then(|| {
        let psess = ParseSess::new();
        let result = parse_item_from_source_str(
            filename(psess.source_map(), "duplicate-function-decreases.rs"),
            "fn bad(n: u32) decreases n decreases n - 1 { }".to_owned(),
            &psess,
        );
        let Err(error) = result else {
            panic!("a function may not author two decreases clauses");
        };
        error.cancel();
    });
}
...
#[test]
fn trust_citation_has_canonical_identity_and_authored_span() {
    create_default_session_globals_then(|| {
        let psess = ParseSess::new();
        let source = "fn cited(x: u32) -> u32 ensures result == x \
            by Clean /* keep */ . Lemmas . /* exact */ bound { x }";
        let item = parse_item_from_source_str(
            filename(psess.source_map(), "citation.rs"),
            source.to_owned(),
            &psess,
        )
        .unwrap()
        .unwrap();
        let ast::ItemKind::Fn(function) = &item.kind else {
            panic!("citation fixture must parse as a function");
        };
        let contract = function.contract.as_deref().expect("native contract must be retained");
        let citation =
            contract.trust_native_ensures[0].citation.expect("citation suffix must be retained");

        assert_eq!(citation.name, Symbol::intern("Clean.Lemmas.bound"));
        assert_eq!(
            psess.source_map().span_to_snippet(citation.span).unwrap(),
            "Clean /* keep */ . Lemmas . /* exact */ bound"
        );
    });
}

#[test]
fn trust_by_identifiers_remain_predicate_vocabulary() {
    create_default_session_globals_then(|| {
        let psess = ParseSess::new();
        let source = "fn by_predicate(by: u32) -> u32 \
            requires by == by ensures foo(by) == by { by }";
        let item = parse_item_from_source_str(
            filename(psess.source_map(), "by-predicate.rs"),
            source.to_owned(),
            &psess,
        )
        .unwrap()
        .unwrap();
        let ast::ItemKind::Fn(function) = &item.kind else {
            panic!("by-identifier fixture must parse as a function");
        };
        let contract = function.contract.as_deref().expect("native contract must be retained");
        let requires = contract.trust_native_requires[0];
        let ensures = contract.trust_native_ensures[0];

        assert_eq!(requires.citation, None);
        assert_eq!(ensures.citation, None);
        assert_eq!(psess.source_map().span_to_snippet(requires.predicate).unwrap(), "by == by");
        assert_eq!(psess.source_map().span_to_snippet(ensures.predicate).unwrap(), "foo(by) == by");
        // The token-rendered payload is the spelling authority whenever macro
        // expansion collapses payload spans; it must match the authored text.
        assert_eq!(requires.payload.as_str(), "by == by");
        assert_eq!(ensures.payload.as_str(), "foo(by) == by");
    });
}

#[test]
fn trust_predicate_final_by_does_not_steal_following_citation_marker() {
    create_default_session_globals_then(|| {
        let psess = ParseSess::new();
        let source = "fn cited_by(by: u32) -> u32 ensures foo(by) == by \
            by Clean /* keep */ . Lemmas . bound { by }";
        let item = parse_item_from_source_str(
            filename(psess.source_map(), "citation-after-by.rs"),
            source.to_owned(),
            &psess,
        )
        .unwrap()
        .unwrap();
        let ast::ItemKind::Fn(function) = &item.kind else {
            panic!("citation-after-by fixture must parse as a function");
        };
        let contract = function.contract.as_deref().expect("native contract must be retained");
        let ensures = contract.trust_native_ensures[0];
        let citation = ensures.citation.expect("the later `by` must start the citation");

        assert_eq!(psess.source_map().span_to_snippet(ensures.predicate).unwrap(), "foo(by) == by");
        assert_eq!(citation.name, Symbol::intern("Clean.Lemmas.bound"));
        assert_eq!(
            psess.source_map().span_to_snippet(citation.span).unwrap(),
            "Clean /* keep */ . Lemmas . bound"
        );
    });
}
