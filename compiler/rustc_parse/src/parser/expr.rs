...
impl<'a> Parser<'a> {
    ...
    fn parse_expr_while(&mut self, opt_label: Option<Label>, lo: Span) -> PResult<'a, Box<Expr>> {
        ...
        // Trust: first-class loop clauses between the condition and the body
        // (`while c invariant P decreases e { .. }`, two-language design
        // E4/E5) — a position vanilla Rust rejects.
        let contract = self.parse_trust_loop_contract()?;
        let (attrs, body) = self
    ...
    // Trust: `output` lets contract synthesis pin the closure's return type
    // (`-> bool`), so a non-bool contract body fails as an E0308 mismatch
    // inside the closure — matching upstream — rather than as an E0271
    // `Fn() -> bool` bound failure at the intrinsic call site.
    pub(crate) fn mk_closure_expr(
        &self,
        span: Span,
        ...
        self.mk_expr(
            ...
            ast::ExprKind::Closure(Box::new(ast::Closure {
                ...
                coroutine_kind: None,
                fn_decl: Box::new(rustc_ast::FnDecl { inputs: Default::default(), output }),
                fn_arg_span: span,
                fn_decl_span: span,
                body,
            })),
