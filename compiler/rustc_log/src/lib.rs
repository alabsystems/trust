    ...
    T: BuildSubscriberRet,
{
    let filter = match cfg.filter {
        ...
        // Trust: the in-process verification backends (the ay solver family)
        // emit WARN-level solver telemetry — e.g. ay_chc's "SAT model …
        // returning Unknown instead of accepting as Sat" fail-closed note.
        // Under the default WARN filter that telemetry lands on the USER's
        // compile stderr — polluting builds and UI-test expectations — even
        // though it is solver-internal diagnosis, not a compiler diagnostic.
        // Cap those targets at ERROR by default; setting RUSTC_LOG explicitly
        // still opts into everything.
        _ => {
            let mut filter =
                EnvFilter::default().add_directive(Directive::from(LevelFilter::WARN));
            for target in
                ["ay_chc", "ay_dpll", "ay_core", "ay_lia", "ay_lra", "ay_nia", "ay_proof"]
            {
                if let Ok(directive) = format!("{target}=error").parse() {
