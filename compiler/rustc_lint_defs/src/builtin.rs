...
declare_lint! {
    /// Trust: rust-lang#100914 — The `large_stack_frame` lint detects functions
    /// whose estimated stack frame size exceeds the configured
    /// `-Z stack_frame_limit`.
    ///
    /// ### Explanation
    ///
    /// Very large stack frames can cause stack overflows and may be
    /// miscompiled by LLVM. This lint warns before that happens.
    pub LARGE_STACK_FRAME,
    Allow,
    "detects functions with large estimated stack frames",
}
