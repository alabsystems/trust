...
impl<'a> State<'a> {
    ...
    pub(crate) fn print_item(&mut self, item: &ast::Item) {
        ...
        match &item.kind {
            ...
            ast::ItemKind::CleanIsland(_) => {
                // Trust: span-only Clean island — the body is not reproducible
                // from the AST (same policy as opaque contract clauses); print
                // a placeholder that is NOT valid vanilla Rust so elision is
                // visible rather than silently round-trippable.
                let (cb, ib) = self.head("clean");
                self.word(" { /* Clean island (span-only) */ }");
                self.end(ib);
                self.end(cb);
            }
    ...
    pub(crate) fn print_contract(&mut self, contract: &ast::FnContract) {
        let Ok(ordered) = contract.ordered_clause_refs() else {
            // Pretty output is diagnostic and has no fallible return channel.
            // Make corrupt metadata visible without manufacturing a grouped or
            // span-sorted order that another tool could mistake for authored
            // identity.
            self.word("trust_invalid_contract_order");
            self.popen();
            self.pclose();
            return;
        };
        ...
        for clause in ordered {
            let keyword = match (clause.marker().kind, clause.clause()) {
                (ast::FnContractClauseKind::Requires, ast::FnContractClauseRef::Typed(pred)) => {
                ...
                (ast::FnContractClauseKind::Ensures, ast::FnContractClauseRef::Typed(pred)) => {
                ...
                (ast::FnContractClauseKind::Requires, ast::FnContractClauseRef::Opaque(_)) => {
                    "trust_requires_opaque"
                }
                (ast::FnContractClauseKind::Ensures, ast::FnContractClauseRef::Opaque(_)) => {
                    "trust_ensures_opaque"
                }
                (ast::FnContractClauseKind::Requires, ast::FnContractClauseRef::Native(_)) => {
                    "trust_requires_native"
                }
                (ast::FnContractClauseKind::Ensures, ast::FnContractClauseRef::Native(_)) => {
                    "trust_ensures_native"
                }
                (ast::FnContractClauseKind::Decreases, ast::FnContractClauseRef::Native(_)) => {
                    "trust_decreases_native"
                }
                (
                    ast::FnContractClauseKind::Decreases,
                    ast::FnContractClauseRef::Typed(_) | ast::FnContractClauseRef::Opaque(_),
                ) => unreachable!("validated contract order excludes non-native decreases lanes"),
            };
            self.word(keyword);
            self.popen();
            self.pclose();
