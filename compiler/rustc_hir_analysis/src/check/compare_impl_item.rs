...
fn compare_method_predicate_entailment<'tcx>(
    ...
    // Upstream does NOT assume the impl header's WF types here: reporting
    // their normalization failures from this early batch relocates E0277s
    // to the impl-header span (issue-79224 wrong-span + duplicate) and
    // feeding them to the region check is more permissive than upstream.
...
pub(super) fn check_type_bounds<'tcx>(
    ...
    let mut assumed_wf_types = ocx.assumed_wf_types_and_report_errors(param_env, impl_ty_def_id)?;
    ...
    // Trust: the extra WF proof inputs belong to the batteries-on verifier only.
    // Use the shared predicate so this type-system hook and the MIR pass agree;
    // `-Ztrust-verify=off` disables both for vanilla/bootstrap compiles.
    let trust_associated_type_wf_proofs = tcx.sess.trust_verification_enabled();
    let unnormalized_obligations =
        trust_associated_type_wf_proofs.then(|| obligations.clone()).unwrap_or_default();
    ...
    // Associated type bounds can introduce WF types that are not part of the
    // impl header. Feed binder-free, non-alias source types into region
    // resolution so their implied outlives facts are available while checking
    // the normalized bounds. The assumptions intentionally come from the
    // unnormalized trait bounds: a projection normalized through
    // `normalize_param_env` may contain the impl's RHS, which would let that
    // RHS justify its own outlives requirements.
    if trust_associated_type_wf_proofs {
        for obligation in &unnormalized_obligations {
            let Some(clause) = obligation.predicate.as_clause() else {
                continue;
            };

            for wf_obligation in traits::wf::clause_obligations(
                &infcx,
                ...
                if let Some(ty::PredicateKind::Clause(ty::ClauseKind::WellFormed(term))) =
                    wf_obligation.predicate.kind().no_bound_vars()
                    && let Some(ty) = term.as_type()
                    && !ty.has_aliases()
                {
                    assumed_wf_types.insert(ty);
                }
            }
        }
    }

    // Prove the normalized WF obligations for impl associated types. Higher-ranked
    // WF obligations are still registered here, but are not converted into assumed
    // region seeds above because that requires placeholder-aware higher-ranked
    // assumptions.
    //
    // For trait associated type defaults, `check_type_bounds` is called with the
    // same associated item as both the trait item and impl item. The default RHS is
    // already well-formedness checked by `wfcheck::check_associated_item`; proving
    // these derived WF obligations here reports the same invalid default type
    // twice under compiletest's `-Zdeduplicate-diagnostics=no` mode.
    let mut wf_obligations = Vec::new();
    if trust_associated_type_wf_proofs && impl_ty.def_id != trait_ty.def_id {
        for obligation in &obligations {
            ...
            for wf_obligation in traits::wf::clause_obligations(
                ...
                wf_obligations.push(wf_obligation);
            }
        }
    }
    ...
    // Check that all obligations are satisfied by the implementation's
    // version.
    ocx.register_obligations(wf_obligations);
