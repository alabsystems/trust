// Env-gated generator corpus sweep: run every case under the generator +
// iterator directories through BOTH trust_js_sem and the Node trace driver and
// require byte-for-byte trace-equality — refusals are sound and counted, a
// wrong trace is gate-fatal. This is the adversarial arbiter for the generator
// head. Set TRUST_JS_NODE (and optionally TRUST_JS262_CORPUS) to run.
//
// Author: Andrew Yates
...
/// (directory, per-dir cap). Generous caps: the whole generator bucket.
const GEN_DIRS: &[(&str, usize)] = &[
    ("test/language/statements/generators", 400),
    ("test/language/expressions/generators", 400),
    ("test/built-ins/GeneratorFunction", 60),
    ("test/built-ins/GeneratorPrototype", 120),
    ("test/built-ins/Array/prototype/values", 30),
    ("test/built-ins/Array/prototype/keys", 30),
    ("test/built-ins/Array/prototype/entries", 30),
];
...
fn parse_frontmatter(body: &str) -> Frontmatter {
    ...
    Frontmatter {
        includes,
        flags,
        negative_parse: fm.contains("phase: parse") || fm.contains("phase: early"),
    }
}
...
fn generators_corpus_vs_node() {
    let Ok(node) = std::env::var("TRUST_JS_NODE") else {
        eprintln!("SKIP generators_corpus_vs_node: set TRUST_JS_NODE to run");
        return;
    };
    ...
    eprintln!("== generators corpus: covered {covered} (equal {equal}) / refused {refused} ==");
    ...
    assert!(
        failures.is_empty(),
        "generator corpus failures (wrong trace is never acceptable) ({}):\n{}",
        failures.len(),
