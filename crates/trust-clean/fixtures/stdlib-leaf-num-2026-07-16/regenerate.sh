#!/bin/sh
# Regenerate or validate the 24 stdlib integer-leaf dumps and their seventeen
# negative/positive controls using this checkout. Nothing under another
# worktree is consulted, and committed dumps are replaced only after all checks pass.
set -eu
...
TMP=$(mktemp -d)
CANDIDATE_ROOT="$HERE/.regenerate-num.$$"
BACKUP_ROOT="$HERE/.dumps.previous.$$"
LOCK_DIR="$HERE/.regenerate-num.lock"
VALIDATION_TARGET_DIR=${TRUST_NUM_VALIDATION_TARGET_DIR:-$TMP/cargo-target}
cleanup() {
  ...
  rm -rf "$TMP" "$CANDIDATE_ROOT" "$LOCK_DIR"
}
if ! mkdir "$LOCK_DIR"; then
  echo "error: another numeric corpus validation/regeneration is already running" >&2
  rm -rf "$TMP"
  exit 1
fi
trap cleanup EXIT
...
if test "$VALIDATE_ONLY" -eq 0; then
  ...
  if test "$dump_status" -ne 0; then
    ...
    echo "accepted recorded post-dump NonZero normalization failure; exact checks follow" >&2
  fi
  test "$total_dumps" -ge 27000 || {
    echo "error: core extraction was partial: only $total_dumps dumps" >&2
    exit 1
  }
...
wanted = {
    *(f"num::<impl {ty}>::{method}" for ty in signed for method in ("is_positive", "is_negative", "signum")),
    *(f"num::<impl {ty}>::{method}" for ty in unsigned for method in ("is_power_of_two", "count_ones", "trailing_zeros")),
}
...
print(f"scanned {scanned} extracted MIR bodies; selected {len(found)} exact numeric targets")
...
cp -R "$HERE/forgeries" "$CANDIDATE_ROOT/forgeries"
...
test "$dump_count" -eq 24 || {
  echo "error: expected exactly 24 numeric-family dumps, found $dump_count" >&2
  exit 1
}
test "$forgery_count" -eq 17 || {
  echo "error: expected exactly 17 numeric forgeries, found $forgery_count" >&2
  exit 1
}
...
test "$(wc -l < "$TMP/expected-forgeries" | tr -d ' ')" -eq 17
test "$(uniq "$TMP/expected-forgeries" | wc -l | tr -d ' ')" -eq 17
...
echo "validated machine fields: 24/24 fully faithful, kernel_rejected=0; seventeen controls exact"
