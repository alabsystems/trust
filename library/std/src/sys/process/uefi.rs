...
pub fn output(command: &mut Command) -> io::Result<(ExitStatus, Vec<u8>, Vec<u8>)> {
    ...
    // Set any new vars
    //
    // Blessed `env_mutation` site: UEFI has no real process spawning, so
    // `Command` env changes are applied to our own environment and rolled
    // back after the image runs.
    #[allow(env_mutation)]
    if let Some(e) = &env {
        for (k, (_, v)) in e {
            match v {
    ...
    //
    // Blessed `env_mutation` site: see above.
    #[allow(env_mutation)]
    if let Some(e) = env {
        for (k, (v, _)) in e {
            match v {
