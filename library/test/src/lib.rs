...
pub fn test_main_static_abort(tests: &[&TestDescAndFn]) {
    ...
    if let Ok(name) = env::var(SECONDARY_TEST_INVOKER_VAR) {
        ...
        // Blessed `env_mutation` site: libtest's secondary-invoker handshake
        // (library crates are only ever built by the in-tree compiler, which
        // knows the trust-only lint).
        #[allow(env_mutation)]
        unsafe {
